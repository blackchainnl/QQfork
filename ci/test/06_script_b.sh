#!/usr/bin/env bash
#
# Copyright (c) 2018-present Blackcoin Core Developers
# Copyright (c) 2018-present Blackcoin More Developers
# Copyright (c) 2018-present Blackcoin Developers
...
DISTDIR="blackcoin-$HOST"

cd "${BASE_BUILD_DIR}/${DISTDIR}"
...
if [ "$RUN_FUNCTIONAL_TESTS" = "true" ]; then
  bash -c "LD_LIBRARY_PATH=${DEPENDS_DIR}/${HOST}/lib ${TEST_RUNNER_ENV} test/functional/test_runner.py --ci $MAKEJOBS --tmpdirprefix ${BASE_SCRATCH_DIR}/test_runner/ --ansi --combinedlogslen=2000 --timeout-factor=${TEST_RUNNER_TIMEOUT_FACTOR} ${TEST_RUNNER_EXTRA} --quiet --failfast"
fi
...
if [ "${RUN_TIDY}" = "true" ]; then
  ...
  python3 "/include-what-you-use/iwyu_tool.py" \
           -p . "${MAKEJOBS}" \
           -- -Xiwyu --cxx17ns -Xiwyu --mapping_file="${BASE_BUILD_DIR}/${DISTDIR}/contrib/devtools/iwyu/bitcoin.core.imp" \
