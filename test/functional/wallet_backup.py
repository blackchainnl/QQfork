...
Miner creates enough blocks so 1 2 3 each have one mature
coins to spend.
...
Sanity check:
  Sum(1,2,3,4 balances) == 24*REGTEST_BLOCK_REWARD
...
class WalletBackupTest(BitcoinTestFramework):
    ...
    def one_send(self, from_node, to_address):
        if (randint(1,2) == 1):
            ...
            self.relay_wallet_tx(from_node, txid)

    def relay_wallet_tx(self, from_node, txid):
        raw_tx = self.nodes[from_node].gettransaction(txid)["hex"]
        for node_num, node in enumerate(self.nodes):
            if node_num == from_node:
                continue
            try:
    ...
    def run_test(self):
        ...
        assert_equal(self.nodes[2].getbalance(), REGTEST_BLOCK_REWARD)
        ...
        # With this fork's 10-block regtest maturity, the three initial wallet
        # rewards, ten round-mining rewards, and eleven final miner rewards are
        # mature here.
        assert_equal(total, 24 * REGTEST_BLOCK_REWARD)
