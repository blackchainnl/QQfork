...
class AbandonConflictTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        self.num_nodes = 2
        self.extra_args = [["-minrelaytxfee=0.00001"], ["-minrelaytxfee=0.00001"]]
    ...
    def run_test(self):
        ...
        # This fork's relay policy can keep these small wallet transactions local to node0.
        # Mine them where they were created; the test only needs confirmed wallet txs here.
        ...
        txAB1_alice_change = Decimal("15") - RAW_TX_FEE
        outputs[alice.getnewaddress()] = txAB1_alice_change
        ...
        nAB = next(tx_out["vout"] for tx_out in alice.gettransaction(txAB1)["details"] if tx_out["amount"] == txAB1_alice_change)
        ...
        outputs = {}
        txABC2_change = txAB1_alice_change + Decimal("10") - RAW_TX_FEE
        outputs[alice.getnewaddress()] = txABC2_change
        signed2 = alice.signrawtransactionwithwallet(alice.createrawtransaction(inputs, outputs))
        txABC2 = self.nodes[0].sendrawtransaction(signed2["hex"])

        # Create a child tx spending ABC2
        signed3_change = txABC2_change - RAW_TX_FEE
        ...
        newbalance = alice.getbalance()
        assert_equal(newbalance, balance - Decimal("20") + txAB1_alice_change)
        balance = newbalance

        # Send child tx again so it is unabandoned
        self.nodes[0].sendrawtransaction(signed2["hex"])
        newbalance = alice.getbalance()
        assert_equal(newbalance, balance - Decimal("10") - txAB1_alice_change + txABC2_change)
        balance = newbalance
        ...
        assert_equal(len(self.nodes[0].getrawmempool()), 0)
        newbalance = alice.getbalance()
        assert_equal(newbalance, balance - txABC2_change)
        balance = newbalance
        ...
        inputs.append({"txid": txA, "vout": nA})
        outputs = {}
        outputs[self.nodes[1].getnewaddress()] = Decimal("4") - RAW_TX_FEE
        outputs[bob.getnewaddress()] = Decimal("6")
