...
BLACKCOIN_POW_SUBSIDY = 10000 * COIN
LARGE_BLOCK_TX_FEE = COIN
MIN_TX_FEE = 10000
MIN_BLOCK_TX_FEE = 100000
P2SH_SIGOPS_CHAIN_FUND = 10 * COIN
...
class CBrokenBlock(CBlock):
    ...
    def serialize(self, with_witness=False, with_pos_marker=True):
        r = b""
        if with_pos_marker:
            r += super(CBlock, self).serialize_pos_marker()
        else:
    ...
    def normal_serialize(self):
        return super().serialize(with_pos_marker=False)
...
class FullBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.raise_spend_output_for_fee(b23, out[6])
        tx = CTransaction()
        ...
        self.raise_spend_output_for_fee(b24, out[7])
        script_length = (MAX_BLOCK_WEIGHT - b24.get_weight() - 276) // 4
        script_output = CScript([b'\x00' * (script_length + 1)])
        tx = CTransaction()
        tx.vout.append(CTxOut(0, script_output))
        tx.vin.append(CTxIn(COutPoint(b24.vtx[1].sha256, 0)))
        ...
        b25 = self.next_block(25, spend=out[8])
        self.send_blocks([b25], success=False, reject_reason='prev block invalid', reconnect=True)
        ...
        tx.vout.append(CTxOut(spend.vout[0].nValue - 1 - MIN_BLOCK_TX_FEE, CScript([OP_TRUE])))
        ...
        while total_weight < MAX_BLOCK_WEIGHT:
            ...
            tx_new.vout.append(CTxOut(tx_last.vout[1].nValue - 1 - MIN_BLOCK_TX_FEE, CScript([OP_TRUE])))
            tx_new.rehash()
        ...
        self.raise_spend_output_for_fee(b40, out[12], value=P2SH_SIGOPS_CHAIN_FUND)
        sigops = get_legacy_sigopcount_block(b40)
        numTxes = (MAX_BLOCK_SIGOPS - sigops) // b39_sigops_per_output
        assert_equal(numTxes <= b39_outputs, True)

        lastOutpoint = COutPoint(b40.vtx[1].sha256, 0)
        lastOutpointValue = b40.vtx[1].vout[0].nValue
        new_txs = []
        for i in range(1, numTxes + 1):
            tx = CTransaction()
            tx.vout.append(CTxOut(lastOutpointValue + b39.vtx[i].vout[0].nValue - MIN_BLOCK_TX_FEE, CScript([OP_TRUE])))
            ...
            lastOutpointValue = tx.vout[0].nValue

        b40_sigops_to_fill = MAX_BLOCK_SIGOPS - (numTxes * b39_sigops_per_output + sigops) + 1
        tx = CTransaction()
        tx.vin.append(CTxIn(lastOutpoint, b''))
        tx.vout.append(CTxOut(lastOutpointValue - MIN_BLOCK_TX_FEE, CScript([OP_CHECKSIG] * b40_sigops_to_fill)))
        tx.rehash()
        new_txs.append(tx)
        ...
        tx = CTransaction()
        tx.vin.append(CTxIn(lastOutpoint, b''))
        tx.vout.append(CTxOut(lastOutpointValue - MIN_BLOCK_TX_FEE, CScript([OP_CHECKSIG] * b41_sigops_to_fill)))
        tx.rehash()
        ...
        self.move_tip(55)
        self.next_block(57)
        tx = self.create_and_sign_transaction(out[16], 2 * MIN_BLOCK_TX_FEE + 1)
        tx1 = self.create_tx(tx, 0, tx.vout[0].nValue - MIN_BLOCK_TX_FEE)
        ...
        tx = self.create_and_sign_transaction(out[16], 5 * MIN_BLOCK_TX_FEE + 1)
        tx1 = self.create_tx(tx, 0, tx.vout[0].nValue - MIN_BLOCK_TX_FEE)
        tx2 = self.create_tx(tx1, 0, tx1.vout[0].nValue - MIN_BLOCK_TX_FEE)
        tx3 = self.create_tx(tx2, 0, tx2.vout[0].nValue - MIN_BLOCK_TX_FEE)
        tx4 = self.create_tx(tx3, 0, tx3.vout[0].nValue - MIN_BLOCK_TX_FEE)
        ...
        # The upstream BIP30 duplicate-coinbase fixture relies on reusing the
        # exact same coinbase script at a later height. This fork enforces the
        # coinbase-height prefix from block 1, so that fixture is structurally
        # impossible. Keep the later test topology by extending the chain with
        # ordinary valid blocks under the same labels.
        self.log.info("Extend chain past the inapplicable duplicate-coinbase fixture")
        self.move_tip(60)
        b61 = self.next_block(61)
        b_spend_dup_cb = self.next_block('spend_dup_cb')
        b_dup_2 = self.next_block('dup_2')
        self.send_blocks([b61, b_spend_dup_cb, b_dup_2], success=True)
        ...
        self.raise_spend_output_for_fee(regular_block, out[18], value=2 * COIN + 1)
        ...
        script_output = CScript([b'\x00' * script_length])
        tx.vout.append(CTxOut(1, script_output))
        tx.vin.append(CTxIn(COutPoint(b64a.vtx[1].sha256, 0)))
        b64a = self.update_block("64a", [tx])
        assert_greater_than(b64a.get_weight(), MAX_BLOCK_WEIGHT)
        ...
        assert b64.get_weight() <= MAX_BLOCK_WEIGHT
        self.blocks[64] = b64
        ...
        self.move_tip(64)
        self.next_block(65)
        tx1 = self.create_and_sign_transaction(out[19], out[19].vout[0].nValue - 2 * MIN_BLOCK_TX_FEE)
        tx2 = self.create_and_sign_transaction(tx1, tx1.vout[0].nValue - MIN_BLOCK_TX_FEE)
        ...
        self.move_tip(65)
        self.next_block(66)
        tx1 = self.create_and_sign_transaction(out[20], out[20].vout[0].nValue - MIN_BLOCK_TX_FEE)
        ...
        self.move_tip(69)
        self.next_block(72)
        tx1 = self.create_and_sign_transaction(out[21], 2 * MIN_BLOCK_TX_FEE + 1)
        tx2 = self.create_and_sign_transaction(tx1, tx1.vout[0].nValue - MIN_BLOCK_TX_FEE)
        ...
        self.submit_blocks_rpc([b80, b81, b82], expected_tip=b82)  # now this chain is longer, triggers re-org
        self.save_spendable_output()
        ...
        tx1 = self.create_and_sign_transaction(out[28], out[28].vout[0].nValue - 2 * MIN_BLOCK_TX_FEE, script)

        tx2 = self.create_and_sign_transaction(tx1, tx1.vout[0].nValue - MIN_BLOCK_TX_FEE, CScript([OP_TRUE]))
        ...
        tx1.vout.append(CTxOut(MIN_BLOCK_TX_FEE, CScript([OP_TRUE])))
        tx1.vout.append(CTxOut(MIN_BLOCK_TX_FEE + 1, CScript([OP_TRUE])))
        tx1.vout.append(CTxOut(MIN_BLOCK_TX_FEE + 1, CScript([OP_TRUE])))
        tx1.vout.append(CTxOut(MIN_BLOCK_TX_FEE, CScript([OP_TRUE])))
        tx1.calc_sha256()
        ...
        self.log.info("Test a max-weight-block re-org inside the rolling sync-checkpoint window")
        ...
        for i in range(89, LARGE_REORG_SIZE + 89):
            ...
            while True:
                ...
                probe_block = copy.deepcopy(b)
                self.raise_spend_output_for_fee(probe_block, spend, value=self.get_min_fee(tx), update_height=False)
                tx.vin[0].prevout.hash = probe_block.vtx[1].sha256
                tx.rehash()
                probe_block.vtx.append(tx)
                probe_block.hashMerkleRoot = probe_block.calc_merkle_root()
                weight_delta = MAX_BLOCK_WEIGHT - probe_block.get_weight()
                if weight_delta == 0:
                    old_sha256 = b.sha256
                    probe_block.solve()
                    self.block_heights[probe_block.sha256] = self.block_heights[old_sha256]
                    del self.block_heights[old_sha256]
                    self.tip = probe_block
                    self.blocks[i] = probe_block
                    b = probe_block
                    break
                assert_equal(weight_delta % 4, 0)
                script_length += weight_delta // 4
        ...
        self.submit_blocks_rpc(blocks, expected_tip=blocks[-1])
        chain1_tip = i
        chain1_tip_block = self.blocks[chain1_tip]
        ...
        self.submit_blocks_rpc(blocks2, expected_tip=chain1_tip_block)

        # extend alt chain to trigger re-org
        block = self.next_block("alt" + str(chain1_tip + 1))
        self.submit_blocks_rpc([block], expected_tip=block)
        alt_tip_block = block

        # ... and re-org back to the first chain
        self.move_tip(chain1_tip)
        block = self.next_block(chain1_tip + 1)
        self.submit_blocks_rpc([block], expected_tip=alt_tip_block)
    ...
    def raise_spend_output_for_fee(self, block, spend_tx, value=LARGE_BLOCK_TX_FEE, update_height=True):
        tx = block.vtx[1]
        delta = value - tx.vout[0].nValue
        if delta <= 0:
            return
        ...
        self.sign_tx(tx, spend_tx)
        tx.rehash()
        block.vtx[0].vout[0].nValue -= delta
        block.vtx[0].rehash()
    ...
    def submit_blocks_rpc(self, blocks, expected_tip):
        ...
        self.nodes[0].setmocktime(max(best_time, expected_tip.nTime))
        for block in blocks:
            # submitblock decodes with normal SER_NETWORK serialization, while
            # P2P block messages on this fork carry the Peercoin/Blackcoin PoS
            # marker. Side-chain submissions can return "inconclusive" before
            # the fork becomes active.
            result = self.nodes[0].submitblock(block.serialize(with_pos_marker=False).hex())
            assert result in (None, "inconclusive"), result
        assert_equal(self.nodes[0].getbestblockhash(), expected_tip.hash)

    def next_block(self, number, spend=None, additional_coinbase_value=0, script=CScript([OP_TRUE]), *, version=7):
        if self.tip is None:
        ...
        coinbase = create_coinbase(height, self.coinbase_pubkey)
        coinbase.vout[0].nValue = BLACKCOIN_POW_SUBSIDY
    ...
    def send_blocks(self, blocks, success=True, reject_reason=None, force_send=True, reconnect=False, timeout=960):
