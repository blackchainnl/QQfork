...
class AddressTypeTest(BitcoinTestFramework):
    ...
    def run_test(self):
        # Mine enough blocks on node5 to bring nodes out of IBD and make sure
        # that no coinbases are maturing for the nodes-under-test during the test.
        ...
        for explicit_type, multisig, from_node in itertools.product([False, True], do_multisigs, range(4)):
            ...
            # The output schedule below sends 10/101 + 20/101 + 30/101 + 40/101,
            # leaving 1/101 for change and fees. Do not tie this split
            # denominator to this fork's shorter COINBASE_MATURITY.
            to_send = (old_balances[from_node] / Decimal(101)).quantize(Decimal("0.00000001"))
            sends = {}
            addresses = {}
            ...
            assert txid in self.nodes[from_node].getrawmempool()

            # Mine from the node that created the transaction so this wallet
            # test proves block acceptance without depending on P2P relay
            # timing. The coinbase still pays node5 to keep accounting simple.
            self.generatetoaddress(self.nodes[from_node], 1, self.nodes[5].getnewaddress(), sync_fun=self.sync_blocks)
        ...
        if self.options.descriptors:
            self.log.info("Descriptor wallets reject receiving bech32m addresses until Taproot wallet support is enabled")
