...
class ToolWalletTest(BitcoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        ...
        if not os.path.isfile(self.options.bitcoinwallet):
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `blackcoin-wallet help` raises an error. Use `blackcoin-wallet -help`.
        ...
        self.assert_raises_tool_error('No method provided. Run `blackcoin-wallet -help` for valid methods.')
    ...
    def test_getwalletinfo_on_different_wallet(self):
        ...
        # This wallet-tool test is about readonly wallet metadata. Quantum
        # Quasar wallets can autostart the staking thread on load, which may
        # reserve a descriptor key before getwalletinfo is queried.
        self.start_node(0, ['-nowallet', '-wallet=foo', '-autostartstaking=0'])
    ...
    def test_dump_createfromdump(self):
        ...
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of blackcoin-wallet only supports version 1 dumpfiles. Got dumpfile with version 0', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
        bad_ver_wallet_dump = self.nodes[0].datadir_path / "wallet-bad_ver2.dump"
        dump_data["BITCOIN_CORE_WALLET_DUMP"] = "2"
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of blackcoin-wallet only supports version 1 dumpfiles. Got dumpfile with version 2', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
