...
class BlockchainTest(BitcoinTestFramework):
    ...
    def _test_getdifficulty(self):
        ...
        assert set(difficulty.keys()) == {"proof-of-work", "proof-of-stake"}
        # 1 hash in 2 should be valid, so difficulty should be 1/2**31
        # binary => decimal => binary math is why we do this check
        assert abs(difficulty["proof-of-work"] * 2**31 - 1) < 0.0001
        assert difficulty["proof-of-stake"] >= 0
    ...
    def _test_stopatheight(self):
        ...
        try:
            ...
            if exc.error["code"] != -342:
                raise
            pass  # The node already shut down before the follow-up mocktime sync RPC
    ...
    def _test_waitforblockheight(self):
        ...
        #
        # Blackcoin's automatic sync-checkpoint and regtest reorg-depth policy reject
        # forks far behind the tip. Keep this synthetic stale branch shallow while
        # still below the current best height.
        fork_height = current_height - 5
    ...
    def _test_getblock(self):
        node = self.nodes[0]
        fee_per_byte = Decimal('0.00000100')
