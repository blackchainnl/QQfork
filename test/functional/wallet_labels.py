...
class WalletLabelsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # the same address, so we call twice to get two matured address groups.
        ...
        mined_balance = node.getbalance()
        generated_amount = mined_balance / 2
        initial_receive_labels = set(node.listlabels(purpose="receive"))

        # there should be 2 address groups
        # each with 1 address with one matured generated output
        ...
        for address_group in address_groups:
            ...
            assert_equal(address_group[0][1], generated_amount)
            assert_equal(address_group[0][2], 'coinbase')
            linked_addresses.add(address_group[0][0])

        # send both generated outputs to a third address not in this wallet
        common_address = "msf4WtN1YQKXvNtvdFYt9JBnUD2FB41kjr"
        node.sendmany(
            amounts={common_address: mined_balance},
        ...
        expected_receive_labels = sorted(initial_receive_labels | {label.name for label in labels})
        assert_equal(node.listlabels(purpose="receive"), expected_receive_labels)

        # Check all labels are returned by listlabels.
        assert_equal(node.listlabels(), sorted(expected_receive_labels + ["node2_addr"]))

        # Check that deladdressbook removes a sending address book entry.
        delete_addr = self.nodes[1].getnewaddress()
        node.setlabel(delete_addr, "deleteme")
        assert_equal(node.getaddressesbylabel("deleteme"), {delete_addr: {"purpose": "send"}})
        node.deladdressbook(delete_addr)
        assert_raises_rpc_error(-11, "No addresses with label", node.getaddressesbylabel, "deleteme")

        # Check that deladdressbook rejects wallet-owned, unknown, and invalid addresses.
        own_addr = node.getnewaddress("keepme")
        assert_raises_rpc_error(-8, "Address book entries for wallet-owned addresses cannot be removed", node.deladdressbook, own_addr)
        assert_equal(node.getaddressesbylabel("keepme"), {own_addr: {"purpose": "receive"}})
        assert_raises_rpc_error(-8, "Address not found in the address book", node.deladdressbook, self.nodes[1].getnewaddress())
        assert_raises_rpc_error(-5, "Invalid Blackcoin address", node.deladdressbook, "invalid_address")
        ...
        if self.options.descriptors:
            ...
            BECH32_VALID = {
                'VER15_PROG40': 'blrt10qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq4k7ccc',
                'VER16_PROG03': 'blrt1sqqqqqvs92lg',
                'VER16_PROB02': 'blrt1sqqqq20sz8n',
            }
            BECH32_INVALID = {
                'VER15_PROG41': 'blrt1sqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqajlxj8',
                'VER16_PROB01': 'blrt1sqq5r4036',
            }
