...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave blackcoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop blackcoinds after the test execution")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "blackcoind": ("bitcoind", "BITCOIND"),
            "blackcoin-cli": ("bitcoincli", "BITCOINCLI"),
            "blackcoin-util": ("bitcoinutil", "BITCOINUTIL"),
            "blackcoin-wallet": ("bitcoinwallet", "BITCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: blackcoinds were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'blackcoind', self.options.bitcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'blackcoin-cli', self.options.bitcoincli) for v in versions]
        ...
        for i in range(num_nodes):
            ...
            if versions[i] is None and not any(arg.startswith("-autostartstaking") for arg in args):
                args.append("-autostartstaking=0")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a blackcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple blackcoind test nodes"""
        for node in self.nodes:
    ...
    def _sync_mocktime_to_generated_tip(self, generator, blocks, *, all_nodes=False):
        if not blocks:
            return
        node = generator.__dict__.get("_test_node", generator)
        if node.chain != "regtest":
            return
        block_hash = blocks[-1] if isinstance(blocks, list) else blocks.get("hash") if isinstance(blocks, dict) else None
        if block_hash is None:
            return
        try:
            tip_time = node.getblockheader(block_hash)["time"]
        except JSONRPCException as exc:
            if exc.error["code"] == -5:
                return
            raise
        candidates = self.nodes if all_nodes else [node]
        for candidate in candidates:
            try:
                if not candidate.rpc_connected:
                    continue
                if candidate.chain != "regtest":
                    continue
                current_time = candidate.mocktime if candidate.mocktime is not None else int(time.time())
                if current_time < tip_time:
                    candidate.setmocktime(tip_time)
            except (AssertionError, JSONRPCException, OSError):
                pass

    def generate(self, generator, *args, sync_fun=None, **kwargs):
        blocks = generator.generate(*args, invalid_call=False, **kwargs)
        self._sync_mocktime_to_generated_tip(generator, blocks)
        sync_fun() if sync_fun else self.sync_all()
        self._sync_mocktime_to_generated_tip(generator, blocks, all_nodes=True)
        return blocks
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as blackcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in blackcoin.conf
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in blackcoin.conf
    ...
    def skip_if_no_bitcoind_tracepoints(self):
        """Skip the running test if blackcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("blackcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if blackcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("blackcoind has not been built with zmq enabled.")
