...
# Copyright (c) 2010-2022 Blackcoin Core Developers
# Copyright (c) 2010-2022 Blackcoin More Developers
# Copyright (c) 2010-2022 Blackcoin Developers
...
V4_MAX_BLOCK_WEIGHT = 32000000
MAX_BLOOM_FILTER_SIZE = 36000
MAX_BLOOM_HASH_FUNCS = 50

COIN = 100000000  # 1 BLK in satoshis
MAX_MONEY = 20_000_000_000 * COIN
...
def _scrypt_blockmix_salsa8(block, r):
    chunks = [block[i * 64:(i + 1) * 64] for i in range(2 * r)]
    x = chunks[-1]
    y = []
    for chunk in chunks:
        x = _salsa20_8_core(_xor_bytes(x, chunk))
        y.append(x)
...
def _scrypt_romix(block, n, r):
    ...
    for _ in range(n):
        v.append(x)
        x = _scrypt_blockmix_salsa8(x, r)
    for _ in range(n):
        j = int.from_bytes(x[(2 * r - 1) * 64:(2 * r - 1) * 64 + 8], "little") & (n - 1)
        x = _scrypt_blockmix_salsa8(_xor_bytes(x, v[j]), r)
    return x


def _scrypt_1024_1_1_256_fallback(s):
    block = hashlib.pbkdf2_hmac("sha256", s, s, 1, 128)
    mixed = _scrypt_romix(block, 1024, 1)
    return hashlib.pbkdf2_hmac("sha256", s, mixed, 1, 32)


def scrypt_1024_1_1_256(s):
    if hasattr(hashlib, "scrypt"):
        return hashlib.scrypt(s, salt=s, n=1024, r=1, p=1, dklen=32)
    return _scrypt_1024_1_1_256_fallback(s)
...
class CBlockHeader:
    ...
    def deserialize_pos_marker(self, f):
        self.deserialize(f)
    ...
    def serialize(self):
        return self.serialize_for_hash()

    def serialize_pos_marker(self):
        return self.serialize_for_hash() + struct.pack("<I", self.nFlags)
    ...
    def calc_pow_hash(self):
        return uint256_from_str(scrypt_1024_1_1_256(self.serialize_for_hash()))

    def calc_sha256(self):
        if self.sha256 is None:
            r = self.serialize_for_hash()
            digest = hash256(r) if self.nVersion > 6 else scrypt_1024_1_1_256(r)
            self.sha256 = uint256_from_str(digest)
            self.hash = digest[::-1].hex()
...
class CBlock(CBlockHeader):
    ...
    def deserialize(self, f):
        ...
        def try_deserialize(with_pos_marker):
            stream = BytesIO(data)
            try:
                block = CBlock()
                block.nVersion = struct.unpack("<i", stream.read(4))[0]
                block.hashPrevBlock = deser_uint256(stream)
                block.hashMerkleRoot = deser_uint256(stream)
                block.nTime = struct.unpack("<I", stream.read(4))[0]
                block.nBits = struct.unpack("<I", stream.read(4))[0]
                block.nNonce = struct.unpack("<I", stream.read(4))[0]
                block.nFlags = struct.unpack("<I", stream.read(4))[0] if with_pos_marker else 0
                block.sha256 = None
                block.hash = None
                block.vtx = deser_vector(stream, CTransaction)
                block.vchBlockSig = deser_string(stream)
        ...
        candidates = [b for b in (try_deserialize(False), try_deserialize(True)) if b is not None]
        if not candidates:
            raise ValueError("Unable to deserialize block")

        def score(block):
            merkle_ok = bool(block.vtx) and block.calc_merkle_root() == block.hashMerkleRoot
            return (merkle_ok, bool(block.vtx), block.nFlags != 0)

        best = max(candidates, key=score)
        self.nVersion = best.nVersion
        self.hashPrevBlock = best.hashPrevBlock
        self.hashMerkleRoot = best.hashMerkleRoot
        self.nTime = best.nTime
        self.nBits = best.nBits
        self.nNonce = best.nNonce
        self.nFlags = best.nFlags
        self.sha256 = best.sha256
        self.hash = best.hash
        self.vtx = best.vtx
        self.vchBlockSig = best.vchBlockSig
        f.seek(start_pos + len(data))

    def serialize(self, with_witness=True, with_pos_marker=False):
        r = b""
        r += super().serialize_pos_marker() if with_pos_marker else super().serialize()
        if with_witness:
    ...
    # serialization size (does NOT use sigops).
    def get_weight(self):
        with_witness_size = len(self.serialize(with_witness=True, with_pos_marker=False))
        without_witness_size = len(self.serialize(with_witness=False, with_pos_marker=False))
        return (WITNESS_SCALE_FACTOR - 1) * without_witness_size + with_witness_size
...
class P2PHeaderAndShortIDs:
    ...
    def deserialize(self, f):
        self.header.deserialize_pos_marker(f)
    ...
    def serialize(self, with_witness=False):
        r = b""
        r += self.header.serialize_pos_marker()
...
class HeaderAndShortIDs:
    ...
    def get_siphash_keys(self):
        # CBlockHeaderAndShortTxIDs::FillShortTxIDSelector serializes the
        # header with plain SER_NETWORK. The fork's nFlags/PoS marker is part
        # of current P2P block-header messages, but it is not part of BIP152's
        # compact-block short-ID SipHash key.
...
class msg_block:
    ...
    def serialize(self):
        return self.block.serialize(with_pos_marker=True)
...
class msg_no_witness_block(msg_block):
    __slots__ = ()
    def serialize(self):
        return self.block.serialize(with_witness=False, with_pos_marker=True)
...
class msg_headers:
    ...
    def deserialize(self, f):
        # Headers messages serialize each block header followed by a zero
        # transaction count. They do not include Blackcoin's block-signature
        # vector used by full block messages.
        nit = deser_compact_size(f)
        for _ in range(nit):
            header = CBlockHeader()
            header.deserialize_pos_marker(f)
            assert_equal(deser_compact_size(f), 0)
            assert_equal(deser_compact_size(f), 0)
