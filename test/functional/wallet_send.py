...
class WalletSendTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        self.num_nodes = 2
        # Whitelist peers and set a test-local default fee above the fork's
        # randomized feefilter floor so wallet relay assertions are not flaky.
        # Explicit fee-rate cases below still exercise the 100 sat/vB minimum.
        self.extra_args = [
            ["-whitelist=forcerelay,noban@127.0.0.1", "-paytxfee=0.0012"],
            ["-whitelist=forcerelay,noban@127.0.0.1", "-paytxfee=0.0012"],
        ]
    ...
    def run_test(self):
        ...
        # This fork's send RPC accepts fee controls through the options object,
        # but does not expose Bitcoin Core's inherited top-level conf_target,
        # estimate_mode, or fee_rate named arguments.
        supports_top_level_send_fee_args = False
        supports_send_conf_target = False
        supports_send_minconf = False
        # w1 is a regular wallet
        self.nodes[1].createwallet(wallet_name="w1")
        w1 = self.nodes[1].get_wallet_rpc("w1")
        # w2 contains the private keys for w3
        self.nodes[1].createwallet(wallet_name="w2", blank=not self.options.descriptors)
        ...
        if self.options.descriptors:
            # Match the private descriptors in w2 using this fork's exported
            # public descriptors. Hard-coded Bitcoin tpub/tprv fixtures are
            # invalid under Blackcoin's extended-key prefixes.
            imports = []
            for desc_info in w2.listdescriptors()["descriptors"]:
                if not desc_info.get("active", False):
                    continue
                desc = desc_info["desc"]
                if not desc.startswith(("pkh(", "sh(wpkh(", "wpkh(")):
                    continue
                import_request = {
                    "desc": desc,
                    "timestamp": "now",
                ...
                if "range" in desc_info:
                    import_request["range"] = desc_info["range"]
                imports.append(import_request)
            res = w3.importdescriptors(imports)
            assert_equal(res, [{"success": True}] * len(imports))

        for _ in range(3):
            a2_receive = w2.getnewaddress()
            a2_change = w2.getrawchangeaddress() # legacy doesn't actually use change derivation
            if self.options.descriptors:
        ...
        if supports_top_level_send_fee_args:
        ...
        res = self.test_send(from_wallet=w3, data="23", change_address=w1.getnewaddress())
        res = w2.walletprocesspsbt(res["psbt"])
        assert res["complete"]
        ...
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, fee_rate=107, add_to_wallet=False)
        fee = self.nodes[1].decodepsbt(res["psbt"])["fee"]
        assert_fee_amount(fee, count_bytes(res["hex"]), Decimal("0.00107"))

        # This fork does not expose estimate_mode in send options; explicit
        # fee_rate remains covered.
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, fee_rate=102, add_to_wallet=False)
        fee = self.nodes[1].decodepsbt(res["psbt"])["fee"]
        assert_fee_amount(fee, count_bytes(res["hex"]), Decimal("0.00102"))

        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, fee_rate=104.531, add_to_wallet=False)
        fee = self.nodes[1].decodepsbt(res["psbt"])["fee"]
        assert_fee_amount(fee, count_bytes(res["hex"]), Decimal("0.00104531"))

        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, fee_rate=103, add_to_wallet=False)
        fee = self.nodes[1].decodepsbt(res["psbt"])["fee"]
        assert_fee_amount(fee, count_bytes(res["hex"]), Decimal("0.00103"))
        ...
        # Test setting explicit fee rate just below this fork's 100 sat/vB floor.
        self.log.info("Explicit fee rate raises RPC error 'fee rate too low' below 100 sat/vB")
        ...
        amount_above_single_utxo = utxo1["amount"] + 1
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=amount_above_single_utxo, inputs=[], add_to_wallet=False)
        assert res["complete"]
        ERR_NOT_ENOUGH_PRESET_INPUTS = "The preselected coins total amount does not cover the transaction target. " \
                                       "Please allow other inputs to be automatically selected or include more coins manually"
        self.test_send(from_wallet=w0, to_wallet=w1, amount=amount_above_single_utxo, inputs=[utxo1],
                       expect_error=(-4, ERR_NOT_ENOUGH_PRESET_INPUTS))
        self.test_send(from_wallet=w0, to_wallet=w1, amount=amount_above_single_utxo, inputs=[utxo1], add_inputs=False,
                       expect_error=(-4, ERR_NOT_ENOUGH_PRESET_INPUTS))
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=amount_above_single_utxo, inputs=[utxo1], add_inputs=True, add_to_wallet=False)
        assert res["complete"]
        ...
        # Blackcoin inherits Blackcoin's transaction timestamp rule: a tx
        # cannot spend a coin whose recorded time is later than the tx time.
        # Keep this unsafe-spend coverage construction-only; broadcasting a
        # same-block parent/child package can make later generated blocks
        # peer-invalid once the parent coin time becomes the block time.
        res = self.test_send(from_wallet=w5, to_wallet=w0, amount=1, include_unsafe=True, add_to_wallet=False)
        assert res["complete"]
        ...
        if supports_send_minconf:
            ...
            # Blackcoin's send RPC rejects Bitcoin Core's inherited
            # minconf option; the default spend path remains covered below.
            self.test_send(from_wallet=minconfw, to_wallet=w0, amount=1, minconf=4, expect_error=(-3, "Unexpected key minconf"))
            self.test_send(from_wallet=minconfw, to_wallet=w0, amount=1, minconf=-4, expect_error=(-3, "Unexpected key minconf"))
            res = self.test_send(from_wallet=minconfw, to_wallet=w0, amount=1)
        assert res["complete"]
        ...
        # Blackcoin/Blackcoin rejects transactions whose nTime predates
        # any spent input's coin time. Advance both nodes past the block that
        # confirms the external input before constructing the spending PSBT.
        self.sync_mocktime_to_tip()
