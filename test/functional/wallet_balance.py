...
REGTEST_BLOCK_REWARD = Decimal("10000")
...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if not self.options.descriptors:
            # Tests legacy watchonly behavior which is not present (and does not need to be tested) in descriptor wallets
            assert_equal(self.nodes[0].getbalances()['mine']['trusted'], REGTEST_BLOCK_REWARD)
            assert_equal(self.nodes[0].getwalletinfo()['balance'], REGTEST_BLOCK_REWARD)
            assert_equal(self.nodes[1].getbalances()['mine']['trusted'], REGTEST_BLOCK_REWARD)

            assert_equal(self.nodes[0].getbalances()['watchonly']['immature'], WATCHONLY_IMMATURE_REWARD)
            assert 'watchonly' not in self.nodes[1].getbalances()

            assert_equal(self.nodes[0].getbalance(), REGTEST_BLOCK_REWARD)
            assert_equal(self.nodes[1].getbalance(), REGTEST_BLOCK_REWARD)

        self.log.info("Test getbalance with different arguments")
        assert_equal(self.nodes[0].getbalance("*"), REGTEST_BLOCK_REWARD)
        assert_equal(self.nodes[0].getbalance("*", 1), REGTEST_BLOCK_REWARD)
        assert_equal(self.nodes[0].getbalance(minconf=1), REGTEST_BLOCK_REWARD)
        if not self.options.descriptors:
            assert_equal(self.nodes[0].getbalance(minconf=0, include_watchonly=True), 2 * REGTEST_BLOCK_REWARD)
            assert_equal(self.nodes[0].getbalance("*", 1, True), 2 * REGTEST_BLOCK_REWARD)
        else:
            assert_equal(self.nodes[0].getbalance(minconf=0, include_watchonly=True), REGTEST_BLOCK_REWARD)
            assert_equal(self.nodes[0].getbalance("*", 1, True), REGTEST_BLOCK_REWARD)
        assert_equal(self.nodes[1].getbalance(minconf=0, include_watchonly=True), REGTEST_BLOCK_REWARD)

        # Send 40 BLK from 0 to 1, then send enough from 1 to 0 that node 1
        # must spend its own mature reward plus node 0's unconfirmed payment.
        node0_send_amount = Decimal('40')
        node1_send_amount = REGTEST_BLOCK_REWARD + Decimal('10')
        node0_send_fee = Decimal('0.01')
        node1_change = REGTEST_BLOCK_REWARD + node0_send_amount - node1_send_amount
        txs = create_transactions(self.nodes[0], self.nodes[1].getnewaddress(), node0_send_amount, [node0_send_fee])
        self.nodes[0].sendrawtransaction(txs[0]['hex'])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])  # sending on both nodes is faster than waiting for propagation

        self.sync_all()
        parent_time = tx_from_hex(txs[0]['hex']).nTime
        for node in self.nodes:
            node.setmocktime(parent_time + 1)
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), node1_send_amount, [Decimal('0.01'), Decimal('0.02')])
        ...
        # Before `test_balance()`, we have had two nodes with one spendable
        # regtest reward each and then we:
        #
        # 1) Sent 40 from node A to node B with fee 0.01
        # 2) Sent one reward plus 10 from node B to node A with fee 0.01
        #
        ...
        #
        # Then our node would report a confirmed balance of 40 + one reward -
        # 10, which is more than would be available if transaction 1 were
        # replaced.


        def test_balances(*, fee_node_1=0):
            node0_change = REGTEST_BLOCK_REWARD - node0_send_amount - node0_send_fee
            # getbalances
            expected_balances_0 = {'mine':      {'immature':          Decimal('0E-8'),
                                                 'trusted':           node0_change,  # change from node 0's send
                                                 'untrusted_pending': node1_send_amount,
                                                 'stake':             Decimal('0E-8')},
                                   'watchonly': {'immature':          WATCHONLY_IMMATURE_REWARD,
                                                 'trusted':           REGTEST_BLOCK_REWARD,
                                                 'untrusted_pending': Decimal('0E-8'),
                                                 'stake':             Decimal('0E-8')}}
            expected_balances_1 = {'mine':      {'immature':          Decimal('0E-8'),
                                                 'trusted':           Decimal('0E-8'),  # node 1's send had an unsafe input
                                                 'untrusted_pending': node1_change - fee_node_1,
                                                 'stake':             Decimal('0E-8')}}  # Doesn't include output of node 0's send since it was spent
            ...
            assert_equal(self.nodes[0].getunconfirmedbalance(), node1_send_amount)  # output of node 1's spend
            assert_equal(self.nodes[1].getunconfirmedbalance(), node1_change - fee_node_1)  # Doesn't include output of node 0's send since it was spent
            # getwalletinfo.unconfirmed_balance
            assert_equal(self.nodes[0].getwalletinfo()["unconfirmed_balance"], node1_send_amount)
            assert_equal(self.nodes[1].getwalletinfo()["unconfirmed_balance"], node1_change - fee_node_1)

        test_balances(fee_node_1=Decimal('0.01'))

        # Node 1 bumps the transaction fee and resends
        node1_active_fee = Decimal('0.01')
        try:
            self.nodes[1].sendrawtransaction(txs[1]['hex'])
            self.nodes[0].sendrawtransaction(txs[1]['hex'])  # sending on both nodes is faster than waiting for propagation
            self.sync_all()
            node1_active_fee = Decimal('0.02')

            self.log.info("Test getbalance and getbalances.mine.untrusted_pending with conflicted unconfirmed inputs")
            test_balances(fee_node_1=node1_active_fee)
        except JSONRPCException as e:
            if "txn-mempool-conflict" not in e.error["message"]:
                raise
            self.log.info("Skipping replacement-specific balance check because this fork rejects the inherited raw RBF conflict")

        self.generatetoaddress(self.nodes[1], 1, ADDRESS_WATCHONLY)

        # balances are correct after the transactions are confirmed
        balance_node0 = REGTEST_BLOCK_REWARD - node0_send_amount - node0_send_fee + node1_send_amount
        balance_node1 = node1_change - node1_active_fee  # change from node 1's active send
        ...
        node1_sweep_fee = Decimal('0.01')
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), balance_node1 - node1_sweep_fee, [node1_sweep_fee])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])
        ...
        self.nodes[1].sendrawtransaction(self.nodes[0].gettransaction(txid)['hex'])
        self.nodes[1].loadwallet(self.default_wallet_name)
        after = self.nodes[1].getbalances()['mine']['untrusted_pending']
        assert_equal(before + Decimal('0.1'), after)

        # Create wallet txs until one is not accepted to the mempool because
        # the descendant limit is reached. The inherited Bitcoin fixture hit
        # this on the third descendant; this fork's policy limit allows at
        # least that many, so keep the test tied to the behavior instead of a
        # Bitcoin-specific count.
        ...
        for _ in range(10):
            ...
            descendant_amount = self.nodes[0].getbalance(minconf=0)
            txid = self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), descendant_amount, "", "", True)
        ...
        # Increase fee by 1 coin by reducing the largest output.
        fee_bump = Decimal('1')
        replace_amount = max(o['value'] for o in self.nodes[0].decoderawtransaction(tx_orig)['vout'])
        assert replace_amount > fee_bump
        tx_replace = tx_orig.replace(
            struct.pack("<q", int(replace_amount * COIN)).hex(),
            struct.pack("<q", int((replace_amount - fee_bump) * COIN)).hex(),
            1,
        )
        ...
        self.sync_blocks()
        for mempool_txid, entry in sorted(self.nodes[0].getrawmempool(True).items(), key=lambda item: item[1].get('ancestorcount', 0)):
            try:
                self.nodes[1].sendrawtransaction(self.nodes[0].getrawtransaction(mempool_txid))
            except JSONRPCException as e:
                if "txn-already-in-mempool" not in e.error["message"]:
                    raise
        self.nodes[1].sendrawtransaction(hexstring=tx_replace, maxfeerate=0)

        # Now confirm tx_replace
        block_reorg = self.generatetoaddress(self.nodes[1], 1, ADDRESS_WATCHONLY)[0]
        balance_with_replace = self.nodes[0].getbalance(minconf=0)
        assert balance_with_replace >= total_amount
        ...
        self.generatetoaddress(self.nodes[1], 1, ADDRESS_WATCHONLY)
        assert_equal(self.nodes[0].getbalance(minconf=0), balance_with_replace + fee_bump)  # The reorg recovered our fee bump
        ...
        if not self.options.descriptors:
            ...
            # After importprivkey the address is spendable and no watch-only
            # funds remain; the watchonly section may be omitted entirely.
            watchonly = self.nodes[0].getbalances().get('watchonly', {'untrusted_pending': 0})
            assert_equal(watchonly['untrusted_pending'], 0)
