...
# Copyright (c) 2023-present Blackcoin Core Developers
# Copyright (c) 2023-present Blackcoin More Developers
# Copyright (c) 2023-present Blackcoin Developers
...
class WalletReindexTest(BitcoinTestFramework):
    ...
    def birthtime_test(self, node, miner_wallet):
        ...
        if self.options.descriptors:
            # QQ's descriptor import birthtime tracks the current chain tip time
            # after the generated-block mocktime sync.
            tip_time = node.getblock(node.getbestblockhash())["time"]
            assert_equal(tip_time, wallet_birthtime)
        else:
