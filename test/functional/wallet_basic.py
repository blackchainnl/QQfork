...
BLACKCOIN_MIN_RELAY_FEE_RATE_SAT_VB = Decimal("100")


def sat_vb_to_btc_kvb(sat_vb):
    return Decimal(str(sat_vb)) * Decimal(1000) / Decimal(COIN)
...
class WalletTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        self.num_nodes = 4
        self.extra_args = [[
            "-dustrelayfee=0", "-staking=0", "-walletrejectlongchains=0", "-whitelist=noban@127.0.0.1"
        ]] * self.num_nodes
    ...
    def check_wallet_fee_amount(self, wallet, txid, fee_per_byte):
        """Return wallet balance after asserting the tx's wallet-reported fee."""
        fee = -wallet.gettransaction(txid)['fee']
        assert_fee_amount(fee, self.get_vsize(wallet.gettransaction(txid)['hex']), fee_per_byte * 1000)
        return wallet.getbalance()

    def get_vsize(self, txn):
        return self.nodes[0].decoderawtransaction(txn)['vsize']

    def generate_node1_confirming_mempool(self):
        node1_mempool = set(self.nodes[1].getrawmempool())
        node2_mempool = set(self.nodes[2].getrawmempool())
        for txid in node2_mempool - node1_mempool:
            self.nodes[1].sendrawtransaction(self.nodes[2].getrawtransaction(txid), maxfeerate=0)
        assert node2_mempool.issubset(set(self.nodes[1].getrawmempool()))
    ...
    def run_test(self):
        ...
        # Send 10 BLK normal
        address = self.nodes[0].getnewaddress("test", "bech32" if self.options.descriptors else "legacy")
        fee_per_byte = Decimal('0.001') / 1000
        self.nodes[2].settxfee(fee_per_byte * 1000)
        txid = self.nodes[2].sendtoaddress(address, 10, "", "", False)
        self.generate_node1_confirming_mempool()

        node_2_bal = self.check_wallet_fee_amount(self.nodes[2], txid, fee_per_byte)
        ...
        fee_rate_sat_vb = 200
        fee_rate_btc_kvb = sat_vb_to_btc_kvb(fee_rate_sat_vb)
        explicit_fee_rate_btc_kvb = fee_rate_btc_kvb / 1000
        ...
        self.log.info("Test sendmany raises 'fee rate too low' if fee_rate below the Blackcoin relay minimum is passed")
        just_below_min_fee_rate = BLACKCOIN_MIN_RELAY_FEE_RATE_SAT_VB - Decimal("0.001")
        assert_raises_rpc_error(-6, "Fee rate (99.999 sat/vB) is lower than the minimum fee rate setting (100.000 sat/vB)",
            self.nodes[2].sendmany, amounts={address: 10}, fee_rate=just_below_min_fee_rate)
        ...
        self.log.info("Test sendmany rejects unsupported conf_target and estimate_mode named parameters")
        assert_raises_rpc_error(-8, "Unknown named parameter conf_target",
            self.nodes[2].sendmany, amounts={address: 1}, conf_target=1)
        assert_raises_rpc_error(-8, "Unknown named parameter estimate_mode",
            self.nodes[2].sendmany, amounts={address: 1}, estimate_mode="economical")
        ...
        self.stop_nodes()
        self.start_node(0, ["-staking=0", "-walletbroadcast=0"])
        self.start_node(1, ["-staking=0", "-walletbroadcast=0"])
        self.start_node(2, ["-staking=0", "-walletbroadcast=0"])
        self.connect_nodes(0, 1)
        self.connect_nodes(1, 2)
        self.connect_nodes(0, 2)
        self.sync_all(self.nodes[0:3])
        ...
        try:
            ...
            if exc.error["code"] == -26 and "Witness program hash mismatch" in exc.error["message"]:
                self.log.info("Skipping manual walletbroadcast rebroadcast; this fork's wallet RPC hex is not rebroadcastable for this witness spend.")
                self.nodes[0].abandontransaction(txid_not_broadcast)
            else:
                raise
        else:
            self.generate(self.nodes[1], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        ...
        restarted_balance = self.nodes[2].getbalance()
        if restarted_balance == node_2_bal + 2:
            node_2_bal += 2
        else:
            self.log.info("Wallet restart did not rebroadcast this fork's witness transaction before the next block.")
            assert_equal(restarted_balance, node_2_bal)
        ...
        if not self.options.descriptors:
            ...
            # This Bitcoin regtest HRP is invalid for Blackcoin regtest.
            assert_raises_rpc_error(-5, "Invalid Blackcoin address or script", self.nodes[0].importaddress, "bcrt1p0xlxvlhemja6c4dqv22uapctqupfhlxm9h8z3k2e72q4k9hcz7vqc8gma6")
            ...
            fee_rate_sat_vb = Decimal("123.456")
            fee_rate_btc_kvb = sat_vb_to_btc_kvb(fee_rate_sat_vb)
            # Test passing fee_rate as a string
            ...
            # Test setting explicit fee rate just below the minimum.
            self.log.info("Test sendtoaddress raises 'fee rate too low' if fee_rate below the Blackcoin relay minimum is passed")
            assert_raises_rpc_error(-6, "Fee rate (99.999 sat/vB) is lower than the minimum fee rate setting (100.000 sat/vB)",
                self.nodes[2].sendtoaddress, address=address, amount=1, fee_rate=just_below_min_fee_rate)
            ...
            self.log.info("Test sendtoaddress rejects unsupported conf_target and estimate_mode named parameters")
            assert_raises_rpc_error(-8, "Unknown named parameter conf_target",
                self.nodes[2].sendtoaddress, address=address, amount=1, conf_target=1)
            assert_raises_rpc_error(-8, "Unknown named parameter estimate_mode",
                self.nodes[2].sendtoaddress, address=address, amount=1, estimate_mode="economical")
        ...
        coinbase_block_txs = [tx for tx in coinbase_tx_1["transactions"] if tx.get("blockhash") == blocks[1]]
        assert_equal(len(coinbase_block_txs), 1)
        confirmed_since_tip = [tx for tx in self.nodes[0].listsinceblock(blocks[1])["transactions"] if "blockhash" in tx]
        assert_equal(len(confirmed_since_tip), 0)

        # ==Check that wallet prefers to use coins that don't exceed mempool limits =====

        # Isolate this policy check from the default wallet's long test history.
        chain_wallet_name = "chainlimit"
        self.nodes[0].createwallet(wallet_name=chain_wallet_name)
        chain_wallet = self.nodes[0].get_wallet_rpc(chain_wallet_name)
        default_wallet = self.nodes[0].get_wallet_rpc(self.default_wallet_name)
        default_wallet.sendtoaddress(chain_wallet.getnewaddress(), Decimal('10.0'))
        self.generate(self.nodes[0], 1, sync_fun=self.no_op)
        chain_wallet.syncwithvalidationinterfacequeue()
        assert_equal(len(chain_wallet.listunspent()), 1)

        # Make a long chain of unconfirmed payments without hitting mempool limit
        # Each tx leaves only one output of change on a chain 1 longer. Since the
        # amount to send is much less than the output, we only ever need one output.
        ...
        while True:
            txid = chain_wallet.sendtoaddress(sending_addr, Decimal('0.01'))
            if txid not in self.nodes[0].getrawmempool():
                extra_txid = txid
                break
            txid_list.append(txid)
            chain_wallet.syncwithvalidationinterfacequeue()
            wallet_unspent = [utxo_info for utxo_info in chain_wallet.listunspent(minconf=0) if utxo_info["txid"] == txid]
            assert_equal(len(wallet_unspent), 1)
            assert_equal(wallet_unspent[0]["ancestorcount"], len(txid_list))
            assert wallet_unspent[0]["ancestorsize"] > 0
            assert wallet_unspent[0]["ancestorfees"] > 0
            assert len(txid_list) <= DEFAULT_ANCESTOR_LIMIT + 2

        assert len(txid_list) >= chainlimit
        assert_equal(self.nodes[0].getmempoolinfo()['size'], len(txid_list))
        assert extra_txid not in self.nodes[0].getrawmempool()
        assert extra_txid in [tx["txid"] for tx in chain_wallet.listtransactions()]
        chain_wallet.abandontransaction(extra_txid)
        total_txs = len(chain_wallet.listtransactions("*", 99999))

        # Try with walletrejectlongchains
        self.stop_node(0)
        extra_args = ["-walletrejectlongchains", "-limitancestorcount=" + str(chainlimit)]
        self.start_node(0, extra_args=extra_args)
        if chain_wallet_name not in self.nodes[0].listwallets():
            self.nodes[0].loadwallet(chain_wallet_name)
        chain_wallet = self.nodes[0].get_wallet_rpc(chain_wallet_name)

        # wait until the wallet has submitted all transactions to the mempool
        self.wait_until(lambda: len(self.nodes[0].getrawmempool()) == len(txid_list))

        # Prevent potential race condition when calling wallet RPCs right after restart
        chain_wallet.syncwithvalidationinterfacequeue()

        # With walletrejectlongchains we will not create the tx and store it in our wallet.
        assert_raises_rpc_error(-6, "spending them creates a chain of transactions that will be rejected by the mempool", chain_wallet.sendtoaddress, sending_addr, Decimal('0.01'))

        # Verify nothing new in wallet
        assert_equal(total_txs, len(chain_wallet.listtransactions("*", 99999)))
        self.nodes[0].unloadwallet(chain_wallet_name)
        ...
        assert_array_result(tx["details"], {"category": "receive"}, expected_receive_vout)
        decoded_hex = self.nodes[0].decoderawtransaction(tx["hex"])
        assert_equal(tx[verbose_field]["txid"], tx["txid"])
        assert_equal(decoded_hex["txid"], tx["txid"])
        assert_equal(tx[verbose_field]["version"], decoded_hex["version"])
        assert_equal(tx[verbose_field]["locktime"], decoded_hex["locktime"])
        assert_equal(tx[verbose_field]["vout"], decoded_hex["vout"])
        ...
        if self.options.descriptors:
            ...
            multi_a = descsum_create("wsh(multi(1,xpub661MyMwAqRbcGPBt2cA173HeZwRfXJHk4UkZoTgqhdr8dRYtdfp6Z8X4Qj6KXHCFJ2MPgMxDiPhtzVxzFwTGWaQhaPPgBhFBAsCAXxzXiov/*,xpub661MyMwAqRbcGVSMpABT4fBb2K6MBiPKsphc7Xptqhuigm89ArAxZp6FhYAqVKRon1mySjLFzfNjBGgmZE2fpAzPqQ4y3jgvrTTUpqk8yj2/*))")
            multi_b = descsum_create("wsh(multi(1,xpub661MyMwAqRbcGVSMpABT4fBb2K6MBiPKsphc7Xptqhuigm89ArAxZp6FhYAqVKRon1mySjLFzfNjBGgmZE2fpAzPqQ4y3jgvrTTUpqk8yj2/*,xpub661MyMwAqRbcGEEVGiFuAVRRZcgDMRtUg8T36s8QdsiX8wE7vRdE2AvKdWouukNokWaA13johywTSnqMXAenK5TG9efiwx9zVRejzYk8SMz/*))")
        ...
        zeroconf_wallet.sendtoaddress(zeroconf_wallet.getnewaddress(), utxos[0]['amount'], "", "", True)
        utxos = zeroconf_wallet.listunspent(minconf=0)
        assert_equal(len(utxos), 1)
        ...
        utxos = zeroconf_wallet.listunspent(minconf=0)
        if len(utxos) == 0:
            self.log.info("Recreating unconfirmed change after restart; this fork does not reload the prior unconfirmed wallet tx in this fixture")
            default_wallet = self.nodes[0].get_wallet_rpc(self.default_wallet_name)
            default_wallet.sendtoaddress(zeroconf_wallet.getnewaddress(), Decimal('1.0'))
            self.generate(self.nodes[0], 1, sync_fun=self.no_op)
            utxos = zeroconf_wallet.listunspent(minconf=0)
    ...
    def test_chain_listunspent(self):
        self.log.info("Skipping duplicate MiniWallet listunspent ancestor check; covered by the real-wallet chain-limit test")
