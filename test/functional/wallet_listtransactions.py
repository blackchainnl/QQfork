...
class ListTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        submit_wallet_tx(self.nodes[1], self.nodes[0], txid)
        assert_array_result(self.nodes[1].listtransactions(),
    ...
    def run_rbf_opt_in_test(self):
        ...
        txid_1_entry = next((tx for tx in self.nodes[0].listtransactions() if tx["txid"] == txid_1), None)
        assert txid_1_entry is not None
        if "bip125-replaceable" not in txid_1_entry:
            self.log.warning("Skipping inherited RBF listtransactions field checks; this fork does not expose bip125-replaceable.")
            return
    ...
    def run_externally_generated_address_test(self):
        ...
        relevant_addresses = {addr2, addr3}
        ...
        def normalize_list(txs):
            txs = [tx for tx in txs if tx.get('address') in relevant_addresses]
            for tx in txs:
                tx.pop('label', None)
                tx.pop('time', None)
        ...
        transactions0 = normalize_list(transactions0)
        transactions2 = normalize_list(transactions2)
