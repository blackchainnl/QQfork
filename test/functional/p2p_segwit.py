...
def decimal_blks(value):
    return Decimal(value) / COIN


def set_tx_fee(tx, input_value, fee=None, output_index=0):
    tx.rehash()
    target_fee = min_tx_fee(tx) if fee is None else fee
    current_fee = input_value - sum(txout.nValue for txout in tx.vout)
    delta = target_fee - current_fee
    tx.vout[output_index].nValue -= delta
    assert tx.vout[output_index].nValue > 0
    tx.rehash()
    return target_fee
...
class SegWitTest(BitcoinTestFramework):
    ...
    def test_block_relay(self):
        """Test block requests to NODE_WITNESS peers.

        This fork requests non-witness blocks before SegWit activation because
        witness data itself is invalid before activation, and witness blocks
        after activation.
        Also test that we don't ask for blocks from unupgraded peers."""

        blocktype = 2 | MSG_WITNESS_FLAG if self.segwit_active else 2
        ...
        if self.segwit_active:
            # After activation this fork does not direct-fetch these synthetic
            # empty block announcements, so connect them directly and keep the
            # witness-vs-stripped retrieval assertions below.
        ...
        if not self.segwit_active:
            ...
            # be different. This fork's raw getblock RPC returns stripped
            # serialization, while getdata respects the requested type.
            block = self.build_next_block()
            ...
            assert_equal(non_wit_block.serialize(), bytes.fromhex(rpc_block))
            ...
            # Since 0.14, inv's will only be responded to with a getheaders, so
            # send a header to announce this block. This fork does not request
            # the old Bitcoin sentinel tx shape from this peer, so use a ping
            # round-trip to let immediate block requests drain.
            msg = msg_headers()
    ...
    def test_v0_outputs_arent_spendable(self):
        """Test v0 witness-output handling before segwit activation.

        Bitcoin Core backdated SCRIPT_VERIFY_WITNESS to all blocks. This fork
        only enables witness script validation when SegWit is active, so
        pre-activation blocks reject witness-bearing serialization but still
        treat v0 witness programs as anyone-can-spend when serialized without
        witness data.
        """
        ...
        set_tx_fee(tx, self.utxo[0].nValue, output_index=2)
        funding_tx = tx
        txid = tx.sha256
        ...
        set_tx_fee(p2sh_p2wsh_tx, value)
        ...
        for tx in [p2wsh_tx, p2sh_p2wsh_tx]:
            ...
            # When the block is serialized without witness, this fork has not
            # yet activated SCRIPT_VERIFY_WITNESS, so the v0 witness program is
            # spendable as an anyone-can-spend output.
            test_witness_block(self.nodes[0], self.test_node, block, accepted=True, with_witness=False)

        self.utxo.pop(0)
        self.utxo.append(UTXO(txid, 2, funding_tx.vout[2].nValue))
    ...
    def test_witness_tx_relay_before_segwit_activation(self):
        ...
        # This fork does not request the body from the non-witness peer in
        # this pre-activation relay shape. Deliver directly below so the test
        # still proves witness-bearing tx rejection and stripped-tx acceptance.
    ...
    def test_standardness_v0(self):
        ...
        set_tx_fee(p2sh_tx, self.utxo[0].nValue)
        ...
        tx.vout.append(CTxOut(2 * MIN_TX_FEE, script_pubkey))  # Might burn this later
        tx.vin[0].nSequence = MAX_BIP125_RBF_SEQUENCE  # Just to have the option to bump this tx from the mempool
        set_tx_fee(tx, p2sh_tx.vout[0].nValue)

        # Bitcoin Core policy treats segwit outputs as standard before
        # activation. This fork only accepts them on the non-standard node until
        # activation.
        policy_node = self.nodes[1] if self.segwit_active else self.nodes[0]
        policy_p2p = self.std_node if self.segwit_active else self.test_node
        test_transaction_acceptance(policy_node, policy_p2p, tx, with_witness=True, accepted=True)
        ...
        tx2.vout = [CTxOut(tx.vout[1].nValue - MIN_TX_FEE, script_pubkey)]
        tx2.wit.vtxinwit.append(CTxInWitness())
        tx2.wit.vtxinwit[0].scriptWitness.stack = [witness_script]
        set_tx_fee(tx2, tx.vout[1].nValue)

        if self.segwit_active:
            test_transaction_acceptance(policy_node, policy_p2p, tx2, with_witness=True, accepted=True)
        else:
            test_transaction_acceptance(policy_node, policy_p2p, tx2, with_witness=True, accepted=False, reason="no-witness-yet")
        ...
        if not self.segwit_active:
            ...
            assert_equal(testres3[0]["allowed"], False)
            assert_equal(testres3[0]["reject-reason"], "no-witness-yet")

            tx3 = CTransaction()
            tx3.vin = [CTxIn(COutPoint(tx.sha256, 0), b"")]
            tx3.vout = [CTxOut(tx.vout[0].nValue - MIN_TX_FEE, CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE]))]
            set_tx_fee(tx3, tx.vout[0].nValue)
            testres3_no_witness = self.nodes[0].testmempoolaccept([tx3.serialize_without_witness().hex()])
            assert_equal(testres3_no_witness[0]["allowed"], False)
            assert_equal(testres3_no_witness[0]["reject-reason"], "non-mandatory-script-verify-flag (Witness program was passed an empty witness)")

            block = self.build_next_block()
            block.vtx.extend([tx, tx3])
    ...
    def test_p2sh_witness(self):
        ...
        set_tx_fee(spend_tx, tx.vout[0].nValue)
    ...
    def test_witness_commitments(self):
        ...
        tx3.vout.append(CTxOut(tx2.vout[0].nValue - 1000, witness_script))
        set_tx_fee(tx3, tx2.vout[0].nValue)
    ...
    def test_witness_block_size(self):
        ...
        set_tx_fee(parent_tx, value)
        child_input_value = sum(txout.nValue for txout in parent_tx.vout)

        child_tx = CTransaction()
        for i in range(NUM_OUTPUTS):
            child_tx.vin.append(CTxIn(COutPoint(parent_tx.sha256, i), b""))
        child_tx.vout = [CTxOut(child_input_value - 100000, CScript([OP_TRUE]))]
        for _ in range(NUM_OUTPUTS):
            child_tx.wit.vtxinwit.append(CTxInWitness())
            child_tx.wit.vtxinwit[-1].scriptWitness.stack = [b'a' * 195] * (2 * NUM_DROPS) + [witness_script]
        set_tx_fee(child_tx, child_input_value)
        ...
        set_tx_fee(block.vtx[-1], child_input_value)
    ...
    def test_submit_block(self):
        ...
        assert_raises_rpc_error(-100, 'bad-txnmrklroot', self.nodes[0].submitblock, block_2.serialize().hex())
    ...
    def test_extra_witness_data(self):
        ...
        test_witness_block(self.nodes[0], self.test_node, block, accepted=False,
                           reason='Witness provided for non-witness script')

        # Try extra signature data.  Ok if we're not spending a witness output.
        block.vtx[1].wit.vtxinwit = []
        block.vtx[1].vin[0].scriptSig = CScript([OP_0])
        set_tx_fee(block.vtx[1], self.utxo[0].nValue)
        add_witness_commitment(block)
        block.solve()
        ...
        tx2_input_value = tx.vout[0].nValue + tx.vout[1].nValue
        set_tx_fee(tx2, tx2_input_value)
        ...
        test_witness_block(self.nodes[0], self.test_node, block, accepted=False,
                           reason='Stack size must be exactly one after execution')
        ...
        test_witness_block(self.nodes[0], self.test_node, block, accepted=False,
                           reason='Witness requires empty scriptSig')
    ...
    def test_witness_input_length(self):
        ...
        set_tx_fee(tx, value)
        tx2_input_value = sum(txout.nValue for txout in tx.vout)
        ...
        tx2.vout.append(CTxOut(tx2_input_value - 3000, CScript([OP_TRUE])))
        ...
        self.update_witness_block_with_transactions(block, [tx2])
        test_witness_block(self.nodes[0], self.test_node, block, accepted=False,
                           reason='Operation not valid with the current stack size')
    ...
    def test_tx_relay_after_segwit_activation(self):
        ...
        # This fork does not request this oversized-witness announcement body
        # from the peer. Deliver it directly to verify standardness rejection.
    ...
    def test_segwit_versions(self):
        ...
        witness_hash = sha256(witness_script)
        assert_equal(len(self.nodes[1].getrawmempool()), 0)
        reserved_blackcoin_versions = {OP_14, OP_15, OP_16}
        for version in list(range(OP_1, OP_16 + 1)) + [OP_0]:
            # First try to spend to a future version segwit script_pubkey.
            if version == OP_1 or version in reserved_blackcoin_versions:
                # Don't use 32-byte v1 witness (used by Taproot; see BIP 341),
                # or a 32-byte Blackcoin-reserved witness program.
        ...
        self.nodes[1].setmocktime(self.nodes[1].getblockheader(self.nodes[1].getbestblockhash())["time"] + 1)
        testres3 = self.nodes[1].testmempoolaccept([tx3.serialize_with_witness().hex()])[0]
        assert_equal(testres3["allowed"], False)
        assert_equal(testres3["reject-reason"], "bad-txns-nonstandard-inputs")
    ...
    def test_premature_coinbase_witness_spend(self):
        ...
        set_tx_fee(spend_tx, block.vtx[0].vout[0].nValue)
    ...
    def test_uncompressed_pubkey(self):
        ...
        set_tx_fee(tx, utxo.nValue, fee=TX_FEE_PER_KB)
        ...
        set_tx_fee(tx2, tx.vout[0].nValue, fee=TX_FEE_PER_KB)
        ...
        set_tx_fee(tx3, tx2.vout[0].nValue, fee=TX_FEE_PER_KB)
        tx3.wit.vtxinwit.append(CTxInWitness())
        ...
        set_tx_fee(tx4, tx3.vout[0].nValue, fee=TX_FEE_PER_KB)
        ...
        set_tx_fee(tx5, tx4.vout[0].nValue, fee=TX_FEE_PER_KB)
    ...
    def test_signature_version_1(self):
        ...
        tx.vin.append(CTxIn(COutPoint(self.utxo[0].sha256, self.utxo[0].n), b""))
        tx.vout.append(CTxOut(self.utxo[0].nValue - 1000, script_pubkey))
        set_tx_fee(tx, self.utxo[0].nValue, fee=TX_FEE_PER_KB)
        ...
        for sigflag in [0, SIGHASH_ANYONECANPAY]:
            for hashtype in [SIGHASH_ALL, SIGHASH_NONE, SIGHASH_SINGLE]:
                ...
                set_tx_fee(tx, prev_utxo.nValue, fee=TX_FEE_PER_KB)
                tx.wit.vtxinwit.append(CTxInWitness())
                # Too-large input value
                sign_p2pk_witness_input(witness_script, tx, 0, hashtype, prev_utxo.nValue + 1, key)
                self.update_witness_block_with_transactions(block, [tx])
                test_witness_block(self.nodes[0], self.test_node, block, accepted=False,
                                   reason='Script evaluated without error '
        ...
        tx = CTransaction()
        tx.vin.append(CTxIn(COutPoint(prev_utxo.sha256, prev_utxo.n), b""))
        split_value = (prev_utxo.nValue - (30 * TX_FEE_PER_KB)) // NUM_SIGHASH_TESTS
        ...
        for i in range(NUM_SIGHASH_TESTS):
            temp_utxos.append(UTXO(tx.sha256, i, tx.vout[i].nValue))
        ...
        for i in range(NUM_SIGHASH_TESTS):
            ...
            temp_utxos.sort(key=lambda u: u.nValue, reverse=True)
            ...
            split_value = (total_value - (3 * TX_FEE_PER_KB)) // num_outputs
            assert split_value > 0
            for _ in range(num_outputs):
                tx.vout.append(CTxOut(split_value, script_pubkey))
            for i in range(num_inputs):
        ...
        temp_utxos.sort(key=lambda u: u.nValue, reverse=True)
        assert temp_utxos[0].nValue > 3 * TX_FEE_PER_KB
        pubkeyhash = hash160(pubkey)
        script_pkh = key_to_p2wpkh_script(pubkey)
        tx = CTransaction()
        tx.vin.append(CTxIn(COutPoint(temp_utxos[0].sha256, temp_utxos[0].n), b""))
        tx.vout.append(CTxOut(temp_utxos[0].nValue - TX_FEE_PER_KB, script_pkh))
        tx.wit.vtxinwit.append(CTxInWitness())
        sign_p2pk_witness_input(witness_script, tx, 0, SIGHASH_ALL, temp_utxos[0].nValue, key)
        tx2 = CTransaction()
        tx2.vin.append(CTxIn(COutPoint(tx.sha256, 0), b""))
        tx2.vout.append(CTxOut(tx.vout[0].nValue - TX_FEE_PER_KB, CScript([OP_TRUE])))
        ...
        consolidation_fee = 100 * TX_FEE_PER_KB
        output_value = (sum(i.nValue for i in temp_utxos) - consolidation_fee) // 2
    ...
    def test_non_standard_witness(self):
        ...
        outputvalue = (self.utxo[0].nValue - (2 * TX_FEE_PER_KB)) // (len(scripts) * 2)
        for i in scripts:
        ...
        for i in range(len(scripts)):
            ...
            p2wsh_tx.vout.append(CTxOut(outputvalue - (5 * TX_FEE_PER_KB), CScript([OP_0, hash160(b"")])))
            p2wsh_tx.wit.vtxinwit.append(CTxInWitness())
            p2wsh_tx.rehash()
            p2wsh_txs.append(p2wsh_tx)
            p2sh_tx = CTransaction()
            p2sh_tx.vin.append(CTxIn(COutPoint(txid, i * 2 + 1), CScript([p2wsh_scripts[i]])))
            p2sh_tx.vout.append(CTxOut(outputvalue - (5 * TX_FEE_PER_KB), CScript([OP_0, hash160(b"")])))
    ...
    def test_witness_sigops(self):
        ...
        split_value = (self.utxo[0].nValue - (20 * TX_FEE_PER_KB)) // outputs
        assert split_value > 0
        tx = CTransaction()
        tx.vin.append(CTxIn(COutPoint(self.utxo[0].sha256, self.utxo[0].n), b""))
        ...
        tx2.vout.append(CTxOut(total_value - (30 * TX_FEE_PER_KB), CScript([OP_TRUE])))
        tx2.rehash()
    ...
    def test_wtxid_relay(self):
        ...
        # Send tx2 through; it's an orphan so won't be accepted into the mempool.
        with p2p_lock:
            self.wtx_node.last_message.pop("getdata", None)
        test_transaction_acceptance(self.nodes[0], self.wtx_node, tx2, with_witness=True, accepted=False)

        # Send the parent through and verify the stored orphan is accepted.
