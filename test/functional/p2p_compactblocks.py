...
class CompactBlocksTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.utxos = []
        self.p2p_services = NODE_NETWORK | NODE_WITNESS
    ...
    def make_utxos(self):
        ...
        fee = get_quantum_quasar_min_tx_fee(tx)
        total_value = block.vtx[0].vout[0].nValue
        out_value = (total_value - fee) // 10
        for txout in tx.vout:
            txout.nValue = out_value
        tx.rehash()
    ...
    def build_block_with_transactions(self, node, utxo, num_transactions):
        ...
        for _ in range(num_transactions):
            ...
            tx.vout.append(CTxOut(utxo[2], CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])))
            tx.vout[0].nValue -= get_quantum_quasar_min_tx_fee(tx)
            tx.rehash()
    ...
    def test_low_work_compactblocks(self, test_node):
        ...
        node = self.nodes[0]
        # The inherited Bitcoin fixture assumes earlier setup has already
        # produced more than 150 blocks. This fork's smaller maturity/setup can
        # reach this point at a lower height, and the low-work compact-block
        # path only applies once the active chain exceeds the 144-block anti-DoS
        # buffer in GetAntiDoSWorkThreshold().
        if node.getblockcount() < 200:
            self.generate(node, 200 - node.getblockcount())
        stale_height = node.getblockcount() - 150
        hashPrevBlock = int(node.getblockhash(stale_height), 16)
        block = self.build_block_on_tip(node)
    ...
    def run_test(self):
        ...
        self.segwit_node = self.nodes[0].add_p2p_connection(TestP2PConn(), services=self.p2p_services)
        self.additional_segwit_node = self.nodes[0].add_p2p_connection(TestP2PConn(), services=self.p2p_services)
        self.onemore_inbound_node = self.nodes[0].add_p2p_connection(TestP2PConn(), services=self.p2p_services)
        self.outbound_node = self.nodes[0].add_outbound_p2p_connection(TestP2PConn(), p2p_idx=3, connection_type="outbound-full-relay", services=self.p2p_services)
