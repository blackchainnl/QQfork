...
class PeerTxRelayer(P2PTxInvStore):
    ...
    def wait_for_parent_requests(self, txids):
        """Wait for requests for missing parents by txid. If the test peer does not advertise
        witness support, this fork may ask for MSG_TX instead of MSG_WITNESS_TX. Requires that
        the getdata message match these txids exactly; all txids must be requested and no
        additional requests are allowed."""
        def test_function():
            last_getdata = self.last_message.get('getdata')
            if not last_getdata:
                return False
            return len(last_getdata.inv) == len(txids) and all([item.type in (MSG_TX, MSG_WITNESS_TX) and item.hash in txids for item in last_getdata.inv])
...
class OrphanHandlingTest(BitcoinTestFramework):
    ...
    def add_tx_relayer(self):
        return self.nodes[0].add_p2p_connection(PeerTxRelayer())

    def relay_transaction(self, peer, tx, *, expect_disconnect=False):
    ...
    def test_arrival_timing_orphan(self):
        ...
        peer_spy = self.add_tx_relayer()
        peer_normal = self.add_tx_relayer()
    ...
    def test_orphan_rejected_parents_exceptions(self):
        node = self.nodes[0]
        peer1 = self.add_tx_relayer()
        peer2 = self.add_tx_relayer()
        ...
        self.relay_transaction(peer1, parent_low_fee_nonsegwit["tx"], expect_disconnect=True)
        ...
        self.relay_transaction(peer1, parent_low_fee["tx"], expect_disconnect=True)
    ...
    def test_orphan_multiple_parents(self):
        ...
        # Use a confirmed input so the missing parent does not conflict with the
        # mempool parent that the orphan also spends.
    ...
    def test_orphans_overlapping_parents(self):
        ...
        # Sends the orphans
        peer_orphans = self.add_tx_relayer()
