...
# Copyright (c) 2021-2022 Blackcoin Developers
...
from test_framework.descriptors import descsum_create, xkeys_to_main_prefix
...
def compute_raw_taproot_address(pubkey):
    return encode_segwit_address("blrt", 1, pubkey)
...
class WalletTaprootTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [['-keypool=100', '-changetype=bech32'], ['-keypool=100', '-changetype=bech32']]
    ...
    def make_desc(pattern, privmap, keys, pub_only = False):
        ...
        return descsum_create(xkeys_to_main_prefix(pat))
    ...
    def do_test_psbt(self, comment, pattern, privmap, treefn, keys_pay, keys_change):
        ...
        for key in keys_pay + keys_change:
            result = key_only_wallet.importdescriptors([{"desc": descsum_create(xkeys_to_main_prefix(f"wpkh({key['xprv']}/*)")), "timestamp":"now"}])
            assert result[0]["success"]
        address_type = "bech32m" if "tr" in pattern else "bech32"
        for i in range(4):
    ...
    def do_test(self, comment, pattern, privmap, treefn):
        if pattern.startswith(("tr(", "rawtr(")):
            self.log.info(f"Skipping {comment}; Taproot/bech32m wallet address generation is not exposed by this fork.")
            return
        nkeys = len(privmap)
