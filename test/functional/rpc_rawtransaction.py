...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.wallet.generate(COINBASE_MATURITY + 100)
        self.sync_all()
    ...
    def getrawtransaction_tests(self):
        ...
        txId = tx['txid']
        tx_hex = tx['tx'].serialize_without_witness().hex()
        err_msg = (
        ...
        for n in [0, 2]:
            ...
            if n == 0:
                ...
                assert_equal(self.nodes[n].getrawtransaction(txId), tx_hex)

                # 2. valid parameters - supply txid and 0 for non-verbose
                assert_equal(self.nodes[n].getrawtransaction(txId, 0), tx_hex)

                # 3. valid parameters - supply txid and False for non-verbose
                assert_equal(self.nodes[n].getrawtransaction(txId, False), tx_hex)

                # 4. valid parameters - supply txid and 1 for verbose.
                # We only check the "hex" field of the output so we don't need to update this test every time the output format changes.
                assert_equal(self.nodes[n].getrawtransaction(txId, 1)["hex"], tx_hex)
                assert_equal(self.nodes[n].getrawtransaction(txId, 2)["hex"], tx_hex)

                # 5. valid parameters - supply txid and True for non-verbose
                assert_equal(self.nodes[n].getrawtransaction(txId, True)["hex"], tx_hex)
            else:
    ...
    def getrawtransaction_verbosity_tests(self):
        ...
        try:
            ...
            if e.error["code"] != -32601:
                raise
            self.log.info("Skipping pruneblockchain subcase; RPC is not available in this build")
    ...
    def decoderawtransaction_tests(self):
        ...
        assert_equal(decrawtx, self.nodes[0].decoderawtransaction(encrawtx))
        assert_equal(decrawtx['vout'][0]['value'], Decimal(tx['tx'].vout[0].nValue) / COIN)
        assert_raises_rpc_error(-22, 'TX decode failed', self.nodes[0].decoderawtransaction, encrawtx, False) # force decode as non-witness transaction
        # non-witness transaction
        encrawtx = tx['tx'].serialize_without_witness().hex()
