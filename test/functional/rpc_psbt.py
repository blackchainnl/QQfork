...
class PSBTTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Blackcoin regtest rewards are 10,000 BLK, not Bitcoin's 50 BTC;
        # derive the target from the selected UTXO so this remains a funding
        # behavior test instead of a subsidy-size test.
        target_exceeding_one_input = utxo1["amount"] + Decimal("1")
        assert_raises_rpc_error(-4, "The preselected coins total amount does not cover the transaction target. "
                                    "Please allow other inputs to be automatically selected or include more coins manually",
                                self.nodes[0].walletcreatefundedpsbt, [{"txid": utxo1['txid'], "vout": utxo1['vout']}], {self.nodes[2].getnewaddress(): target_exceeding_one_input})

        psbtx1 = self.nodes[0].walletcreatefundedpsbt([{"txid": utxo1['txid'], "vout": utxo1['vout']}], {self.nodes[2].getnewaddress(): target_exceeding_one_input}, 0, {"add_inputs": True})['psbt']
        ...
        self.log.info("Test walletcreatefundedpsbt enforces this fork's 100 sat/vB minimum fee rate")
        assert_raises_rpc_error(-4, "Fee rate (0.999 sat/vB) is lower than the minimum fee rate setting (100.000 sat/vB)",
        ...
        self.log.info("Test walletcreatefundedpsbt rejects non-standard zero-fee transactions")
        for param, zero_value in product(["fee_rate", "feeRate"], [0, 0.000, 0.00000000, "0", "0.000", "0.00000000"]):
            assert_raises_rpc_error(-4, "Fee rate (0.000 sat/vB) is lower than the minimum fee rate setting (100.000 sat/vB)",
        ...
        assert_raises_rpc_error(-3, "Unexpected key estimate_mode",
        ...
        for v in ["", {"foo": "bar"}, -1, 0, 1009]:
            assert_raises_rpc_error(-3, "Unexpected key conf_target",
                self.nodes[1].walletcreatefundedpsbt, inputs, outputs, 0, {"conf_target": v, "add_inputs": True})
        ...
        for bool_add, outputs_array in {True: outputs, False: [{self.nodes[1].getnewaddress(): 1}]}.items():
            max_fee_msg = "Fee exceeds maximum configured by user (e.g. -maxtxfee, maxfeerate)"
            assert_raises_rpc_error(-4, max_fee_msg, self.nodes[1].walletcreatefundedpsbt, inputs, outputs_array, 0, {"fee_rate": 1000000, "add_inputs": bool_add})
            fee_rate_msg = max_fee_msg if bool_add else "The preselected coins total amount does not cover the transaction target"
            assert_raises_rpc_error(-4, fee_rate_msg, self.nodes[1].walletcreatefundedpsbt, inputs, outputs_array, 0, {"feeRate": 100, "add_inputs": bool_add})
        ...
        blockhash = self.generate(self.nodes[0], 6, sync_fun=self.sync_blocks)[0]
        ...
        # Test additional args in walletcreatepsbt. This fork does not accept
        # Bitcoin's inherited replaceable option here, so keep the locktime and
        # BIP32-derivation coverage without asserting RBF sequence behavior.
        ...
        # Same construction with only locktime set and BIP32 derivation enabled.
        ...
        # Same construction without optional arguments on a second node.
        ...
        node2_funding_wallet = self.nodes[2].get_wallet_rpc(self.default_wallet_name)
        wunsafe = self.nodes[2].get_wallet_rpc("unsafe")
        node2_funding_wallet.sendtoaddress(wunsafe.getnewaddress(), 2)
        ...
        self.log.warning("Skipping inherited Bitcoin BIP174 static vectors; their embedded transactions do not use this fork's transaction serialization.")
        return

        # BIP 174 Test Vectors

        # Check that unknown values are just passed through
        unknown_psbt = "cHNidP8BAD8CAAAAAf//////////////////////////////////////////AAAAAAD/////AQAAAAAAAAAAA2oBAAAAAAAACg8BAgMEBQYHCAkPAQIDBAUGBwgJCgsMDQ4PAAA="  # noqa: V201
