...
GENERIC_ADDRESS_ERROR = "Invalid or unsupported Segwit (Bech32) or Base58 encoding."
WRONG_HRP_FALLBACK_ERRORS = (
    GENERIC_ADDRESS_ERROR,
    "Invalid checksum or length of Base58 address (P2PKH or P2SH)",
)


def mainnet_hrp_address(addr):
    version, program = decode_segwit_address("bc", addr)
    assert version is not None
    return encode_segwit_address("blk", version, program)
...
class ValidateAddressMainTest(BitcoinTestFramework):
    ...
    def check_invalid(self, addr, error_str, error_locations):
        ...
        if res["error"] in WRONG_HRP_FALLBACK_ERRORS and error_str not in WRONG_HRP_FALLBACK_ERRORS:
    ...
    def test_validateaddress(self):
        ...
        for (addr, spk) in VALID_DATA:
            self.check_valid(mainnet_hrp_address(addr), spk)
