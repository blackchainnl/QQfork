#!/usr/bin/env python3
# Copyright (c) 2021-present Blackcoin Core Developers
# Copyright (c) 2021-present Blackcoin More Developers
# Copyright (c) 2021-present Blackcoin Developers
...
class AssumeutxoTest(BitcoinTestFramework):
    ...
    def wait_for_indexes(self, node, required_indexes):
        """Wait for required indexes while allowing other enabled indexes."""
        def indexes_synced():
            index_info = node.getindexinfo()
            return all(index_info.get(name) == COMPLETE_IDX for name in required_indexes)

        self.wait_until(indexes_synced)
    ...
    def test_invalid_snapshot_scenarios(self, valid_snapshot_path):
        ...
        cases = [
            [b"\xff" * 32, 0, "3f2850fcaf2607ff59d23e381ab1460dbc38997192f2bac4787df140eb117bd4"], # wrong outpoint hash
            [(1).to_bytes(4, "little"), 32, "cb0ca19df01e9c05ededf698a76181e8de792efb99c1ae0e12a022b87210f004"], # wrong outpoint index
            [b"\x81", 36, "2e7014349b33e81bd9389255d7658270f1b5a84e19763555600d8588b2fb2ac8"], # wrong coin code VARINT((coinbase ? 1 : 0) | (height << 1))
            [b"\x83", 36, "9449917a1c6404d5289d1072b4dc165533cbfa069e92394a760f6e0d53988157"], # another wrong coin code
        ]
        ...
        for content, offset, wrong_hash in cases:
            ...
            expected_error(log_msg=f"[snapshot] bad snapshot content hash: expected c77dbcdc4a4a6f4f59d535ad1f4c449d056b63185b7ac3e0c916f6d4683c4066, got {wrong_hash}")
    ...
    def run_test(self):
        ...
        assert_equal(
            ...
            'c77dbcdc4a4a6f4f59d535ad1f4c449d056b63185b7ac3e0c916f6d4683c4066')
        ...
        # Ensure the indexes exercised by this path have synced. Blackcoin
        # may expose additional enabled indexes such as txindex.
        n1_indexes = ['basic block filter index', 'coinstatsindex']
        self.wait_for_indexes(n1, n1_indexes)
        ...
        for i in (0, 1):
            ...
            if i != 0:
                ...
                self.wait_for_indexes(n, n1_indexes)
        ...
        n2_indexes = ['basic block filter index', 'coinstatsindex', 'txindex']
