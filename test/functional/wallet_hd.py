...
class WalletHDTest(BitcoinTestFramework):
    ...
    def remove_chain_data(self, node):
        shutil.rmtree(node.blocks_path)
        shutil.rmtree(node.chain_path / "chainstate")
        shutil.rmtree(node.chain_path / "indexes", ignore_errors=True)
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            assert_equal(change_addrV["hdkeypath"], "m/44h/1h/0h/1/0")
        else:
            assert_equal(change_addrV["hdkeypath"], "m/0'/1'/0'")  #first internal child key

        # Legacy wallets also cover imported non-HD private keys. Descriptor
        # wallets do not expose dumpprivkey/importprivkey for this path, so keep
        # the descriptor variant focused on deterministic HD derivation.
        non_hd_add = None
        non_hd_balance = 0
        if not self.options.descriptors:
            non_hd_add = self.nodes[0].getnewaddress(address_type='legacy')
            non_hd_key = self.nodes[0].dumpprivkey(non_hd_add)
        ...
        change_addrV = self.nodes[1].getaddressinfo(change_addr)
        if self.options.descriptors:
            assert_equal(change_addrV["hdkeypath"], "m/44h/1h/0h/1/1")
        else:
            assert_equal(change_addrV["hdkeypath"], "m/0'/1'/1'")  #second internal child key

        self.sync_all()
        assert_equal(self.nodes[1].getbalance(), NUM_HD_ADDS + non_hd_balance)

        self.log.info("Restore backup ...")
        self.stop_node(1)
        # we need to delete the complete chain directory
        # otherwise node1 would auto-recover all funds in flag the keypool keys as used
        self.remove_chain_data(self.nodes[1])
        shutil.copyfile(
        ...
        if self.options.descriptors:
            assert_equal(keypath[0:14], "m/44h/1h/0h/1/")
        else:
