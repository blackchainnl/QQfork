...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def lock_outputs_type(self, wallet, outputtype):
        ...
        # The inherited Bitcoin fixture assumes a mixed script-type UTXO pool.
        # Descriptor wallets in this fork may expose only one spendable class;
        # locking all of it would test setup, not fundrawtransaction behavior.
        if len(to_lock) == len(utxos):
            return
    ...
    def assert_internal_weight_fee(self, wallet, fundedtx, fee_rate_sat_vb):
        fee_sats = int(fundedtx['fee'] * COIN)
        assert_equal(fee_sats % fee_rate_sat_vb, 0)
        internal_vsize = fee_sats // fee_rate_sat_vb
        assert_greater_than_or_equal(internal_vsize, wallet.decoderawtransaction(fundedtx['hex'])['vsize'])

    def bump_mocktime(self, node, seconds=1):
        if node.mocktime is not None:
            node.setmocktime(node.mocktime + seconds)
    ...
    def test_change_position(self):
        ...
        exact_match_addr = self.nodes[2].getnewaddress()
        self.nodes[2].sendtoaddress(exact_match_addr, 50)
        self.generate(self.nodes[2], 1)
        exact_match_utxo = [utxo for utxo in self.nodes[2].listunspent() if utxo["amount"] == 50 and utxo["address"] == exact_match_addr][0]
        locked_utxos = [{"txid": utxo["txid"], "vout": utxo["vout"]} for utxo in self.nodes[2].listunspent() if utxo["txid"] != exact_match_utxo["txid"] or utxo["vout"] != exact_match_utxo["vout"]]
        self.nodes[2].lockunspent(False, locked_utxos)
        rawmatch = self.nodes[2].createrawtransaction([], {self.nodes[2].getnewaddress():50})
        rawmatch = self.nodes[2].fundrawtransaction(rawmatch, changePosition=1, subtractFeeFromOutputs=[0])
        assert_equal(rawmatch["changepos"], -1)
        self.nodes[2].lockunspent(True, locked_utxos)
    ...
    def test_locked_wallet(self):
        ...
        if self.options.descriptors:
            with WalletUnlock(wallet, "test"):
                wallet.importdescriptors([{
                    'desc': descsum_create_with_main_xkeys('wpkh(tprv8ZgxMBicQKsPdYeeZbPSKd2KYLmeVKtcFA7kqCxDvDR13MQ6us8HopUR2wLcS2ZKPhLyKsqpDL2FtL73LMHcgoCL7DXsciA8eX8nbjCR2eG/0h/*h)'),
                    'timestamp': 'now',
                    'active': True
                },
                {
                    'desc': descsum_create_with_main_xkeys('wpkh(tprv8ZgxMBicQKsPdYeeZbPSKd2KYLmeVKtcFA7kqCxDvDR13MQ6us8HopUR2wLcS2ZKPhLyKsqpDL2FtL73LMHcgoCL7DXsciA8eX8nbjCR2eG/1h/*h)'),
                    'timestamp': 'now',
                    'active': True,
                    'internal': True
                }])
        ...
        # Produce a changeless transaction without relying on the inherited
        # Bitcoin fixture's exact serialized-size estimate.
        value = inputs[0]["amount"]

        outputs = {self.nodes[0].getnewaddress():value}
        rawtx = wallet.createrawtransaction(inputs, outputs)
        # fund a transaction that does not require a new key for the change output
        funded_tx = wallet.fundrawtransaction(rawtx, subtractFeeFromOutputs=[0])
        ...
        if self.options.descriptors:
            ...
            assert_greater_than_or_equal(funded_tx["changepos"], 0)
        else:
        ...
        inputs = []
        receive_addr = self.nodes[0].getnewaddress()
        outputs = {receive_addr:1.1}
        rawtx = wallet.createrawtransaction(inputs, outputs)
        ...
        with WalletUnlock(wallet, "test"):
            ...
            # Make sure funds are received by the intended wallet.
            assert_equal(self.nodes[0].getreceivedbyaddress(receive_addr), Decimal('1.10000000'))
    ...
    def test_many_inputs_fee(self):
        ...
        self.bump_mocktime(self.nodes[1])
    ...
    def test_many_inputs_send(self):
        ...
        outputs = {receive_addr_a:0.15, receive_addr_b:0.04}
        rawtx = self.nodes[1].createrawtransaction(inputs, outputs)
        fundedTx = self.nodes[1].fundrawtransaction(rawtx)
        fundedAndSignedTx = self.nodes[1].signrawtransactionwithwallet(fundedTx['hex'])
        self.nodes[1].sendrawtransaction(fundedAndSignedTx['hex'])
        self.generate(self.nodes[1], 1)
        assert_equal(self.nodes[0].getreceivedbyaddress(receive_addr_a), Decimal('0.15000000'))
        assert_equal(self.nodes[0].getreceivedbyaddress(receive_addr_b), Decimal('0.04000000'))
    ...
    def test_watchonly(self):
        ...
        desc_import = [{
            "desc": descsum_create_with_main_xkeys("wpkh(tpubD6NzVbkrYhZ4YNXVQbNhMK1WqguFsUXceaVJKbmno2aZ3B6QfbMeraaYvnBSGpV3vxLyTTK9DYT1yoEck4XUScMzXoQ2U2oSmE2JyMedq3H/1/*)"),
            "timestamp": "now",
            "internal": True,
            "active": True,
            "keypool": True,
        ...
        if self.options.descriptors:
            result = wwatch.fundrawtransaction(rawtx, {"includeWatching": True, "change_type": "bech32"})
        else:
    ...
    def test_option_feerate(self):
        ...
        # This fork enforces the configured minimum relay fee; inherited
        # zero-fee Bitcoin fixtures are rejected by policy.
        for param, zero_value in product(["fee_rate", "feeRate"], [0, 0.000, 0.00000000, "0", "0.000", "0.00000000"]):
            assert_raises_rpc_error(-4, "lower than the minimum fee rate", node.fundrawtransaction, rawtx, {param: zero_value})

        default_result = node.fundrawtransaction(rawtx)
        assert_fee_amount(default_result["fee"], count_bytes(default_result["hex"]), result_fee_rate)
        high_rate_sat_vb = 10000
        result = node.fundrawtransaction(rawtx, fee_rate=high_rate_sat_vb)
        self.assert_internal_weight_fee(node, result, high_rate_sat_vb)

        self.log.info("Test fundrawtxn rejects unsupported fee-estimation options")
        assert_raises_rpc_error(-8, "Unknown named parameter estimate_mode",
            node.fundrawtransaction, rawtx, estimate_mode="economical", add_inputs=True)
        assert_raises_rpc_error(-8, "Unknown named parameter conf_target",
            node.fundrawtransaction, rawtx, conf_target=1, add_inputs=True)

        self.log.info("Test invalid fee rate settings")
        for param, value in {("fee_rate", 100000), ("feeRate", 1.000)}:
            assert_greater_than(node.fundrawtransaction(rawtx, add_inputs=True, **{param: value})["fee"], 0)
        ...
        self.log.info("Test fundrawtxn enforces this fork's minimum fee rate")
        assert_raises_rpc_error(-4, "lower than the minimum fee rate",
        ...
        for param in ["feeRate", "fee_rate"]:
            ...
            assert_raises_rpc_error(-3, "Unexpected key conf_target",
    ...
    def test_external_inputs(self):
        ...
        funded_tx2 = wallet.fundrawtransaction(raw_tx, input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": low_input_weight}], fee_rate=input_weight_fee_rate)
        assert_greater_than(funded_tx["fee"], funded_tx2["fee"])
        # Increasing the weight should have a higher fee
        funded_tx2 = wallet.fundrawtransaction(raw_tx, input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": high_input_weight}], fee_rate=input_weight_fee_rate)
        assert_greater_than(funded_tx2["fee"], funded_tx["fee"])
        # The provided weight should override the calculated weight when solving data is provided
        funded_tx3 = wallet.fundrawtransaction(raw_tx, solving_data={"descriptors": [desc]}, input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": high_input_weight}], fee_rate=input_weight_fee_rate)
        assert_equal(funded_tx2["fee"], funded_tx3["fee"])
        # The feerate should be met
        high_fee_rate = 200
        funded_tx4 = wallet.fundrawtransaction(raw_tx, input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": high_input_weight}], fee_rate=high_fee_rate)
        ...
        assert_fee_amount(funded_tx4["fee"], tx4_vsize, Decimal(high_fee_rate) / Decimal(100000))

        # Funding with weight at csuint boundaries should not cause problems
        funded_tx = wallet.fundrawtransaction(raw_tx, input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": 255}], fee_rate=input_weight_fee_rate)
        funded_tx = wallet.fundrawtransaction(raw_tx, input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": 65539}], fee_rate=input_weight_fee_rate)
    ...
    def test_add_inputs_default_value(self):
        ...
        # Create and fund the wallet. The inherited Bitcoin test used a tight
        # 5-BTC fixture; this fork's fee/dust policy needs more headroom while
        # preserving the add_inputs coverage.
        ...
        self.nodes[0].sendtoaddress(addr1, 20)
        self.generate(self.nodes[0], 1)
    ...
    def test_weight_calculation(self):
        ...
        weight_test_fee_rate = 100
        fundedtx = wallet.fundrawtransaction(rawtx, fee_rate=weight_test_fee_rate, change_type="bech32")
        self.assert_internal_weight_fee(wallet, fundedtx, weight_test_fee_rate)
        ...
        fundedtx = wallet.fundrawtransaction(rawtx, fee_rate=weight_test_fee_rate, change_type="bech32", solving_data={"descriptors": [ext_desc]})
        self.assert_internal_weight_fee(wallet, fundedtx, weight_test_fee_rate)
    ...
    def test_include_unsafe(self):
        ...
        for i in range(0, 2):
            ...
            if txid not in self.nodes[0].getrawmempool():
                self.nodes[0].sendrawtransaction(self.nodes[2].getrawtransaction(txid))
    ...
    def test_feerate_rounding(self):
        ...
        # The inherited Bitcoin fixture used 1.85 sat/vB; this fork enforces a
        # 100 sat/vB policy floor, so scale the same rounding edge to 185 sat/vB.
        # The input will need a fee of 12580 sats and the rest 7770 sats.
        # The entire tx fee should be 20350 sats.
        ...
        # To test this does not happen, we subtract 20200 sats from the input value. If working correctly, this should
        # fail with insufficient funds rather than bitcoind asserting.
        rawtx = w.createrawtransaction(inputs=[], outputs=[{self.nodes[0].getnewaddress(address_type="bech32"): 1 - 0.00020200}])
        assert_raises_rpc_error(-4, "Insufficient funds", w.fundrawtransaction, rawtx, fee_rate=185)
    ...
    def test_input_confs_control(self):
        ...
        wallet.sendtoaddress(wallet.getnewaddress(), 0.5)

        self.log.info("Assert unsupported minconf/maxconf fundrawtransaction options are rejected")
        target_address = self.nodes[2].getnewaddress()
        raw_tx1 = wallet.createrawtransaction([], {target_address: 0.1}, 0, True)
        assert_raises_rpc_error(-3, "Unexpected key maxconf", wallet.fundrawtransaction, raw_tx1, {'fee_rate': 100, 'maxconf': 0})
        assert_raises_rpc_error(-3, "Unexpected key minconf", wallet.fundrawtransaction, raw_tx1, {'fee_rate': 100, 'minconf': 2})
