...
class CompactFiltersTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # This fork only serves stale blocks inside a 10-hour anti-fingerprinting
        # window. Keep the stale branch near the active tip so the stale compact
        # filter serving checks below still exercise the allowed P2P path.
        ...
        assert_equal(
            response.headers,
            [int(header, 16) for header in (main_cfcheckpt,)],
        )
        ...
        self.log.info("Extend active chain for the too-many-cfheaders invalid request.")
        self.generate(self.nodes[0], 2000 - self.nodes[0].getblockcount())
        self.nodes[0].syncwithvalidationinterfacequeue()
        long_tip_hash = self.nodes[0].getbestblockhash()
        ...
        requests = [
            ...
            (
                msg_getcfheaders(
                    filter_type=FILTER_TYPE_BASIC,
                    start_height=0,
                    stop_hash=int(long_tip_hash, 16),
                ), "requested too many cfilters/cfheaders"
            ),
