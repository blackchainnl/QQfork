...
class RESTTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        hex_tx = hex_response.read().decode('utf-8').strip()
        assert_equal(self.nodes[0].decoderawtransaction(hex_tx)['txid'], txid)
        assert_greater_than_or_equal(int(hex_response.getheader('content-length')), len(hex_tx))
        ...
        self.wait_until(basic_filter_index_synced)
