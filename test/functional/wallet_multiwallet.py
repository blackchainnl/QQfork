...
class MultiWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(w5_info['immature_balance'], REGTEST_BLOCK_REWARD)
        ...
        for wallet_name, wallet in zip(wallet_names, wallets):
            ...
            assert_equal(info['immature_balance'], REGTEST_BLOCK_REWARD if wallet is wallets[0] else 0)
        ...
        # Blackcoin's transaction timestamp rule rejects a spend whose nTime is
        # earlier than the input coin time. Pin mocktime past the generated tip
        # before creating wallet spends, otherwise the first CLI-routed send can
        # be constructed earlier than a freshly matured input and fail mempool
        # broadcast with bad-txns-time-earlier-than-input.
        node.setmocktime(node.getblockheader(node.getbestblockhash())["time"] + 1)
        assert_equal(w1.getbalance(), 2 * REGTEST_BLOCK_REWARD)
