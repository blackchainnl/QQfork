...
""" Example logging Blackcoin mempool events using the mempool:added,
...
def main(daemon_path):
    daemon_with_usdts = USDT(path=str(daemon_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    daemon_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    daemon_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    daemon_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    daemon_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[daemon_with_usdts])
