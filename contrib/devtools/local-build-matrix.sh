...
scan_warnings() {
    ...
    # The warning expressions intentionally contain literal backticks and
    # apostrophes from third-party output; they must not be shell-expanded.
    # shellcheck disable=SC2016
    grep -vE 'apt-key|debconf: delaying package configuration|WARNING: apt does not have a stable CLI interface|sha256sum: WARNING: [0-9]+ computed checksum|computed checksum did NOT match|update-alternatives: warning: skip creation of|libtool: (install: )?warning: (remember to run|relinking)|configure: WARNING: (dot|doxygen) not found - .*(doxygen|documentation) targets will be skipped|configure: WARNING: using cross tools not prefixed with host triplet|WARNING: QDoc will not be compiled|WARNING: Failure to find: .*/qt5(core|gui|widgets)_metatypes\.json|(\.\./3rdparty/md4c/md4c\.c|access/qnetworkaccessmanager\.cpp|/usr/include/.*/bits/string_fortified\.h|../../include/QtCore/.*/qdatastream\.h):.*warning:|(icccm|ewmh)\.c:[0-9]+:[0-9]+: warning: ISO C90 forbids mixed declarations and code|(\.\./dist/.*db_err\.c|\./db_int\.h):[0-9]+:[0-9]+: warning: .*-Wformat-truncation|configure\.ac:[0-9]+: warning: (The macro `AC_|AC_LIBTOOL_WIN32_DLL)|configure: WARNING: xgettext is required to update qt translations|Package xorg-macros was not found in the pkg-config search path|Perhaps you should add the directory containing `xorg-macros\.pc|No package '\''xorg-macros'\'' found|ar: `u'\'' modifier ignored since `D'\'' is the default|clang: warning: argument unused during compilation: '\''-nostdlibinc'\'' \[-Wunused-command-line-argument\]|/native/bin/.*-strip: warning: changes being made to the file will invalidate the code signature in:|Warning: Could not detect Qt'\''s path, skipping plugin deployment!|Warning: Could not find any external frameworks to deploy in dist/blackcoin-qt\.app\.' \
        "$warnings_file.tmp" \
        > "$warnings_file" || true
