...
static int64_t QuantumColdStakeInputWeight()
{
    const int64_t non_witness = 32 + 4 + GetSizeOfCompactSize(0) + 4;
...
static util::Result<CAmount> GetDemurrageAdjustedSelectedValue(CWallet& wallet, const CTransaction& tx) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    if (tx.IsCoinBase()) return 0;

    const CBlockIndex* tip = wallet.chain().getTip();
    if (!tip) {
        return util::Error{_("Unable to evaluate demurrage without an active chain tip")};
    }
    const int spend_height = tip->nHeight + 1;
    const int64_t spend_time = tip->GetMedianTimePast();
    const Consensus::Params& consensus = Params().GetConsensus();
    ...
    CAmount adjusted_value{0};
    for (const CTxIn& txin : tx.vin) {
        auto it = coins.find(txin.prevout);
        if (it == coins.end() || it->second.out.IsNull()) {
        ...
        if (!MoneyRange(adjusted_value + eval.effective_value)) {
            return util::Error{_("Demurrage-adjusted input value is out of range")};
        }
        adjusted_value += eval.effective_value;
    }
...
int CalculateMaximumSignedInputSize(const CTxOut& txout, const COutPoint outpoint, const SigningProvider* provider, bool can_grind_r, const CCoinControl* coin_control)
{
    if (IsQuantumMigrationScript(txout.scriptPubKey)) {
        return static_cast<int>(GetVirtualTransactionSize(QuantumMigrationInputWeight(), 0, 0));
    }
    if (IsQuantumColdStakeScript(txout.scriptPubKey)) {
        return static_cast<int>(GetVirtualTransactionSize(QuantumColdStakeInputWeight(), 0, 0));
    }
                                                  ...
                                                  const bool can_grind_r)
{
    ...
    if (IsQuantumMigrationScript(txo.scriptPubKey)) {
        return QuantumMigrationInputWeight();
    }
    if (IsQuantumColdStakeScript(txo.scriptPubKey)) {
        return QuantumColdStakeInputWeight();
    }
...
TxSize CalculateMaximumSignedTxSize(const CTransaction &tx, const CWallet *wallet, const std::vector<CTxOut>& txouts, const CCoinControl* coin_control)
{
    ...
    bool is_segwit = std::any_of(txouts.begin(), txouts.end(), [&](const CTxOut& txo) {
        if (IsQuantumMigrationScript(txo.scriptPubKey)) return true;
        if (IsQuantumColdStakeScript(txo.scriptPubKey)) return true;
        std::unique_ptr<Descriptor> desc{GetDescriptor(wallet, coin_control, txo.scriptPubKey)};
                           ...
                           const CoinFilterParams& params)
{
    ...
    for (const auto& entry : wallet.mapWallet)
    {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            // Spending an RGB seal / EUTXO-state UTXO as ordinary coin burns the asset; never
            // auto-select it. Deliberate RGB transfers select the seal explicitly via coin control.
        ...
        const QuantumFundingPhase& quantum_phase) EXCLUSIVE_LOCKS_REQUIRED(::cs_main, wallet.cs_wallet)
{
    ...
    for (const auto& recipient : vecSend)
    {
        CTxOut txout(recipient.nAmount, recipient.scriptPubKey ? *recipient.scriptPubKey : GetScriptForDestination(recipient.dest));
    ...
    const CAmount nominal_selected_value = result.GetSelectedValue();
    CAmount selected_value = nominal_selected_value;
    if (const CBlockIndex* tip = wallet.chain().getTip();
        tip && Params().GetConsensus().IsDemurrageActive(tip->nHeight + 1, tip->GetMedianTimePast())) {
        auto adjusted_selected_value = GetDemurrageAdjustedSelectedValue(wallet, CTransaction(txNew));
        if (!adjusted_selected_value) return util::Error{util::ErrorString(adjusted_selected_value)};
        selected_value = *adjusted_selected_value;
    }
    CAmount current_fee = selected_value - output_value;

    // Sanity check that the fee cannot be negative as that means we have more output value than input value
    if (current_fee < 0) {
        if (selected_value < nominal_selected_value) {
            return util::Error{_("Selected inputs have decayed below the requested output value under demurrage. Reduce the amount or use a full-sweep spend so the decayed remainder is burned.")};
        }
    ...
    if (nChangePosInOut != -1 && fee_needed < current_fee) {
        ...
        if (!MoneyRange(change.nValue)) {
            return util::Error{Untranslated(STR_INTERNAL_BUG("Change adjustment: change output out of range"))};
        }
        current_fee = selected_value - CalculateOutputValue(txNew);
    ...
    // Blackcoin: and if we don't pay enough fees, reduce the change
        ...
        auto& change = txNew.vout.at(nChangePosInOut);
        const CAmount change_reduction = fee_needed - current_fee;
        if (change_reduction >= change.nValue) {
            txNew.vout.erase(txNew.vout.begin() + nChangePosInOut);
            nChangePosInOut = -1;
        } else {
            change.nValue -= change_reduction;
            if (!MoneyRange(change.nValue)) {
                return util::Error{Untranslated(STR_INTERNAL_BUG("Change adjustment: change output out of range"))};
            }
            if (change.nValue < coin_selection_params.min_viable_change || IsDust(change, wallet.chain().relayDustFee())) {
                txNew.vout.erase(txNew.vout.begin() + nChangePosInOut);
                nChangePosInOut = -1;
            }
        }
    ...
    if (sign) {
        ...
        if (!wallet.SignTransaction(txNew, input_errors)) {
            if (!input_errors.empty()) {
                return util::Error{strprintf(_("Signing transaction failed: %s"), input_errors.begin()->second)};
            }
...
util::Result<CreatedTransactionResult> CreateUTXOOptimizationTransaction(CWallet& wallet, const CTxDestination& dest, CAmount utxo_amount, const std::optional<CTxDestination>& from_address)
{
    if (utxo_amount <= 0) {
    ...
    if (from_address) {
        for (const COutput& out : AvailableCoins(wallet, &coin_control).All()) {
            CTxDestination out_dest;
            if (ExtractDestination(out.txout.scriptPubKey, out_dest) && out_dest == *from_address) {
        ...
        available_coins = GetBalance(wallet).m_mine_trusted;
    }

    if (available_coins == 0) {
        return util::Error{_("No available coins to optimize")};
    }

    wallet.WalletLogPrintf("optimizing outputs %d satoshis\n", available_coins);
    CAmount remaining = available_coins;

    const CRecipient recipient{dest, utxo_amount, false};
    CMutableTransaction tx_tmp;
    const uint32_t sequence{CTxIn::MAX_SEQUENCE_NONFINAL};
    for (const COutPoint& outpoint : coin_control.ListSelected()) {
        tx_tmp.vin.push_back(CTxIn(outpoint, CScript(), sequence));
    }

    // Estimate the base transaction size and the marginal size of each added
    // output so the fee can be provisioned before choosing how many
    // equal-value outputs fit.
    TxSize tx_sizes = CalculateMaximumSignedTxSize(CTransaction(tx_tmp), &wallet, &coin_control);
    const int base_bytes = tx_sizes.vsize;

    tx_tmp.vout.push_back(CTxOut(utxo_amount, GetScriptForDestination(dest)));
...
bool FundTransaction(CWallet& wallet, CMutableTransaction& tx, CAmount& nFeeRet, int& nChangePosInOut, bilingual_str& error, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, CCoinControl coinControl)
{
    ...
    for (size_t idx = 0; idx < tx.vout.size(); idx++) {
        ...
        CTxDestination dest;
        const bool extracted = ExtractDestination(txOut.scriptPubKey, dest);
        CRecipient recipient = {dest, txOut.nValue, setSubtractFeeFromOutputs.count(idx) == 1};
        if (!extracted || GetScriptForDestination(dest) != txOut.scriptPubKey) {
            recipient.scriptPubKey = txOut.scriptPubKey;
        }
