...
extern const std::string CRYPTED_QUANTUM_KEY;
...
extern const std::string EUTXO_STATE;
extern const std::string RGB_ASSIGNMENT;
extern const std::string RGB_CONTRACT;
extern const std::string RGB_GENESIS_PROOF;
...
public:
    ...
    bool WriteQuantumRedelegationLastSuccessHeight(const std::vector<unsigned char>& witness_program, int height);
    bool WriteQuantumRedelegationLastWinHeight(const std::vector<unsigned char>& witness_program, int height);
    bool EraseQuantumRedelegationLastWinHeight(const std::vector<unsigned char>& witness_program);
    bool WriteRGBContract(const uint256& contract_id, const RGBContractRecord& record);
    bool WriteRGBAssignment(const uint256& contract_id, const COutPoint& outpoint, const RGBOwnedAssignmentRecord& record);
    bool WriteRGBGenesisProof(const uint256& contract_id, const RGBGenesisProofRecord& record);
    bool WriteRGBTransition(const uint256& contract_id, const uint256& transition_id, const RGBTransitionRecord& record);
    bool WriteRGBTransitionProof(const uint256& contract_id, const uint256& transition_id, const RGBTransitionProofRecord& record);
    bool WriteEUTXOState(const COutPoint& outpoint, const EUTXOStateRecord& record);
    bool EraseRGBAssignment(const uint256& contract_id, const COutPoint& outpoint);
    bool EraseRGBTransition(const uint256& contract_id, const uint256& transition_id);
    bool EraseRGBTransitionProof(const uint256& contract_id, const uint256& transition_id);
