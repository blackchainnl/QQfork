...
const std::string CRYPTED_QUANTUM_KEY{"cquantumkey"};
...
const std::string EUTXO_STATE{"eutxostate"};
const std::string RGB_ASSIGNMENT{"rgbassignment"};
const std::string RGB_CONTRACT{"rgbcontract"};
const std::string RGB_GENESIS_PROOF{"rgbgenesisproof"};
...
bool WalletBatch::WriteQuantumKey(const std::vector<unsigned char>& public_key, const CKeyingMaterial& private_key, const CKeyMetadata& keyMeta)
{
    std::vector<unsigned char> private_key_bytes(private_key.begin(), private_key.end());

    std::vector<unsigned char> key_material;
    key_material.reserve(public_key.size() + private_key_bytes.size());
    key_material.insert(key_material.end(), public_key.begin(), public_key.end());
    key_material.insert(key_material.end(), private_key_bytes.begin(), private_key_bytes.end());

    const bool written = WriteIC(std::make_pair(DBKeys::QUANTUM_KEY, public_key), std::make_pair(std::make_pair(private_key_bytes, keyMeta), Hash(key_material)), false);
    if (!private_key_bytes.empty()) {
        memory_cleanse(private_key_bytes.data(), private_key_bytes.size());
    }
    if (!key_material.empty()) {
        memory_cleanse(key_material.data(), key_material.size());
    }
    return written;
}

bool WalletBatch::WriteCryptedQuantumKey(const std::vector<unsigned char>& public_key, const std::vector<unsigned char>& crypted_private_key, const CKeyMetadata& keyMeta)
{
    if (!WriteIC(std::make_pair(DBKeys::CRYPTED_QUANTUM_KEY, public_key), std::make_pair(std::make_pair(crypted_private_key, keyMeta), Hash(crypted_private_key)), false)) {
        return false;
    }
...
{
    return WriteIC(std::make_pair(DBKeys::QUANTUM_COLDSTAKE, witness_program), record);
}

bool WalletBatch::WriteQuantumRedelegationAutoScanHeight(int height)
{
    return WriteIC(DBKeys::QUANTUM_REDELEGATION_AUTO_SCAN, height);
}

bool WalletBatch::WriteQuantumRedelegationLastAttemptHeight(const std::vector<unsigned char>& witness_program, int height)
{
    return WriteIC(std::make_pair(DBKeys::QUANTUM_REDELEGATION_LAST_ATTEMPT, witness_program), height);
}

bool WalletBatch::WriteQuantumRedelegationLastSuccessHeight(const std::vector<unsigned char>& witness_program, int height)
{
    return WriteIC(std::make_pair(DBKeys::QUANTUM_REDELEGATION_LAST_SUCCESS, witness_program), height);
}

bool WalletBatch::WriteQuantumRedelegationLastWinHeight(const std::vector<unsigned char>& witness_program, int height)
{
    return WriteIC(std::make_pair(DBKeys::QUANTUM_REDELEGATION_LAST_WIN, witness_program), height);
}

bool WalletBatch::EraseQuantumRedelegationLastWinHeight(const std::vector<unsigned char>& witness_program)
{
    return EraseIC(std::make_pair(DBKeys::QUANTUM_REDELEGATION_LAST_WIN, witness_program));
}

bool WalletBatch::WriteRGBContract(const uint256& contract_id, const RGBContractRecord& record)
{
    return WriteIC(std::make_pair(DBKeys::RGB_CONTRACT, contract_id), record);
}

bool WalletBatch::WriteRGBAssignment(const uint256& contract_id, const COutPoint& outpoint, const RGBOwnedAssignmentRecord& record)
{
    return WriteIC(std::make_pair(DBKeys::RGB_ASSIGNMENT, std::make_pair(contract_id, outpoint)), record);
}

bool WalletBatch::WriteRGBGenesisProof(const uint256& contract_id, const RGBGenesisProofRecord& record)
{
    return WriteIC(std::make_pair(DBKeys::RGB_GENESIS_PROOF, contract_id), record);
}

bool WalletBatch::WriteRGBTransition(const uint256& contract_id, const uint256& transition_id, const RGBTransitionRecord& record)
{
    return WriteIC(std::make_pair(DBKeys::RGB_TRANSITION, std::make_pair(contract_id, transition_id)), record);
}

bool WalletBatch::WriteRGBTransitionProof(const uint256& contract_id, const uint256& transition_id, const RGBTransitionProofRecord& record)
{
    return WriteIC(std::make_pair(DBKeys::RGB_TRANSITION_PROOF, std::make_pair(contract_id, transition_id)), record);
}

bool WalletBatch::WriteEUTXOState(const COutPoint& outpoint, const EUTXOStateRecord& record)
{
    return WriteIC(std::make_pair(DBKeys::EUTXO_STATE, outpoint), record);
}

bool WalletBatch::EraseRGBAssignment(const uint256& contract_id, const COutPoint& outpoint)
{
    return EraseIC(std::make_pair(DBKeys::RGB_ASSIGNMENT, std::make_pair(contract_id, outpoint)));
}

bool WalletBatch::EraseRGBTransition(const uint256& contract_id, const uint256& transition_id)
{
    return EraseIC(std::make_pair(DBKeys::RGB_TRANSITION, std::make_pair(contract_id, transition_id)));
}

bool WalletBatch::EraseRGBTransitionProof(const uint256& contract_id, const uint256& transition_id)
{
    return EraseIC(std::make_pair(DBKeys::RGB_TRANSITION_PROOF, std::make_pair(contract_id, transition_id)));
}
...
bool LoadQuantumKey(CWallet* pwallet, DataStream& ssKey, DataStream& ssValue, std::string& strErr)
{
    LOCK(pwallet->cs_wallet);
    try {
        if (pwallet->IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS) || pwallet->IsWalletFlagSet(WALLET_FLAG_EXTERNAL_SIGNER)) {
            strErr = "Error reading wallet database: quantum private key found in wallet that disallows private keys";
            return false;
        }
        ...
        std::pair<std::vector<unsigned char>, CKeyMetadata> payload;
        uint256 checksum;
        ssValue >> payload;
        ssValue >> checksum;

        std::vector<unsigned char> key_material;
        key_material.reserve(public_key.size() + payload.first.size());
        key_material.insert(key_material.end(), public_key.begin(), public_key.end());
        key_material.insert(key_material.end(), payload.first.begin(), payload.first.end());
        if (Hash(key_material) != checksum) {
            if (!payload.first.empty()) {
                memory_cleanse(payload.first.data(), payload.first.size());
            }
            if (!key_material.empty()) {
                memory_cleanse(key_material.data(), key_material.size());
            }
            strErr = "Error reading wallet database: ML-DSA public/private key corrupt";
            return false;
        }

        CKeyingMaterial private_key(payload.first.begin(), payload.first.end());
        if (!payload.first.empty()) {
            memory_cleanse(payload.first.data(), payload.first.size());
        }
        if (!key_material.empty()) {
        ...
        if (!pwallet->LoadQuantumKey(public_key, private_key, payload.second.nCreateTime)) {
            strErr = "Error reading wallet database: CWallet::LoadQuantumKey failed";
            return false;
        }
    } catch (const std::exception& e) {
        if (strErr.empty()) {
...
bool LoadCryptedQuantumKey(CWallet* pwallet, DataStream& ssKey, DataStream& ssValue, std::string& strErr)
{
    LOCK(pwallet->cs_wallet);
    try {
        if (pwallet->IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS) || pwallet->IsWalletFlagSet(WALLET_FLAG_EXTERNAL_SIGNER)) {
            strErr = "Error reading wallet database: encrypted quantum private key found in wallet that disallows private keys";
            return false;
        }
        ...
        ssValue >> payload;
        ssValue >> checksum;
        if (Hash(payload.first) != checksum) {
            strErr = "Error reading wallet database: encrypted ML-DSA key corrupt";
            return false;
        }

        if (!pwallet->LoadCryptedQuantumKey(public_key, payload.first, payload.second.nCreateTime)) {
            strErr = "Error reading wallet database: CWallet::LoadCryptedQuantumKey failed";
            return false;
        }
    } catch (const std::exception& e) {
        if (strErr.empty()) {
...
bool LoadQuantumColdStakeDelegation(CWallet* pwallet, DataStream& ssKey, DataStream& ssValue, std::string& strErr)
{
    LOCK(pwallet->cs_wallet);
    try {
        std::vector<unsigned char> witness_program;
        ssKey >> witness_program;

        QuantumColdStakeDelegationRecord record;
        ssValue >> record;

        if (!pwallet->LoadQuantumColdStakeDelegation(witness_program, record)) {
            strErr = "Error reading wallet database: CWallet::LoadQuantumColdStakeDelegation failed";
            return false;
        }
    } catch (const std::exception& e) {
        if (strErr.empty()) {
...
bool LoadRGBContract(CWallet* pwallet, DataStream& ssKey, DataStream& ssValue, std::string& strErr)
{
    LOCK(pwallet->cs_wallet);
    try {
        uint256 contract_id;
        ...
        if (!pwallet->LoadRGBContract(contract_id, record)) {
            strErr = "Error reading wallet database: CWallet::LoadRGBContract failed";
            return false;
        }
    } catch (const std::exception& e) {
        if (strErr.empty()) {
...
bool LoadRGBAssignment(CWallet* pwallet, DataStream& ssKey, DataStream& ssValue, std::string& strErr)
{
    LOCK(pwallet->cs_wallet);
    try {
        std::pair<uint256, COutPoint> key;
        ssKey >> key;

        RGBOwnedAssignmentRecord record;
        ssValue >> record;

        if (!pwallet->LoadRGBAssignment(key.first, key.second, record)) {
            strErr = "Error reading wallet database: CWallet::LoadRGBAssignment failed";
            return false;
        }
    } catch (const std::exception& e) {
        if (strErr.empty()) {
...
bool LoadRGBGenesisProof(CWallet* pwallet, DataStream& ssKey, DataStream& ssValue, std::string& strErr)
{
    LOCK(pwallet->cs_wallet);
    try {
        uint256 contract_id;
        ...
        RGBGenesisProofRecord record;
        ssValue >> record;

        if (!pwallet->LoadRGBGenesisProof(contract_id, record)) {
            strErr = "Error reading wallet database: CWallet::LoadRGBGenesisProof failed";
            return false;
        }
    } catch (const std::exception& e) {
        if (strErr.empty()) {
...
bool LoadRGBTransition(CWallet* pwallet, DataStream& ssKey, DataStream& ssValue, std::string& strErr)
{
    LOCK(pwallet->cs_wallet);
    try {
        ...
        RGBTransitionRecord record;
        ssValue >> record;

        if (!pwallet->LoadRGBTransition(key.first, key.second, record)) {
            strErr = "Error reading wallet database: CWallet::LoadRGBTransition failed";
            return false;
        }
    } catch (const std::exception& e) {
        if (strErr.empty()) {
...
bool LoadRGBTransitionProof(CWallet* pwallet, DataStream& ssKey, DataStream& ssValue, std::string& strErr)
{
    LOCK(pwallet->cs_wallet);
    try {
        std::pair<uint256, uint256> key;
        ...
        RGBTransitionProofRecord record;
        ssValue >> record;

        if (!pwallet->LoadRGBTransitionProof(key.first, key.second, record)) {
            strErr = "Error reading wallet database: CWallet::LoadRGBTransitionProof failed";
            return false;
        }
    } catch (const std::exception& e) {
        if (strErr.empty()) {
...
bool LoadEUTXOState(CWallet* pwallet, DataStream& ssKey, DataStream& ssValue, std::string& strErr)
{
    LOCK(pwallet->cs_wallet);
    try {
        COutPoint outpoint;
        ...
        if (!pwallet->LoadEUTXOState(outpoint, record)) {
            strErr = "Error reading wallet database: CWallet::LoadEUTXOState failed";
            return false;
        }
    } catch (const std::exception& e) {
        if (strErr.empty()) {
...
static DBErrors LoadQuantumWalletRecords(CWallet* pwallet, DatabaseBatch& batch) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    AssertLockHeld(pwallet->cs_wallet);
    DBErrors result = DBErrors::LOAD_OK;

    LoadResult ckey_res = LoadRecords(pwallet, batch, DBKeys::CRYPTED_QUANTUM_KEY,
        [] (CWallet* pwallet, DataStream& key, CDataStream& value, std::string& err) {
        return LoadCryptedQuantumKey(pwallet, key, value, err) ? DBErrors::LOAD_OK : DBErrors::CORRUPT;
    });
    result = std::max(result, ckey_res.m_result);

    LoadResult key_res = LoadRecords(pwallet, batch, DBKeys::QUANTUM_KEY,
        [] (CWallet* pwallet, DataStream& key, CDataStream& value, std::string& err) {
        return LoadQuantumKey(pwallet, key, value, err) ? DBErrors::LOAD_OK : DBErrors::CORRUPT;
    });
    result = std::max(result, key_res.m_result);
    ...
    result = std::max(result, coldstake_res.m_result);

    int redelegation_scan_height{-1};
    if (batch.Read(DBKeys::QUANTUM_REDELEGATION_AUTO_SCAN, redelegation_scan_height)) {
        pwallet->m_redelegation_last_auto_scan_height = redelegation_scan_height;
    }

    LoadResult redelegation_attempt_res = LoadRecords(pwallet, batch, DBKeys::QUANTUM_REDELEGATION_LAST_ATTEMPT,
        [] (CWallet* pwallet, DataStream& key, CDataStream& value, std::string& err) {
        std::vector<unsigned char> witness_program;
        int height{0};
        try {
            ...
            err = "Error reading wallet database: malformed quantum redelegation attempt record";
            return DBErrors::CORRUPT;
        }
        if (witness_program.empty() || height < 0) {
            err = "Error reading wallet database: invalid quantum redelegation attempt record";
            return DBErrors::CORRUPT;
        }
        pwallet->m_redelegation_last_auto_attempt_height[witness_program] = height;
        return DBErrors::LOAD_OK;
    });
    result = std::max(result, redelegation_attempt_res.m_result);

    LoadResult redelegation_success_res = LoadRecords(pwallet, batch, DBKeys::QUANTUM_REDELEGATION_LAST_SUCCESS,
        [] (CWallet* pwallet, DataStream& key, CDataStream& value, std::string& err) {
        std::vector<unsigned char> witness_program;
        int height{0};
        try {
            ...
            err = "Error reading wallet database: malformed quantum redelegation success record";
            return DBErrors::CORRUPT;
        }
        if (witness_program.empty() || height < 0) {
            err = "Error reading wallet database: invalid quantum redelegation success record";
            return DBErrors::CORRUPT;
        }
        pwallet->m_redelegation_last_auto_success_height[witness_program] = height;
        return DBErrors::LOAD_OK;
    });
    result = std::max(result, redelegation_success_res.m_result);

    LoadResult redelegation_win_res = LoadRecords(pwallet, batch, DBKeys::QUANTUM_REDELEGATION_LAST_WIN,
        [] (CWallet* pwallet, DataStream& key, CDataStream& value, std::string& err) {
        std::vector<unsigned char> witness_program;
        int height{0};
        try {
            ...
            err = "Error reading wallet database: malformed quantum redelegation win-history record";
            return DBErrors::CORRUPT;
        }
        if (witness_program.empty() || height < 0) {
            err = "Error reading wallet database: invalid quantum redelegation win-history record";
            return DBErrors::CORRUPT;
        }
        pwallet->m_redelegation_last_win_height[witness_program] = height;
        return DBErrors::LOAD_OK;
    });
    result = std::max(result, redelegation_win_res.m_result);

    LoadResult rgb_contract_res = LoadRecords(pwallet, batch, DBKeys::RGB_CONTRACT,
        [] (CWallet* pwallet, DataStream& key, CDataStream& value, std::string& err) {
        return LoadRGBContract(pwallet, key, value, err) ? DBErrors::LOAD_OK : DBErrors::CORRUPT;
    });
    result = std::max(result, rgb_contract_res.m_result);

    LoadResult rgb_assignment_res = LoadRecords(pwallet, batch, DBKeys::RGB_ASSIGNMENT,
        [] (CWallet* pwallet, DataStream& key, CDataStream& value, std::string& err) {
        return LoadRGBAssignment(pwallet, key, value, err) ? DBErrors::LOAD_OK : DBErrors::CORRUPT;
    });
    result = std::max(result, rgb_assignment_res.m_result);

    LoadResult rgb_genesis_proof_res = LoadRecords(pwallet, batch, DBKeys::RGB_GENESIS_PROOF,
        [] (CWallet* pwallet, DataStream& key, CDataStream& value, std::string& err) {
        return LoadRGBGenesisProof(pwallet, key, value, err) ? DBErrors::LOAD_OK : DBErrors::CORRUPT;
    });
    result = std::max(result, rgb_genesis_proof_res.m_result);

    LoadResult rgb_transition_res = LoadRecords(pwallet, batch, DBKeys::RGB_TRANSITION,
        [] (CWallet* pwallet, DataStream& key, CDataStream& value, std::string& err) {
        return LoadRGBTransition(pwallet, key, value, err) ? DBErrors::LOAD_OK : DBErrors::CORRUPT;
    });
    result = std::max(result, rgb_transition_res.m_result);

    LoadResult rgb_transition_proof_res = LoadRecords(pwallet, batch, DBKeys::RGB_TRANSITION_PROOF,
        [] (CWallet* pwallet, DataStream& key, CDataStream& value, std::string& err) {
        return LoadRGBTransitionProof(pwallet, key, value, err) ? DBErrors::LOAD_OK : DBErrors::CORRUPT;
    });
    result = std::max(result, rgb_transition_proof_res.m_result);

    LoadResult eutxo_state_res = LoadRecords(pwallet, batch, DBKeys::EUTXO_STATE,
        [] (CWallet* pwallet, DataStream& key, CDataStream& value, std::string& err) {
        return LoadEUTXOState(pwallet, key, value, err) ? DBErrors::LOAD_OK : DBErrors::CORRUPT;
    });
    result = std::max(result, eutxo_state_res.m_result);

    if (result <= DBErrors::NONCRITICAL_ERROR && (key_res.m_records + ckey_res.m_records) > 0 && !pwallet->IsWalletFlagSet(WALLET_FLAG_QUANTUM_KEYS)) {
        pwallet->WalletLogPrintf("Upgrading wallet flags for existing Quantum Quasar wallet keys.\n");
        pwallet->SetWalletFlag(WALLET_FLAG_QUANTUM_KEYS);
    }
...
DBErrors WalletBatch::LoadWallet(CWallet* pwallet)
{
    ...
    try {
        ...
        // Load Quantum Quasar wallet-managed ML-DSA migration keys. These are wallet-wide
        // private keys, not legacy ScriptPubKeyMan records.
        result = std::max(LoadQuantumWalletRecords(pwallet, *m_batch), result);
        ...
        if (result <= DBErrors::NONCRITICAL_ERROR) {
            if (!pwallet->mapMasterKeys.empty() && pwallet->HasPlaintextQuantumKeys()) {
                pwallet->WalletLogPrintf("Error reading wallet database: encrypted wallet contains plaintext Quantum Quasar private keys\n");
                return DBErrors::CORRUPT;
            }
            if (pwallet->mapMasterKeys.empty() && pwallet->HasCryptedQuantumKeys()) {
                pwallet->WalletLogPrintf("Error reading wallet database: unencrypted wallet contains encrypted Quantum Quasar private keys without a master key\n");
                return DBErrors::CORRUPT;
            }
        }
    } catch (...) {
