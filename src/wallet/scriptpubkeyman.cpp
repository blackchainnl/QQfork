...
std::unique_ptr<FlatSigningProvider> DescriptorScriptPubKeyMan::GetSigningProviderForScript(const CScript& script) const
{
    std::vector<valtype> solutions;
    const TxoutType which_type = Solver(script, solutions);
    if (which_type == TxoutType::PUBKEY) {
        return GetSigningProvider(GetScriptForDestination(PKHash(CPubKey(solutions[0]))), true);
    }
                                                ...
                                                unsigned int extra_verify_flags) const
{
    ...
    for (const auto& coin_pair : coins) {
        std::unique_ptr<FlatSigningProvider> coin_keys = GetSigningProviderForScript(coin_pair.second.out.scriptPubKey);
        if (!coin_keys) {
            continue;
        }
