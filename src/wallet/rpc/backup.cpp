...
RPCHelpMan importquantumkey()
{
    return RPCHelpMan{"importquantumkey",
                "\nAdds an ML-DSA private/public key pair for a wallet-backed Blackcoin migration address. Requires a new wallet backup.\n"
                "The public key is required because the current ML-DSA interface stores it explicitly and does not derive it from the private key.\n"
                "\nNote: This call can take over an hour to complete if rescan is true, during that time other RPC calls\n"
                "may report that the imported key exists but related transactions are still missing until rescan completes.\n",
                {
                    {"public_key", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The hex-encoded ML-DSA-44 public key"},
                    {"private_key", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The hex-encoded ML-DSA-44 private key"},
                    {"label", RPCArg::Type::STR, RPCArg::Default{""}, "An optional label"},
                    {"rescan", RPCArg::Type::BOOL, RPCArg::Default{true}, "Scan the chain and mempool for wallet transactions."},
                    {"timestamp", RPCArg::Type::NUM, RPCArg::Default{0}, "Timestamp in seconds since epoch for the key creation time. Use 0 to rescan from the genesis block."},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                ...
                RPCExamples{
                    HelpExampleCli("importquantumkey", "\"publickeyhex\" \"privatekeyhex\" \"migration\" false")
            + HelpExampleRpc("importquantumkey", "\"publickeyhex\", \"privatekeyhex\", \"migration\", false")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
    {
        std::shared_ptr<CWallet> const pwallet = GetWalletForJSONRPCRequest(request);
        ...
        const std::string public_key_hex = request.params[0].get_str();
        const std::string private_key_hex = request.params[1].get_str();
        if (!IsHex(public_key_hex)) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "public_key must be hex");
        }
        if (!IsHex(private_key_hex)) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "private_key must be hex");
        }

        std::vector<unsigned char> public_key = ParseHex(public_key_hex);
        std::vector<unsigned char> private_key_bytes = ParseHex(private_key_hex);
        ...
        const int64_t timestamp = request.params[4].isNull() ? TIMESTAMP_MIN : request.params[4].getInt<int64_t>();
        ...
        CKeyingMaterial private_key(private_key_bytes.begin(), private_key_bytes.end());
        if (!private_key_bytes.empty()) {
            memory_cleanse(private_key_bytes.data(), private_key_bytes.size());
        }
        ...
        if (rescan) {
            EnsureBlockDataFromTime(*pwallet, timestamp);
        }

        auto op_dest = pwallet->AddQuantumKey(public_key, private_key, label, timestamp);
        if (!op_dest) {
            throw JSONRPCError(RPC_WALLET_ERROR, util::ErrorString(op_dest).original);
        }

        if (rescan) {
            RescanWallet(*pwallet, reserver, timestamp);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    CHECK_NONFATAL(pwallet->chain().findBlock(last_block_hash, FoundBlock().time(nTimeBegin)));
    {
        ...
        std::vector<std::tuple<std::vector<unsigned char>, CKeyingMaterial, int64_t, std::string>> quantum_keys;
        ...
        while (file.good()) {
            ...
            if (vstr.size() < 2)
                continue;
            bool is_quantum_key = false;
            std::string quantum_public_key_hex;
            std::string quantum_label;
            for (unsigned int nStr = 2; nStr < vstr.size(); nStr++) {
                if (vstr[nStr].front() == '#')
                    break;
                if (vstr[nStr] == "quantumkey=1") {
                    is_quantum_key = true;
                } else if (vstr[nStr].substr(0, 11) == "public_key=") {
                    quantum_public_key_hex = vstr[nStr].substr(11);
                } else if (vstr[nStr].substr(0, 6) == "label=") {
                    quantum_label = DecodeDumpString(vstr[nStr].substr(6));
                }
            }
            if (is_quantum_key) {
                int64_t nTime = ParseISO8601DateTime(vstr[1]);
                if (!IsHex(vstr[0]) || !IsHex(quantum_public_key_hex)) {
                    fGood = false;
                    continue;
                }
                std::vector<unsigned char> private_key_bytes(ParseHex(vstr[0]));
                std::vector<unsigned char> public_key(ParseHex(quantum_public_key_hex));
                if (private_key_bytes.size() != ML_DSA::SECRETKEY_BYTES || public_key.size() != ML_DSA::PUBLICKEY_BYTES) {
                    if (!private_key_bytes.empty()) {
                        memory_cleanse(private_key_bytes.data(), private_key_bytes.size());
                    }
                    fGood = false;
                ...
                quantum_keys.emplace_back(std::move(public_key), std::move(private_key), nTime, std::move(quantum_label));
                continue;
            }
        ...
        double total = (double)(keys.size() + quantum_keys.size() + scripts.size());
        double progress = 0;
        ...
        for (const auto& key_tuple : quantum_keys) {
            pwallet->chain().showProgress("", std::max(50, std::min(75, (int)((progress / total) * 100) + 50)), false);
            const std::vector<unsigned char>& public_key = std::get<0>(key_tuple);
            const CKeyingMaterial& private_key = std::get<1>(key_tuple);
            int64_t time = std::get<2>(key_tuple);
            const std::string& label = std::get<3>(key_tuple);

            auto op_dest = pwallet->AddQuantumKey(public_key, private_key, label, time);
            if (!op_dest) {
                pwallet->WalletLogPrintf("Error importing Blackcoin key %s: %s\n", HexStr(public_key), util::ErrorString(op_dest).original);
                fGood = false;
                continue;
            }
            pwallet->WalletLogPrintf("Imported Blackcoin key %s...\n", EncodeDestination(*op_dest));
            progress++;
        }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (const QuantumKeyInfo& info : wallet.ListQuantumKeyInfos()) {
        ...
        bilingual_str error;
        if (!wallet.GetQuantumKey(info.witness_program, public_key, private_key, error)) {
            throw JSONRPCError(RPC_WALLET_ERROR, error.original);
        }
        std::vector<unsigned char> private_key_bytes(private_key.begin(), private_key.end());
        const auto* address_book_entry = wallet.FindAddressBookEntry(info.destination);
        const std::string label = address_book_entry ? EncodeDumpString(address_book_entry->GetLabel()) : "";
        const std::string create_time = FormatISO8601DateTime(info.creation_time > 0 ? info.creation_time : 0);
        file << strprintf("%s %s quantumkey=1 public_key=%s label=%s # addr=%s\n",
                          HexStr(private_key_bytes), create_time, HexStr(public_key), label, EncodeDestination(info.destination));
        if (!private_key_bytes.empty()) {
            memory_cleanse(private_key_bytes.data(), private_key_bytes.size());
        }
    }
