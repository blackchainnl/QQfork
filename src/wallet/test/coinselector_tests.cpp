...
// Copyright (c) 2017-2022 Quantum Quasar Developers
...
static COutput& bech32_coin(CoinsResult& available_coins, size_t index)
{
    return available_coins.coins.at(OutputType::BECH32).at(index);
}

static CAmount GetTestInputFee(CWallet& wallet, CFeeRate feerate)
{
    CoinsResult sizing_coins;
    add_coin(sizing_coins, wallet, 1 * CENT, feerate, 6 * 24, false, 0, true);
    return feerate.GetFee(bech32_coin(sizing_coins, 0).input_bytes);
}
...
static std::string SelectionAmounts(const SelectionResult& result)
{
...
BOOST_AUTO_TEST_CASE(bnb_search_test)
{
    ...
    auto update_fee_dependent_params = [&] {
...

    {
        ...
        bech32_coin(available_coins, 0).input_bytes = 40; // Make sure that it has a negative effective value. The next check should assert if this somehow got through. Otherwise it will fail
        ...
        bech32_coin(available_coins, 0).input_bytes = 40;
    ...
    }
    {
        ...
        CAmount input_fee = GetTestInputFee(*wallet, coin_selection_params_bnb.m_effective_feerate);
        ...
        BOOST_CHECK_MESSAGE(EquivalentResult(expected_result, *result11), "expected " << SelectionAmounts(expected_result) << " got " << SelectionAmounts(*result11) << " algo " << GetAlgorithmName(result11->GetAlgo()) << " waste " << result11->GetWaste());
        available_coins.Clear();

        // more coins should be selected when effective fee < long term fee
        coin_selection_params_bnb.m_effective_feerate = CFeeRate(50000);
        update_fee_dependent_params();

        // Add selectable outputs, increasing their raw amounts by their input fee to make the effective value equal to the raw amount
        input_fee = GetTestInputFee(*wallet, coin_selection_params_bnb.m_effective_feerate);
        add_coin(available_coins, *wallet, 10 * CENT + input_fee, coin_selection_params_bnb.m_effective_feerate, 6 * 24, false, 0, true);
        add_coin(available_coins, *wallet, 9 * CENT + input_fee, coin_selection_params_bnb.m_effective_feerate, 6 * 24, false, 0, true);
        add_coin(available_coins, *wallet, 1 * CENT + input_fee, coin_selection_params_bnb.m_effective_feerate, 6 * 24, false, 0, true);

        const auto result12 = SelectCoins(*wallet, available_coins, /*pre_set_inputs=*/{}, 10 * CENT, coin_control, coin_selection_params_bnb);
        BOOST_CHECK_MESSAGE(result12->GetInputSet().size() > 1, "got " << SelectionAmounts(*result12) << " algo " << GetAlgorithmName(result12->GetAlgo()) << " waste " << result12->GetWaste());
        available_coins.Clear();
        ...
        BOOST_CHECK_MESSAGE(EquivalentResult(expected_result, *result13), "expected " << SelectionAmounts(expected_result) << " got " << SelectionAmounts(*result13) << " algo " << GetAlgorithmName(result13->GetAlgo()) << " waste " << result13->GetWaste());
    }
