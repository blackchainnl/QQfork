...
private:
    ...
    void addPackageTxs(const CTxMemPool& mempool, int& nPackagesSelected, int& nDescendantsUpdated, uint32_t nTime) EXCLUSIVE_LOCKS_REQUIRED(::cs_main, mempool.cs);

    // helper functions for addPackageTxs()
    /** Remove confirmed (inBlock) entries from given set */
    void onlyUnconfirmed(CTxMemPool::setEntries& testSet);
    size_t MaxActiveBlockWeight() const EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    int64_t MaxActiveBlockSigOpsCost() const EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    /** Test if a new package would "fit" in the block */
    bool TestPackage(uint64_t packageSize, int64_t packageSigOpsCost) const EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
