    ...
    bool fAddToMempool)
{
    ...
    if (m_chainman.GetConsensus().IsDemurrageActive(next_block_height, next_block_mtp)) {
        ...
        for (const CTxMemPoolEntryRef& entry : m_mempool->entryAll()) {
            ...
            if (!Consensus::CheckDemurrageAttestations(tx, attestation_view, m_chainman.GetConsensus(),
                                                       next_block_height, next_block_mtp,
                                                       attested_keys, tx_attestation_count,
                                                       reject_reason)) {
                invalid_lifecycle_transactions.insert(tx.GetHash());
            }
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!demurrage_attestations.empty()) {
        ...
        if (!Consensus::CheckDemurrageAttestations(tx, m_view, consensus, next_height, next_block_mtp,
                                                   attested_keys, transaction_attestation_count,
                                                   demurrage_reject_reason)) {
            return state.Invalid(demurrage_reject_reason.rfind("local-demurrage-", 0) == 0
    ...
    int64_t nSigOpsCost = GetTransactionSigOpCost(tx, m_view, script_verify_flags);
    ...
    */
    ...
    if (!ancestors) {
        ...
        CTxMemPool::Limits cpfp_carve_out_limits = m_pool.m_limits;
        cpfp_carve_out_limits.ancestor_count = 2;
        cpfp_carve_out_limits.descendant_count += 1;
        cpfp_carve_out_limits.descendant_size_vbytes += EXTRA_DESCENDANT_TX_SIZE_LIMIT;
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    const unsigned int scriptVerifyFlags{GetNextBlockPolicyScriptFlags(m_active_chainstate.m_chain.Tip(), m_active_chainstate.m_chainman)};
...
PackageMempoolAcceptResult MemPoolAccept::AcceptMultipleTransactions(const std::vector<CTransactionRef>& txns, ATMPArgs& args)
{
    ...
    // more across the complete atomic package.
    {
        ...
        if (consensus.IsDemurrageActive(next_height, next_block_mtp)) {
            ...
            for (Workspace& ws : workspaces) {
                std::string demurrage_reject_reason;
                if (!Consensus::CheckDemurrageAttestations(*ws.m_ptx, m_view, consensus, next_height,
                                                           next_block_mtp, attested_keys,
                                                           package_attestation_count,
                                                           demurrage_reject_reason)) {
                    ws.m_state.Invalid(demurrage_reject_reason.rfind("local-demurrage-", 0) == 0
                                           ? TxValidationResult::TX_MEMPOOL_POLICY
                                           : TxValidationResult::TX_CONSENSUS,
                                       demurrage_reject_reason);
                    package_state.Invalid(PackageValidationResult::PCKG_TX, "transaction failed");
...
CAmount GetColdStakeProofOfStakeSubsidy()
{
    return GetProofOfStakeSubsidy() / 2;
}

CAmount GetProofOfStakeSubsidyForCoinstake(bool fColdStake)
{
    return fColdStake ? GetColdStakeProofOfStakeSubsidy() : GetProofOfStakeSubsidy();
}
                       ...
                       std::vector<CScriptCheck>* pvChecks)
{
    ...
    if (spends_quantum_migration_output) {
        for (unsigned int i = 0; i < tx.vout.size(); ++i) {
            if (IsQuantumMigrationSpendAllowedOutput(tx, i)) continue;
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "quantum-output-downgrade", strprintf("Quantum-protected spends may only create quantum outputs (output %u)", i));
        }
    }
    ...
    // flags. The non-script Quantum Quasar input/output consensus rules
    // above must run before this cache can short-circuit script execution.
...
struct TieredTransitionKey {
    ...
    friend bool operator<(const TieredTransitionKey& a, const TieredTransitionKey& b)
    {
        return std::tie(a.cold_stake, a.primary, a.secondary, a.unbonding_blocks) <
               std::tie(b.cold_stake, b.primary, b.secondary, b.unbonding_blocks);
    }
};

struct TieredTransitionRequirement {
    CAmount required_value{0};
    uint32_t min_unlock_height{0};
};

static CScript GetScriptForQuantumWitnessProgram(unsigned int witness_version, const std::vector<unsigned char>& witness_program)
{
    return CScript() << CScript::EncodeOP_N(witness_version) << witness_program;
}

static bool AddTieredAmount(CAmount& total, CAmount amount)
{
    if (!MoneyRange(amount) || total > MAX_MONEY - amount) return false;
    total += amount;
    return MoneyRange(total);
}

static bool ExtractTieredTransitionKey(const CScript& prev_script, const CTxIn& input, const QuantumStakeTierProgram& tier, TieredTransitionKey& key, std::string& reject_reason)
{
    key = {};
    key.cold_stake = tier.cold_stake;
    key.unbonding_blocks = tier.unbonding_blocks;

    const CScriptWitness& witness = input.scriptWitness;
    if (!tier.cold_stake) {
        if (witness.stack.size() != 2 || witness.stack[1].size() != ML_DSA::PUBLICKEY_BYTES) {
            reject_reason = "bad-stake-tier-transition";
            return false;
        }
        uint256 pubkey_hash;
        CSHA256().Write(witness.stack[1].data(), witness.stack[1].size()).Finalize(pubkey_hash.begin());
        if (pubkey_hash != tier.commitment) {
            reject_reason = "bad-stake-tier-transition";
            return false;
        }
        key.primary = pubkey_hash;
        return true;
    }

    if (witness.stack.size() != 4 ||
        witness.stack[1].size() != ML_DSA::PUBLICKEY_BYTES ||
        witness.stack[2].size() != uint256::size() ||
        !witness.stack[3].empty()) {
        reject_reason = "bad-stake-tier-transition";
        return false;
    }

    uint256 owner_hash;
    CSHA256().Write(witness.stack[1].data(), witness.stack[1].size()).Finalize(owner_hash.begin());
    uint256 staker_hash;
    std::copy(witness.stack[2].begin(), witness.stack[2].end(), staker_hash.begin());
    const std::vector<unsigned char> expected_program = QuantumTieredColdStakeProgramForKeyHashes(
        staker_hash, owner_hash, tier.state, tier.unbonding_blocks, tier.unlock_height);
    if (GetScriptForQuantumWitnessProgram(QUANTUM_COLDSTAKE_WITNESS_VERSION, expected_program) != prev_script) {
        reject_reason = "bad-stake-tier-transition";
        return false;
    }

    key.primary = staker_hash;
    key.secondary = owner_hash;
    return true;
}

static bool AddTieredTransitionRequirement(std::map<TieredTransitionKey, TieredTransitionRequirement>& requirements,
                                           const TieredTransitionKey& key,
                                           uint32_t min_unlock_height,
                                           CAmount amount,
                                           std::string& reject_reason)
{
    TieredTransitionRequirement& requirement = requirements[key];
    requirement.min_unlock_height = std::max(requirement.min_unlock_height, min_unlock_height);
    if (!AddTieredAmount(requirement.required_value, amount)) {
        reject_reason = "bad-stake-tier-transition";
        return false;
    }
    return true;
...
static bool TieredOutputMatchesRequirement(const CTxOut& output, const TieredTransitionKey& key, const TieredTransitionRequirement& requirement)
{
    const std::optional<QuantumStakeTierProgram> output_tier = GetQuantumStakeTierProgram(output.scriptPubKey);
    if (!output_tier || !output_tier->tiered || !output_tier->IsUnbonding()) return false;
    if (output_tier->cold_stake != key.cold_stake) return false;
    if (output_tier->unbonding_blocks != key.unbonding_blocks) return false;
    if (output_tier->unlock_height < requirement.min_unlock_height) return false;

    if (!key.cold_stake) {
        return output_tier->commitment == key.primary;
    }

    const std::vector<unsigned char> expected_program = QuantumTieredColdStakeProgramForKeyHashes(
        key.primary, key.secondary, QUANTUM_TIERED_STATE_UNBONDING, key.unbonding_blocks, output_tier->unlock_height);
    return GetScriptForQuantumWitnessProgram(QUANTUM_COLDSTAKE_WITNESS_VERSION, expected_program) == output.scriptPubKey;
}

static bool GetDemurrageAwarePrincipal(const Coin& coin,
                                       ...
                                       std::string& reject_reason)
{
    ...
    if (!MoneyRange(principal)) {
        reject_reason = "bad-stake-tier-transition";
        return false;
    }

    if (!consensus_params || !consensus_params->IsDemurrageActive(spend_height, spend_time)) {
        return true;
    }
    ...
    if (!MoneyRange(eval.effective_value)) {
        reject_reason = "bad-stake-tier-transition";
        return false;
    }
...
bool CheckTieredStakePrincipalCovenant(const CTransaction& tx, const CCoinsViewCache& inputs, unsigned int flags, int spend_height, std::string& reject_reason, const Consensus::Params* consensus_params, int64_t spend_time)
{
    if (!(flags & SCRIPT_VERIFY_QUANTUM_STAKE_TIERS)) return true;

    std::map<CScript, CAmount> exact_script_required;
    std::map<TieredTransitionKey, TieredTransitionRequirement> transition_required;

    for (const CTxIn& txin : tx.vin) {
        const Coin& coin = inputs.AccessCoin(txin.prevout);
        const std::optional<QuantumStakeTierProgram> tier = GetQuantumStakeTierProgram(coin.out.scriptPubKey);
        if (!tier || !tier->tiered) continue;

        CAmount principal{0};
        if (!GetDemurrageAwarePrincipal(coin, inputs, consensus_params, spend_height, spend_time, principal, reject_reason)) return false;

        if (tx.IsCoinStake()) {
            if (!AddTieredAmount(exact_script_required[coin.out.scriptPubKey], principal)) {
                reject_reason = "bad-stake-tier-transition";
                return false;
            }
            continue;
        ...
        if (tier->IsUnbonding() && spend_height >= 0 && tier->unlock_height <= static_cast<uint32_t>(spend_height)) {
            continue;
        }

        TieredTransitionKey key;
        if (!ExtractTieredTransitionKey(coin.out.scriptPubKey, txin, *tier, key, reject_reason)) return false;

        uint32_t min_unlock_height = tier->unlock_height;
        if (tier->IsBonded()) {
            const uint64_t requested_unlock = static_cast<uint64_t>(std::max(spend_height, 0)) + tier->unbonding_blocks;
            if (requested_unlock > std::numeric_limits<uint32_t>::max()) {
                reject_reason = "bad-stake-tier-transition";
                return false;
            }
            min_unlock_height = static_cast<uint32_t>(requested_unlock);
        } else if (!tier->IsUnbonding()) {
            reject_reason = "bad-stake-tier-transition";
            return false;
        }

        if (!AddTieredTransitionRequirement(transition_required, key, min_unlock_height, principal, reject_reason)) {
            return false;
        }
    }

    for (const auto& [script, required_value] : exact_script_required) {
        CAmount returned_value{0};
        for (const CTxOut& output : tx.vout) {
            if (output.scriptPubKey != script) continue;
            if (!AddTieredAmount(returned_value, output.nValue)) {
                reject_reason = "bad-stake-tier-covenant";
                return false;
            }
        }
        if (returned_value < required_value) {
            reject_reason = "bad-stake-tier-covenant";
            return false;
        }
    }

    for (const auto& [key, requirement] : transition_required) {
        CAmount returned_value{0};
        for (const CTxOut& output : tx.vout) {
            if (!TieredOutputMatchesRequirement(output, key, requirement)) continue;
            if (!AddTieredAmount(returned_value, output.nValue)) {
                reject_reason = "bad-stake-tier-transition";
                return false;
            }
        }
        if (returned_value < requirement.required_value) {
            reject_reason = "bad-stake-tier-transition";
            return false;
        }
    }
...
static bool CheckQuantumMigrationSpendIsolation(const CTransaction& tx, const CCoinsViewCache& inputs, unsigned int flags, int spend_height, const Consensus::Params& consensus_params, int64_t spend_time, std::string& reject_reason)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (!coin.IsCoinBase() && consensus_params.UsesHeightLifecycle() &&
            coin.nHeight > static_cast<uint32_t>(consensus_params.nGoldRushEndHeight)) {
    ...
    if (spends_quantum_migration_output) {
        for (unsigned int i = 0; i < tx.vout.size(); ++i) {
            if (IsQuantumMigrationSpendAllowedOutput(tx, i)) continue;
            reject_reason = "quantum-output-downgrade";
            return false;
        }
    }
...
/**
 * Cold-staking covenant (Quantum Quasar QCS witness / ML-DSA delegation).
 *
 * Only native Quantum Cold-Stake outputs are privileged cold-staking
 * principal. The previous generic OP_ISCOINSTAKE/P2WSH shape is deliberately
 * not recognized here: it was not quantum-only and had wrapper/prelude evasion
 * paths. A QCS coinstake must return at least the spent principal to the exact
 * same QCS scriptPubKey. Emission outputs earmarked by other consensus rules do
 * not count as returned principal.
 */
bool IsColdStakeInput(const CTxOut& prevout, const CTxIn& input)
{
    (void)input;
    return IsQuantumColdStakeScript(prevout.scriptPubKey);
}

bool HasColdStakeInputs(const CTransaction& coinstake, const CTxUndo& coinstake_undo)
{
    const size_t n = std::min(coinstake.vin.size(), coinstake_undo.vprevout.size());
    for (size_t j = 0; j < n; ++j) {
        if (IsColdStakeInput(coinstake_undo.vprevout[j].out, coinstake.vin[j])) {
            return true;
        }
    }
    return false;
...
namespace {
struct StakeRewardContributor {
    size_t input_index{0};
    CScript return_script;
    CAmount principal{0};
    arith_uint256 weight;
    CAmount gross_reward{0};
    CAmount net_reward{0};
    CAmount deduction{0};
    bool delegated{false};
};

bool AddChecked(CAmount a, CAmount b, CAmount& out)
{
    if (a < 0 || b < 0 || a > MAX_MONEY - b) return false;
    out = a + b;
    return MoneyRange(out);
}

bool AddToScriptAmount(std::map<CScript, CAmount>& amounts, const CScript& script, CAmount amount)
{
    if (amount < 0 || !MoneyRange(amount)) return false;
    CAmount updated{0};
    if (!AddChecked(amounts[script], amount, updated)) return false;
    amounts[script] = updated;
    return true;
}

bool IsColdStakeStakerBranchSelector(const std::vector<unsigned char>& selector)
{
    return selector.size() == 1 && selector[0] == 1;
}

bool DeriveOperatorRewardScript(const CTxOut& prevout, const CTxIn& input, CScript& operator_script)
{
    if (!IsQuantumColdStakeScript(prevout.scriptPubKey)) return false;

    const auto& stack = input.scriptWitness.stack;
    if (stack.size() != 4) return false;
    if (!IsColdStakeStakerBranchSelector(stack[3])) return false;

    const std::vector<unsigned char>& staker_pubkey = stack[1];
    if (staker_pubkey.size() != ML_DSA::PUBLICKEY_BYTES) return false;
    if (stack[2].size() != uint256::size()) return false;

    operator_script = GetScriptForDestination(WitnessUnknown{
        QUANTUM_MIGRATION_WITNESS_VERSION,
        QuantumMigrationProgramForPubkey(staker_pubkey)});
    return !operator_script.empty();
}

CAmount ArithToMoney(const arith_uint256& amount, bool& ok)
{
    if (amount > arith_uint256(static_cast<uint64_t>(MAX_MONEY))) {
        ok = false;
        return 0;
    }
    const CAmount value = static_cast<CAmount>(amount.GetLow64());
    if (!MoneyRange(value)) {
        ok = false;
        return 0;
    }
...
bool ComputeStakeRewardSplit(const CTransaction& coinstake, const CTxUndo& coinstake_undo, CAmount block_fees, int candidate_height, bool stake_tiers_active, StakeRewardSplit& split, std::string& reject_reason, const std::vector<CAmount>* effective_principals)
{
    split = StakeRewardSplit{};
    split.base_pie = GetProofOfStakeSubsidy();
    split.fees = block_fees;

    if (!coinstake.IsCoinStake() || coinstake.vin.empty() || coinstake.vin.size() != coinstake_undo.vprevout.size()) {
        reject_reason = "bad-stake-reward-split";
        return false;
    }
    ...
    if (block_fees < 0 || !MoneyRange(block_fees)) {
        reject_reason = "bad-stake-reward-split";
        return false;
    }
    if (!MoneyRange(split.base_pie)) {
        reject_reason = "bad-stake-reward-split";
        return false;
    }
    if (!AddChecked(split.base_pie, block_fees, split.total_reward)) {
        reject_reason = "bad-stake-reward-split";
        return false;
    }

    std::vector<StakeRewardContributor> contributors;
    contributors.reserve(coinstake.vin.size());
    arith_uint256 total_weight;
    size_t largest_weight_index{0};
    bool have_largest{false};
    std::optional<CScript> operator_script;

    for (size_t i = 0; i < coinstake.vin.size(); ++i) {
        const CTxOut& prevout = coinstake_undo.vprevout[i].out;
        const CAmount principal = effective_principals ? (*effective_principals)[i] : prevout.nValue;
        if (principal <= 0 || !MoneyRange(principal)) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }
        ...
        const StakeWeightContext context = GetStakeWeightContext(
            ...
            candidate_height,
            stake_tiers_active);
        const arith_uint256 weight = EffectiveStakeWeight(principal, context.multiplier_ppm10k, context.pool_ppm10k);
        if (weight == arith_uint256(0)) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }

        StakeRewardContributor contributor;
        contributor.input_index = i;
        contributor.return_script = prevout.scriptPubKey;
        contributor.principal = principal;
        contributor.weight = weight;
        contributor.delegated = IsColdStakeInput(prevout, coinstake.vin[i]);

        if (contributor.delegated) {
            CScript derived_operator_script;
            if (!DeriveOperatorRewardScript(prevout, coinstake.vin[i], derived_operator_script)) {
                reject_reason = "bad-stake-reward-split";
                return false;
            }
            if (!operator_script) {
                operator_script = derived_operator_script;
            } else if (*operator_script != derived_operator_script) {
                reject_reason = "bad-stake-reward-split";
                return false;
            }
        }

        if (!have_largest || contributor.weight > contributors[largest_weight_index].weight) {
            largest_weight_index = contributors.size();
            have_largest = true;
        }
    ...
    if (total_weight == arith_uint256(0) || contributors.empty()) {
        reject_reason = "bad-stake-reward-split";
        return false;
    }
    ...
    bool ok{true};
    CAmount gross_sum{0};
    for (StakeRewardContributor& contributor : contributors) {
        const arith_uint256 gross = (arith_uint256(static_cast<uint64_t>(split.base_pie)) * contributor.weight) / total_weight;
        contributor.gross_reward = ArithToMoney(gross, ok);
        contributor.net_reward = contributor.gross_reward;
        if (!ok || !AddChecked(gross_sum, contributor.gross_reward, gross_sum)) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }
        if (contributor.delegated && !AddChecked(split.delegated_gross_reward, contributor.gross_reward, split.delegated_gross_reward)) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }
    }

    if (gross_sum > split.base_pie) {
        reject_reason = "bad-stake-reward-split";
        return false;
    }
    contributors[largest_weight_index].gross_reward += split.base_pie - gross_sum;
    contributors[largest_weight_index].net_reward += split.base_pie - gross_sum;
    if (contributors[largest_weight_index].delegated && !AddChecked(split.delegated_gross_reward, split.base_pie - gross_sum, split.delegated_gross_reward)) {
        reject_reason = "bad-stake-reward-split";
        return false;
    }

    if (split.delegated_gross_reward > 0) {
        const CAmount operator_floor = (split.delegated_gross_reward * 5) / 100;
        split.operator_comp = std::max(operator_floor, block_fees);
        if (!MoneyRange(split.operator_comp) || !operator_script) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }

        const CAmount shortfall = split.operator_comp > block_fees ? split.operator_comp - block_fees : 0;
        if (shortfall > split.delegated_gross_reward) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }

        CAmount deducted{0};
        std::optional<size_t> largest_delegated_index;
        for (size_t i = 0; i < contributors.size(); ++i) {
            StakeRewardContributor& contributor = contributors[i];
            if (!contributor.delegated) continue;
            const arith_uint256 deduction = (arith_uint256(static_cast<uint64_t>(shortfall)) * arith_uint256(static_cast<uint64_t>(contributor.gross_reward))) /
                                            arith_uint256(static_cast<uint64_t>(split.delegated_gross_reward));
            contributor.deduction = ArithToMoney(deduction, ok);
            if (!ok || contributor.deduction > contributor.net_reward || !AddChecked(deducted, contributor.deduction, deducted)) {
                reject_reason = "bad-stake-reward-split";
                return false;
            }
            contributor.net_reward -= contributor.deduction;

            if (!largest_delegated_index ||
                contributor.gross_reward > contributors[*largest_delegated_index].gross_reward) {
                largest_delegated_index = i;
            }
        }

        if (deducted > shortfall || !largest_delegated_index) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }
        const CAmount deduction_remainder = shortfall - deducted;
        StakeRewardContributor& largest_delegated = contributors[*largest_delegated_index];
        if (deduction_remainder > largest_delegated.net_reward) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }
        largest_delegated.net_reward -= deduction_remainder;

        if (split.operator_comp > 0 && !AddToScriptAmount(split.required_outputs, *operator_script, split.operator_comp)) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }
    } else if (block_fees > 0) {
        if (!AddChecked(contributors[largest_weight_index].net_reward, block_fees, contributors[largest_weight_index].net_reward)) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }
    }

    for (const StakeRewardContributor& contributor : contributors) {
        CAmount required{0};
        if (!AddChecked(contributor.principal, contributor.net_reward, required) ||
            !AddToScriptAmount(split.required_outputs, contributor.return_script, required)) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }
    }
...
bool CheckStakeRewardSplit(const CTransaction& coinstake, const CTxUndo& coinstake_undo, CAmount block_fees, int candidate_height, bool stake_tiers_active, StakeRewardSplit& split, std::string& reject_reason, const std::vector<CAmount>* effective_principals)
{
    if (!ComputeStakeRewardSplit(coinstake, coinstake_undo, block_fees, candidate_height, stake_tiers_active, split, reject_reason, effective_principals)) {
        return false;
    }

    std::map<CScript, CAmount> actual_outputs;
    for (const CTxOut& out : coinstake.vout) {
        if (!MoneyRange(out.nValue)) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }
        if (out.nValue == 0) continue;
        if (!split.required_outputs.count(out.scriptPubKey)) {
            reject_reason = "bad-stake-reward-split-output";
            return false;
        }
        if (!AddToScriptAmount(actual_outputs, out.scriptPubKey, out.nValue)) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }
    }

    if (actual_outputs.size() != split.required_outputs.size()) {
        reject_reason = "bad-stake-reward-split";
        return false;
    }
    for (const auto& [script, required] : split.required_outputs) {
        const auto it = actual_outputs.find(script);
        if (it == actual_outputs.end() || it->second != required) {
            reject_reason = "bad-stake-reward-split";
            return false;
        }
    }
...
static bool GetDemurrageAdjustedInputPrincipals(const CTransaction& tx,
                                                const CCoinsViewCache& inputs,
                                                const Consensus::Params& params,
                                                int spend_height,
                                                int64_t spend_time,
                                                std::vector<CAmount>& effective_principals,
                                                CAmount& total_value_in,
                                                std::string& reject_reason)
{
    effective_principals.clear();
    effective_principals.reserve(tx.vin.size());
    total_value_in = 0;
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (!MoneyRange(eval.effective_value) || !AddChecked(total_value_in, eval.effective_value, total_value_in)) {
            reject_reason = "bad-txns-inputvalues-outofrange";
            return false;
        }
        effective_principals.push_back(eval.effective_value);
    }
    return true;
}
                            ...
                            const std::vector<CAmount>* effective_principals)
{
    ...
    std::map<CScript, CAmount> returned; // value returned to each cold-staking script
    for (const CTxOut& out : coinstake.vout) {
        if (required.find(out.scriptPubKey) == required.end()) continue;
        if (!MoneyRange(out.nValue)) { reject_reason = "bad-coldstake-covenant"; return false; }
        CAmount& got = returned[out.scriptPubKey];
        if (got > MAX_MONEY - out.nValue) { reject_reason = "bad-coldstake-covenant"; return false; }
        got += out.nValue;
    }
    for (const auto& [spk, req] : required) {
        CAmount available = returned[spk];
        const auto shadow_it = shadow_direct_payouts.find(spk);
        if (shadow_it != shadow_direct_payouts.end()) {
            if (!MoneyRange(shadow_it->second) || available < shadow_it->second) {
                reject_reason = "bad-coldstake-covenant";
                return false;
            }
...
static unsigned int ApplyNextBlockV4ScriptFlags(unsigned int flags, const CBlockIndex* active_tip, const Consensus::Params& consensusparams)
{
...
bool Chainstate::ConnectBlock(const CBlock& block, BlockValidationState& state, CBlockIndex* pindex,
                               CCoinsViewCache& view, bool fJustCheck, bool fCheckBlockSig)
{
    AssertLockHeld(cs_main);
    assert(pindex);
    ...
    // Precomputed transaction data pointers must not be invalidated until
    // after `control` has run the script checks (potentially in multiple
    // threads). Declare txsdata first so `control` is destroyed first, and
    ...
    std::vector<CAmount> coinstake_effective_principals;
    int nInputs = 0;
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        int64_t maxBlockSigOpsCost = expanded_block_limits ? V4_MAX_BLOCK_SIGOPS_COST : MAX_BLOCK_SIGOPS_COST;
        ...
        if (!tx.IsCoinBase())
        {
            if (tx.IsCoinStake()) {
                CAmount value_in{0};
                if (params.GetConsensus().IsDemurrageActive(pindex->nHeight, block_mtp)) {
                    std::string demurrage_reward_reject_reason;
                    if (!GetDemurrageAdjustedInputPrincipals(tx, view, params.GetConsensus(), pindex->nHeight, block_mtp, coinstake_effective_principals, value_in, demurrage_reward_reject_reason)) {
                        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, demurrage_reward_reject_reason);
                    }
                } else {
                    coinstake_effective_principals.clear();
                    value_in = view.GetValueIn(tx);
                }
    ...
    if (block.IsProofOfStake() && params.GetConsensus().IsProtocolV3(block.GetBlockTime())) {
        if (params.GetConsensus().IsStakeRewardSplitActive(pindex->nHeight)) {
            if (blockundo.vtxundo.empty()) {
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-stake-reward-split");
            }
            ...
            if (nActualStakeReward != split.total_reward) {
                LogPrintf("ERROR: ConnectBlock(): coinstake reward mismatch under stake-reward split (actual=%d vs expected=%d)\n", nActualStakeReward, split.total_reward);
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cs-amount");
            }
        } else {
            CAmount blockReward = nFees + GetProofOfStakeSubsidyForCoinstake(coinstake_spends_cold_stake);
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, Chainstate& chainstate, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Absolute size limits. Activation-specific limits are enforced in
    // ContextualCheckBlock() when previous-block MTP is available.
    if (block.vtx.empty() ||
        block.vtx.size() * WITNESS_SCALE_FACTOR > V4_MAX_BLOCK_WEIGHT ||
        ::GetSerializeSize(TX_NO_WITNESS(block)) * WITNESS_SCALE_FACTOR > V4_MAX_BLOCK_WEIGHT) {
    ...
    if (nSigOps * WITNESS_SCALE_FACTOR > V4_MAX_BLOCK_SIGOPS_COST)
...
{
    return std::all_of(headers.cbegin(), headers.cend(), [&](const CBlockHeader& header) {
        if (header.nFlags & CBlockIndex::BLOCK_PROOF_OF_STAKE) {
            return true;
        }
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked, bool fOldClient)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        ...
        // Qtum
        // Check for the checkpoint. Assumeutxo background validation is a
        // special case: after a snapshot is active, the node still has to
        // receive historical headers/blocks that are ancestors of the snapshot
        // base so the background chainstate can validate the snapshot.
        bool assumeutxo_background_header = false;
        if (BackgroundSyncInProgress()) {
            const CBlockIndex* snapshot_base = GetSnapshotBaseBlock();
            const auto mi_prev = m_blockman.m_block_index.find(block.hashPrevBlock);
            if (snapshot_base && mi_prev != m_blockman.m_block_index.end()) {
                const int nHeight = mi_prev->second.nHeight + 1;
                const CBlockIndex* snapshot_ancestor = snapshot_base->GetAncestor(nHeight);
                assumeutxo_background_header = snapshot_ancestor &&
                    snapshot_ancestor->GetBlockHash() == hash;
            }
        }
        if (!assumeutxo_background_header && chainstate.m_chain.Tip() && block.hashPrevBlock != chainstate.m_chain.Tip()->GetBlockHash())
        {
                       ...
                       bool fCheckBlockSig)
{
    ...
    if (!CheckBlock(block, state, chainparams.GetConsensus(), chainstate, fCheckPOW, fCheckMerkleRoot, fCheckBlockSig))
        return error("%s: Consensus::CheckBlock: %s", __func__, state.ToString());
    if (!ContextualCheckBlock(block, state, chainstate.m_chainman, pindexPrev))
        return error("%s: Consensus::ContextualCheckBlock: %s", __func__, state.ToString());
    if (!chainstate.ConnectBlock(block, state, &indexDummy, viewNew, true, fCheckBlockSig)) {
        return false;
    }
...
bool IsCanonicalBlockSignature(const std::shared_ptr<const CBlock>& pblock, bool checkLowS)
{
    ...
    std::vector<unsigned char> quantum_pubkey;
    if (ExtractQuantumBlockSigningPubKey(*pblock, quantum_pubkey)) {
        return pblock->vchBlockSig.size() == ML_DSA::SIGNATURE_BYTES;
    }
...
bool Chainstate::ReplayBlocks()
{
    ...
    // A partially flushed reorganization can leave positive synthetic payout
    // coins from the old branch even when their provenance marker was in a
    // different write batch. Those coins are not base-block outputs, so a
    // generic UTXO rollback cannot authenticate and remove every possible
    // PRE/POST mixture. Fail closed instead of risking cross-branch inflation;
    // a chainstate rebuild deterministically reconstructs only the active
    // branch. Straight-line interrupted flushes continue only when the exact
    // new-tip auxiliary commitment survives and authenticates below.
    if (pindexOld && pindexFork && pindexOld != pindexFork &&
        pindexOld->nHeight >= SHADOW_REWARD_START_HEIGHT) {
        return error("ReplayBlocks(): interrupted reorganization crossed Quantum Quasar shadow state; restart with -reindex-chainstate");
    }
    ...
    // A straight-line two-head recovery is produced after the atomic CoinsDB
    // batch (including auxiliary markers) was written but before the head
    // marker was finalized. Preserve that state only if its full v8 replay
    // commitment and both rolling inventories authenticate at the recovered
    // new tip. Deleting an already-current QQRSTATE here would turn every
    // otherwise recoverable interrupted flush at/after Gold Rush into a forced
    // reindex on the next line of startup.
    ...
    const bool auxiliary_state_current =
        pindexNew->nHeight < SHADOW_WHITELIST_HEIGHT ||
        (HasCurrentShadowReplayState(cache, m_chainman.GetConsensus(), pindexNew,
                                     &replay_block_reader) &&
         Consensus::DeepAuditDemurrageInventory(
             cache, pindexNew, m_chainman.GetConsensus()));
    if (pindexNew->nHeight >= SHADOW_WHITELIST_HEIGHT && !auxiliary_state_current) {
        return error("ReplayBlocks(): interrupted flush left Quantum Quasar auxiliary state inconsistent at recovered height %d; restart with -reindex-chainstate",
                     pindexNew->nHeight);
    }

    if (!cache.Flush()) return error("ReplayBlocks(): failed to flush recovered base chainstate");
    if (pindexNew->nHeight < SHADOW_WHITELIST_HEIGHT && !ReplayShadowBlocks()) {
...
{
    if (!MoneyRange(a) || !MoneyRange(b) || a > MAX_MONEY - b) return std::nullopt;
    return a + b;
}

static std::optional<CAmount> GetExpectedShadowObligation(const Consensus::Params& consensus, const CBlockIndex* pindexTip, int* first_active_height = nullptr)
{
    if (first_active_height) *first_active_height = 0;
    if (!pindexTip || pindexTip->nHeight < SHADOW_REWARD_START_HEIGHT) return CAmount{0};

    CAmount total{0};
    const int last_height = std::min(pindexTip->nHeight, SHADOW_REWARD_END_HEIGHT);
    for (int height = SHADOW_REWARD_START_HEIGHT; height <= last_height; ++height) {
        ...
        total = *next_total;
        if (first_active_height && *first_active_height == 0) *first_active_height = height;
    }
    return total;
}

static std::optional<CAmount> GetCurrentShadowObligation(const CCoinsViewCache& view, const CBlockIndex* pindexTip)
{
    const ShadowGoldRushInfo info = GetShadowGoldRushInfo(view, pindexTip);
    auto total = AddMoney(info.pow_amount, info.pos_amount);
    if (!total) return std::nullopt;
    total = AddMoney(*total, info.claimed_amount);
    if (!total) return std::nullopt;
    return *total;
}
...
bool Chainstate::ReplayShadowBlocks()
{
    ...
    // The v8 replay marker commits the exact tip, schedule, pool, signal,
    // whitelist, Gold Rush inventory, and demurrage inventory. Check it before
    // any historical obligation calculation so a normal pre-Final restart is
    // bounded and performs no reward-height walk or block read. After Final,
    // QQALIVE is reconciled in one constant-memory UTXO cursor pass.
    if (!whitelist_missing && HasCurrentShadowReplayState(
            cache, m_chainman.GetConsensus(), pindexTip, &replay_block_reader) &&
        Consensus::DeepAuditDemurrageInventory(
            cache, pindexTip, m_chainman.GetConsensus())) {
        return true;
    }
    ...
    // v30.1.1 builds QQRSTATE v8 and its rolling inventory commitments while
    // chainstate is connected. If any authenticated checkpoint is absent or
    // mismatched, do not enter the legacy O(all claims + full UTXO cursor)
    // purge/replay path: at mainnet scale that recovery can exhaust memory and
    // make every restart fail. Rebuild from an empty chainstate instead. A
    // pruned node that no longer has the pinned history must redownload it.
    ...
    std::multimap<uint256, FlatFilePos>* blocks_with_unknown_parent)
{
    ...
    int nLoaded = 0;
    try {
        BufferedFile blkdat{file_in, 2 * V4_MAX_BLOCK_SERIALIZED_SIZE, V4_MAX_BLOCK_SERIALIZED_SIZE + 8, SER_NETWORK};
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // read size
                blkdat >> nSize;
                if (nSize < 80 || nSize > V4_MAX_BLOCK_SERIALIZED_SIZE)
                    continue;
...
void ChainstateManager::CheckBlockIndex()
{
    ...
    while (pindex != nullptr) {
        ...
        // As we've never pruned, then HAVE_DATA should be equivalent to nTx > 0, except for
        // assumed-valid snapshot ancestors whose transaction counts are intentionally faked.
