...
public:
    ...
    /**
     * Post-Quantum ML-DSA-44 (Dilithium2):
     */
    static constexpr unsigned int MLDSA_SIZE             = 1312;
    static constexpr unsigned int MLDSA_SIGNATURE_SIZE   = 2420;

    static_assert(
        MLDSA_SIZE >= COMPRESSED_SIZE,
        "COMPRESSED_SIZE is larger than MLDSA_SIZE");

private:

    /**
     * Store the serialized ECDSA public key (<= 65 bytes). Its actual length can very
     * cheaply be computed from the first byte. NOTE: post-quantum ML-DSA public keys are
     * 1312 bytes and are NOT stored here -- they flow through the witness/script paths as
     * std::vector<unsigned char>. Keeping this buffer at 65 avoids ~20x memory bloat of
     * every CPubKey instance node-wide (coin/sig caches, mempool, net messages).
     */
