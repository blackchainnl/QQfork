...
enum : uint32_t {
    ...
    // Require the SIGHASH_FORKID flag in signatures
    ...
    // Enable OP_ISCOINSTAKE validation (Quantum Quasar V4)
    SCRIPT_VERIFY_ISCOINSTAKE = (1U << 22),

    // Allow V4-sized script stack elements in V4 blocks. This is a hard-fork
    // relaxation and must only be set by contextual block validation.
    SCRIPT_VERIFY_V4_LARGE_SCRIPT_ELEMENT = (1U << 23),

    // Validate Quantum Quasar ML-DSA witness-v16 spends.
    SCRIPT_VERIFY_QUANTUM_ML_DSA = (1U << 24),

    // Reject spends of non-quantum UTXOs after the final migration deadline.
    SCRIPT_VERIFY_LEGACY_ECDSA_LOCKOUT = (1U << 25),

    // Validate Quantum Quasar EUTXO witness-v15 datum/validator spends.
    SCRIPT_VERIFY_EUTXO = (1U << 26),

    // Validate Quantum Quasar ML-DSA cold-staking witness spends.
    SCRIPT_VERIFY_QUANTUM_COLDSTAKE = (1U << 27),

    // Validate tiered quantum staking witness-program extensions.
    SCRIPT_VERIFY_QUANTUM_STAKE_TIERS = (1U << 28),
...
static constexpr size_t QUANTUM_ML_DSA_WITNESS_SIGOPS = 16;
static constexpr size_t QUANTUM_COLDSTAKE_WITNESS_SIGOPS = 16;
...
public:
    ...
    virtual bool CheckMLDSASignature(Span<const unsigned char> sig, Span<const unsigned char> pubkey, ScriptError* serror = nullptr) const
    {
        return false;
    }
...
public:
    ...
    bool CheckMLDSASignature(Span<const unsigned char> sig, Span<const unsigned char> pubkey, ScriptError* serror = nullptr) const override;
    bool CheckLockTime(const CScriptNum& nLockTime) const override;
    bool CheckSequence(const CScriptNum& nSequence) const override;
    bool IsCoinStake() const override { return txTo ? txTo->IsCoinStake() : false; }
};
...
public:
    ...
    bool CheckMLDSASignature(Span<const unsigned char> sig, Span<const unsigned char> pubkey, ScriptError* serror = nullptr) const override
    {
        return m_checker.CheckMLDSASignature(sig, pubkey, serror);
    }
