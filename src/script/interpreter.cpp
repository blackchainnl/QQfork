...
unsigned int GetMaxScriptElementSize(unsigned int flags)
{
    return (flags & SCRIPT_VERIFY_V4_LARGE_SCRIPT_ELEMENT) ? V4_MAX_SCRIPT_ELEMENT_SIZE : MAX_SCRIPT_ELEMENT_SIZE;
}

bool ContainsLegacySignatureOpcode(const CScript& script)
{
...
bool EvalScript(std::vector<std::vector<unsigned char> >& stack, const CScript& script, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptExecutionData& execdata, ScriptError* serror)
{
    ...
    const unsigned int max_script_element_size{GetMaxScriptElementSize(flags)};
    ...
    try
    {
        for (; pc < pend; ++opcode_pos) {
            ...
            switch (opcode)
            {
                ...
                case OP_ISCOINSTAKE:
                {
                    if (flags & SCRIPT_VERIFY_ISCOINSTAKE) {
                        // Push 1 if the transaction is a coinstake, else 0
                        stack.push_back(checker.IsCoinStake() ? std::vector<unsigned char>{1} : std::vector<unsigned char>{});
                    } else {
                        if (flags & SCRIPT_VERIFY_DISCOURAGE_UPGRADABLE_NOPS)
                            return set_error(serror, SCRIPT_ERR_DISCOURAGE_UPGRADABLE_NOPS);
                    }
...
template <class T>
bool GenericTransactionSignatureChecker<T>::CheckMLDSASignature(Span<const unsigned char> sig, Span<const unsigned char> pubkey, ScriptError* serror) const
{
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror, bool is_p2sh)
{
    ...
    if (witversion == 0) {
        ...
        const valtype& validator_script_bytes = SpanPopBack(stack);
        const valtype& redeemer = SpanPopBack(stack);
        const valtype& datum = SpanPopBack(stack);
        const unsigned int max_script_element_size{GetMaxScriptElementSize(flags)};
        if (datum.size() > max_script_element_size ||
            redeemer.size() > max_script_element_size ||
            validator_script_bytes.size() > max_script_element_size) {
            return set_error(serror, SCRIPT_ERR_PUSH_SIZE);
        }

        exec_script = CScript(validator_script_bytes.begin(), validator_script_bytes.end());
        if ((flags & SCRIPT_VERIFY_LEGACY_ECDSA_LOCKOUT) && ContainsLegacySignatureOpcode(exec_script)) {
            return set_error(serror, SCRIPT_ERR_EUTXO_LEGACY_SIGOP);
        }
        const std::vector<unsigned char> expected_program = EUTXOProgramForDatumAndValidator(datum, exec_script);
        if (expected_program.size() != program.size() ||
            memcmp(expected_program.data(), program.data(), EUTXO_PROGRAM_SIZE) != 0) {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        }

        std::vector<valtype> eutxo_stack{stack.begin(), stack.end()};
        ...
        const valtype& signature = stack[0];
        const valtype& pubkey = stack[1];
        if (pubkey.size() != ML_DSA::PUBLICKEY_BYTES) return set_error(serror, SCRIPT_ERR_ML_DSA_PUBKEY_SIZE);
        if (signature.size() != ML_DSA::SIGNATURE_BYTES) return set_error(serror, SCRIPT_ERR_ML_DSA_SIG_SIZE);
        uint256 pubkey_hash;
        CSHA256().Write(pubkey.data(), pubkey.size()).Finalize(pubkey_hash.begin());
        QuantumStakeTierProgram tier;
        if (!DecodeQuantumStakeTierProgram(witversion, program, tier)) return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        if (memcmp(pubkey_hash.begin(), tier.commitment.begin(), QUANTUM_MIGRATION_PROGRAM_SIZE) != 0) {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        }
        ...
        const valtype& signature = stack[0];
        const valtype& revealed_pubkey = stack[1];
        const valtype& other_pubkey_hash_bytes = stack[2];
        const valtype& branch_selector = stack[3];

        if (revealed_pubkey.size() != ML_DSA::PUBLICKEY_BYTES) return set_error(serror, SCRIPT_ERR_ML_DSA_PUBKEY_SIZE);
        if (signature.size() != ML_DSA::SIGNATURE_BYTES) return set_error(serror, SCRIPT_ERR_ML_DSA_SIG_SIZE);
        if (other_pubkey_hash_bytes.size() != uint256::size()) return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);

        const bool staker_branch = branch_selector.size() == 1 && branch_selector[0] == 1;
        const bool owner_branch = branch_selector.empty();
        if (!staker_branch && !owner_branch) return set_error(serror, SCRIPT_ERR_QUANTUM_COLDSTAKE_BRANCH);
        if (staker_branch && !checker.IsCoinStake()) return set_error(serror, SCRIPT_ERR_QUANTUM_COLDSTAKE_NOT_COINSTAKE);
        if (owner_branch && checker.IsCoinStake()) return set_error(serror, SCRIPT_ERR_QUANTUM_COLDSTAKE_OWNER_COINSTAKE);

        uint256 revealed_pubkey_hash;
        CSHA256().Write(revealed_pubkey.data(), revealed_pubkey.size()).Finalize(revealed_pubkey_hash.begin());
        uint256 other_pubkey_hash;
        std::copy(other_pubkey_hash_bytes.begin(), other_pubkey_hash_bytes.end(), other_pubkey_hash.begin());

        QuantumStakeTierProgram tier;
        if (!DecodeQuantumStakeTierProgram(witversion, program, tier)) return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        const std::vector<unsigned char> expected_program = tier.tiered
            ? (staker_branch
                ? QuantumTieredColdStakeProgramForKeyHashes(revealed_pubkey_hash, other_pubkey_hash, tier.state, tier.unbonding_blocks, tier.unlock_height)
                : QuantumTieredColdStakeProgramForKeyHashes(other_pubkey_hash, revealed_pubkey_hash, tier.state, tier.unbonding_blocks, tier.unlock_height))
            : (staker_branch
        ...
        if (expected_program.size() != program.size() ||
            memcmp(expected_program.data(), program.data(), program.size()) != 0) {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        }
        return checker.CheckMLDSASignature(signature, revealed_pubkey, serror);
    } else {
                            ...
                            const CScriptWitness& witness, unsigned int flags)
{
    ...
    if ((flags & SCRIPT_VERIFY_EUTXO) &&
        ...
        return CScript(validator_script.begin(), validator_script.end()).GetSigOpCount(true);
    }
