...
struct WalletQuantumRedelegationInfo;
struct WalletUTXOOptimizationTx;
...
public:
    ...
    virtual util::Result<WalletUTXOOptimizationTx> optimizeUTXOSet(const std::string& dest_address, CAmount utxo_amount) = 0;
...
//! Result of a planned or executed Quantum Cold-Stake redelegation.
struct WalletQuantumRedelegationInfo
{
    bool dry_run{true};
...
//! Result of a committed wallet UTXO optimization transaction.
struct WalletUTXOOptimizationTx
{
    std::string txid;
    int outputs{0};
