...
struct StakeRewardSplit {
    std::map<CScript, CAmount> required_outputs;
    CAmount base_pie{0};
    CAmount fees{0};
    CAmount delegated_gross_reward{0};
    CAmount operator_comp{0};
    CAmount total_reward{0};
};

bool ComputeStakeRewardSplit(const CTransaction& coinstake, const CTxUndo& coinstake_undo, CAmount block_fees, int candidate_height, bool stake_tiers_active, StakeRewardSplit& split, std::string& reject_reason, const std::vector<CAmount>* effective_principals = nullptr);
bool CheckStakeRewardSplit(const CTransaction& coinstake, const CTxUndo& coinstake_undo, CAmount block_fees, int candidate_height, bool stake_tiers_active, StakeRewardSplit& split, std::string& reject_reason, const std::vector<CAmount>* effective_principals = nullptr);
...
/** Cold-staking consensus helpers (Quantum Quasar V4). Cold-stake coinstakes
 *  intentionally earn a reduced subsidy so self-staking full nodes remain the
 *  economically preferred security model. */
class CTxIn;
class CTxOut;
class CTxUndo;
...
bool CheckTieredStakePrincipalCovenant(const CTransaction& tx, const CCoinsViewCache& inputs, unsigned int flags, int spend_height, std::string& reject_reason, const Consensus::Params* consensus_params = nullptr, int64_t spend_time = 0);
...
bool TestBlockValidity(BlockValidationState& state,
                       ...
                       bool fCheckBlockSig = true) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
