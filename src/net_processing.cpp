...
bool PeerManagerImpl::ProcessNetBlock(const std::shared_ptr<const CBlock> pblock, bool force_processing, bool min_pow_checked, bool* new_block, CNode& pfrom)
{
...

    {
        ...
        // Check for the checkpoint. During assumeutxo background validation we
        // intentionally download historical blocks that are ancestors of the
        // active snapshot base; those blocks must reach the background
        // chainstate even though they are older than the active snapshot tip.
        bool assumeutxo_background_block = false;
        if (pindex && m_chainman.BackgroundSyncInProgress()) {
            if (const CBlockIndex* snapshot_base = m_chainman.GetSnapshotBaseBlock()) {
                assumeutxo_background_block = pindex->nHeight <= snapshot_base->nHeight &&
                    snapshot_base->GetAncestor(pindex->nHeight) == pindex;
            }
        }
        CBlockIndex* tip = m_chainman.ActiveChain().Tip();
        if (!assumeutxo_background_block && tip && pblock->hashPrevBlock != tip->GetBlockHash())
        {
...
void PeerManagerImpl::RelayTransaction(const uint256& txid, const uint256& wtxid)
{
    ...
    for(auto& it : m_peer_map) {
        ...
        if (!tx_relay->m_tx_inventory_known_filter.contains(hash)) {
            queued_inventory |= tx_relay->m_tx_inventory_to_send.insert(hash).second;
        }
    };
    if (queued_inventory) m_connman.WakeMessageHandler();
}
