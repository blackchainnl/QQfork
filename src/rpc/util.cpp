...
std::string GetAllOutputTypes()
{
    ...
    for (const TxoutType type : {
             ...
             TxoutType::EUTXO_COMMITMENT,
             TxoutType::WITNESS_UNKNOWN,
             TxoutType::RGB_COMMITMENT,
         }) {
        ret.emplace_back(GetTxnOutputType(type));
    }
    return Join(ret, ", ");
}
...
std::string RPCArg::ToStringObj(const bool oneline) const
{
    ...
    case Type::OBJ_USER_KEYS:
        res += "{";
        res += Join(m_inner, ",", [&](const RPCArg& i) { return i.ToStringObj(oneline); });
        if (m_type != Type::OBJ) {
            if (!m_inner.empty()) res += ",";
            res += "...";
        }
        return res + "}";
    } // no default case, so the compiler can warn about missing cases
