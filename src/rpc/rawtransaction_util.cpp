...
static constexpr uint32_t MAX_BIP125_RBF_SEQUENCE{CTxIn::SEQUENCE_FINAL - 2};
...
void AddInputs(CMutableTransaction& rawTx, const UniValue& inputs_in, std::optional<bool> rbf)
{
    ...
    for (unsigned int idx = 0; idx < inputs.size(); idx++) {
        ...
        if (rbf && has_sequence && (*rbf != (nSequence <= MAX_BIP125_RBF_SEQUENCE))) {
...
void AddOutputs(CMutableTransaction& rawTx, const UniValue& outputs_in)
{
    ...
    for (const std::string& name_ : outputs.getKeys()) {
        if (name_ == "data") {
        ...
        } else if (name_ == "rgb_commitment") {
            if (has_data) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, duplicate data/RGB commitment output");
            }
            has_data = true;
            const uint256 state_hash = ParseHashV(outputs[name_], "rgb_commitment");
            ...
            has_eutxo = true;
            const UniValue& eutxo = outputs[name_].get_obj();
            RPCTypeCheckObj(eutxo,
                {
                    {"amount", UniValueType()},
                    {"datum", UniValueType(UniValue::VSTR)},
                    {"validator", UniValueType(UniValue::VSTR)},
                });
            const CAmount nAmount = AmountFromValue(eutxo.find_value("amount"));
            const std::vector<unsigned char> datum = ParseHexO(eutxo, "datum");
            const std::vector<unsigned char> validator_bytes = ParseHexO(eutxo, "validator");
            CScript validator_script(validator_bytes.begin(), validator_bytes.end());
            CTxOut out(nAmount, CreateEUTXOCommitment(datum, validator_script));
            rawTx.vout.push_back(out);
        } else {
...
{
    return ConstructTransaction(inputs_in, outputs_in, locktime, UniValue{});
}
...
CMutableTransaction ConstructTransaction(const UniValue& inputs_in, const UniValue& outputs_in, const UniValue& locktime, const UniValue& rbf)
{
    ...
    AddInputs(rawTx, inputs_in, replaceable);
