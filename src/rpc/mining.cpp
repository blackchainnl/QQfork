...
static bool gbt_is_blackcoin_readiness_deployment(const Consensus::DeploymentPos pos)
{
    return pos == Consensus::DEPLOYMENT_QUANTUM_QUASAR || pos == Consensus::DEPLOYMENT_QUANTUM_MIGRATION;
}

static const char* gbt_blackcoin_phase_name(Consensus::QuantumQuasarPhase phase)
{
    switch (phase) {
        ...
        },
        {
            ...
            RPCResult{"Otherwise", RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::OBJ, "quantumquasar", "Blackcoin V4 roadmap data for miners", {
                    ...
                    {RPCResult::Type::BOOL, "quantum_migration_outputs_fundable", "whether policy permits funding quantum migration outputs"},
                    {RPCResult::Type::OBJ_DYN, "readiness_signalling", "versionbits readiness signals keyed by deployment name", {
                        {RPCResult::Type::NUM, "deployment", "version bit used for readiness signalling"},
                    }},
                }},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (int j = 0; j < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; ++j) {
        ...
        switch (state) {
            ...
            {
                if (gbt_is_blackcoin_readiness_deployment(pos)) {
                    break;
                }
                // Add to rules only
                const struct VBDeploymentInfo& vbinfo = VersionBitsDeploymentInfo[pos];
    ...
    quantumquasar.pushKV("readiness_signalling", readiness_signalling);
    result.pushKV("quantumquasar", quantumquasar);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!CheckBlock(block, state, Params().GetConsensus(), active_chainstate, true, true, false)) {
        throw JSONRPCError(-100, strprintf("Block failed CheckBlock() function: %s", state.ToString()));
    }
...
static RPCHelpMan signrawblockwithquantumkey()
{
    return RPCHelpMan{"signrawblockwithquantumkey",
        "\nSign a raw Blackcoin proof-of-stake block with an ML-DSA key.\n"
        "This does not mine, submit, or validate the block. It only fills the block signature\n"
        "when the block's coinstake signing output commits to the supplied public key.\n",
        {
            {"hexdata", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The hex-encoded block data to sign"},
            {"public_key", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The ML-DSA-44 public key hex committed by the coinstake signing output"},
            {"private_key", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The ML-DSA-44 private key hex"},
        },
        RPCResult{RPCResult::Type::OBJ, "", "", {
            {RPCResult::Type::STR_HEX, "hex", "The signed block hex"},
            {RPCResult::Type::STR_HEX, "hash", "The block hash signed by ML-DSA"},
        }},
        RPCExamples{
            HelpExampleCli("signrawblockwithquantumkey", "\"myblockhex\" \"pubhex\" \"privhex\"")
          + HelpExampleRpc("signrawblockwithquantumkey", "\"myblockhex\", \"pubhex\", \"privhex\"")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    CBlock block;
    ...
    if (!block.IsProofOfStake() || block.vtx.size() < 2 || block.vtx[1]->vout.size() < 2) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Block is not a Blackcoin proof-of-stake candidate");
    }

    const std::vector<unsigned char> public_key = ParseHexV(request.params[1], "public_key");
    std::vector<unsigned char> private_key = ParseHexV(request.params[2], "private_key");
    if (public_key.size() != ML_DSA::PUBLICKEY_BYTES) {
    ...
    if (private_key.size() != ML_DSA::SECRETKEY_BYTES) {
        if (!private_key.empty()) memory_cleanse(private_key.data(), private_key.size());
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("private_key must be exactly %u bytes", ML_DSA::SECRETKEY_BYTES));
    }

    std::vector<unsigned char> committed_pubkey;
    if (!ExtractQuantumBlockSigningPubKey(block.vtx[1]->vout[1].scriptPubKey, committed_pubkey)) {
        memory_cleanse(private_key.data(), private_key.size());
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Coinstake does not contain a Blackcoin ML-DSA block-signing output");
    }
    if (committed_pubkey != public_key) {
        memory_cleanse(private_key.data(), private_key.size());
        throw JSONRPCError(RPC_INVALID_PARAMETER, "public_key does not match the coinstake block-signing output");
    }

    std::vector<uint8_t> signature;
    const uint256 block_hash = block.GetHash();
    const bool signed_block = ML_DSA::Sign(private_key, block_hash.begin(), uint256::size(), signature);
    memory_cleanse(private_key.data(), private_key.size());
    if (!signed_block) {
        throw JSONRPCError(RPC_MISC_ERROR, "ML-DSA block signing failed");
    }
...
static RPCHelpMan getshadowpowwork()
{
    return RPCHelpMan{"getshadowpowwork",
        "\nReturns the current Blackcoin shadow PoW grind parameters.\n"
        "Use the returned height/prevhash/target_bits with the Argon2id QQSPROOF scheme,\n"
        "then pass the resulting proof to a shadow claim transaction. PoW targets are not whitelist-gated,\n"
        "and the claim payout must be a Blackcoin migration address. Read-only.\n",
        {
            {"target_address", RPCArg::Type::STR, RPCArg::Optional::OMITTED,
        ...
        RPCResult{RPCResult::Type::OBJ, "", "", {
            ...
            {RPCResult::Type::NUM, "height", "Height of the block a claim mined now would target (tip height + 1)."},
            {RPCResult::Type::STR_HEX, "prevhash", "Previous block hash bound into the Argon2id proof (current tip hash)."},
            {RPCResult::Type::NUM, "target_bits", "Required leading-zero bits for the next-block PoW claim (RetargetedBits)."},
            {RPCResult::Type::STR_AMOUNT, "pow_jackpot", "Estimated PoW-side payout the next valid claim would receive."},
            {RPCResult::Type::NUM, "pow_amount", "Estimated PoW-side payout the next valid claim would receive in satoshis."},
            ...
            {RPCResult::Type::STR, "quantum_address", /*optional=*/true, "Quantum migration payout address committed into the proof, if supplied."},
            {RPCResult::Type::STR_HEX, "quantum_payout_script", /*optional=*/true, "Quantum migration scriptPubKey committed into the proof, if quantum_address supplied."},
        }},
        RPCExamples{
            HelpExampleCli("getshadowpowwork", "")
          + HelpExampleRpc("getshadowpowwork", "")
        },
    [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ChainstateManager& chainman = EnsureAnyChainman(request.context);
    ...
    if (!tip) throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "No active chain tip");
    ...
    UniValue obj(UniValue::VOBJ);
    obj.pushKV("active", active_phase);
    obj.pushKV("height", next_height);
    obj.pushKV("prevhash", tip->GetBlockHash().GetHex());
    obj.pushKV("target_bits", (uint64_t)info.pow_target_bits);
    obj.pushKV("pow_jackpot", ValueFromAmount(next_pow_payout));
    obj.pushKV("pow_amount", next_pow_payout);
    obj.pushKV("pow_pool_jackpot", ValueFromAmount(info.pow_amount));
    obj.pushKV("pow_pool_amount", info.pow_amount);
    ...
    if (!request.params[0].isNull()) {
        ...
        if (!IsValidDestination(dest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid target_address");
    ...
    if (!request.params[1].isNull()) {
        const CTxDestination quantum_dest = DecodeDestination(request.params[1].get_str());
...
void RegisterMiningRPCCommands(CRPCTable& t)
{
    static const CRPCCommand commands[]{
        {"mining", &getshadowpowwork},
        {"mining", &getnetworkhashps},
        {"mining", &getmininginfo},
        {"mining", &getblocktemplate},
        {"mining", &signrawblockwithquantumkey},
        {"mining", &submitblock},
        {"mining", &submitheader},
