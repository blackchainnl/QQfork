...
struct DBShadowPayoutKey {
    uint256 block_hash;

    explicit DBShadowPayoutKey(const uint256& hash_in) : block_hash(hash_in) {}

    SERIALIZE_METHODS(DBShadowPayoutKey, obj)
    {
        uint8_t prefix{DB_SHADOW_PAYOUTS};
        READWRITE(prefix);
        if (prefix != DB_SHADOW_PAYOUTS) {
            throw std::ios_base::failure("Invalid format for coinstatsindex DB shadow payout key");
        }
...
static CAmount GetUndoAwareBlockSubsidy(const CBlock& block, const CBlockIndex& pindex, const CBlockUndo& block_undo, const Consensus::Params& consensus)
{
    ...
    const bool coldstake = coldstake_active &&
                           ...
                           HasColdStakeInputs(*block.vtx[1], block_undo.vtxundo.front());
...
void CoinStatsIndex::RemoveCreatedCoin(const COutPoint& outpoint, const Coin& coin)
{
...
void CoinStatsIndex::RemoveSpentCoin(const COutPoint& outpoint, const Coin& coin)
{
    RemoveCoinHash(m_muhash, outpoint, coin);
...
void CoinStatsIndex::RestoreSpentCoin(const COutPoint& outpoint, const Coin& coin)
{
    ApplyCoinHash(m_muhash, outpoint, coin);
...
bool CoinStatsIndex::CustomAppend(const interfaces::BlockInfo& block)
{
    ...
    std::vector<std::pair<COutPoint, Coin>> shadow_payouts;
    CAmount block_subsidy{GetBlockSubsidy(block.height, Params().GetConsensus(), block.is_pos)};
    ...
    if (block.height > 0) {
        ...
        assert(block.data);
        block_subsidy = GetUndoAwareBlockSubsidy(*block.data, *pindex, block_undo, Params().GetConsensus());
        ...
        for (size_t i = 0; i < block.data->vtx.size(); ++i) {
            ...
            for (uint32_t j = 0; j < tx->vout.size(); ++j) {
                ...
                const uint32_t coin_time = GetCoinTime(*tx, block.data->nTime);
...

        {
            ...
            for (const ShadowSyntheticPayoutCoin& payout : GetAppliedShadowClaimPayoutCoins(m_chainstate->CoinsTip(), block.height, block.hash, block.data->GetBlockTime())) {
                if (payout.time < 0 || payout.time > static_cast<int64_t>(std::numeric_limits<uint32_t>::max())) {
                    return error("%s: synthetic payout time is outside the serialized block-time range", __func__);
                }
    ...
    CDBBatch batch(*m_db);
    if (!shadow_payouts.empty()) {
        batch.Write(DBShadowPayoutKey(block.hash), shadow_payouts);
    }
...
bool CoinStatsIndex::ReverseBlock(const CBlock& block, const CBlockIndex* pindex)
{
    ...
    std::pair<uint256, DBVal> read_out;
    std::vector<std::pair<COutPoint, Coin>> shadow_payouts;
    m_db->Read(DBShadowPayoutKey(pindex->GetBlockHash()), shadow_payouts);

    CAmount block_subsidy{GetBlockSubsidy(pindex->nHeight, Params().GetConsensus(), block.IsProofOfStake())};
    ...
    if (pindex->nHeight > 0) {
        ...
        block_subsidy = GetUndoAwareBlockSubsidy(block, *pindex, block_undo, Params().GetConsensus());
    ...
    for (size_t i = 0; i < block.vtx.size(); ++i) {
        ...
        for (uint32_t j = 0; j < tx->vout.size(); ++j) {
            const CTxOut& out{tx->vout[j]};
            COutPoint outpoint{tx->GetHash(), j};
            const uint32_t coin_time = GetCoinTime(*tx, block.nTime);
    ...
    for (auto it = shadow_payouts.rbegin(); it != shadow_payouts.rend(); ++it) {
        RemoveCreatedCoin(it->first, it->second);
        m_total_subsidy -= it->second.out.nValue;
    }
