...
static void check_computeblockversion(VersionBitsCache& versionbitscache, const Consensus::Params& params, Consensus::DeploymentPos dep)
{
    ...
    // Blackcoin uses block time instead of median time past after Protocol V2, so
    // Bitcoin's synthetic MTP transition checks below do not apply to later live
    // deployments. The explicit regtest/testdummy cases still exercise BIP9
    // transitions with pre-V2-style median-time behavior.
    if (params.IsProtocolV2(nStartTime)) return;
