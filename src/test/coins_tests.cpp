...
BOOST_AUTO_TEST_CASE(ccoins_serialization)
{
    ...
    // Coinstake example with a nonzero transaction time
    DataStream ss_coinstake{ParseHex("2a85b0a1be603206")};
    Coin coinstake;
    ss_coinstake >> coinstake;
    BOOST_CHECK_EQUAL(coinstake.fCoinBase, false);
    BOOST_CHECK_EQUAL(coinstake.fCoinStake, true);
    BOOST_CHECK_EQUAL(coinstake.nHeight, 10U);
    BOOST_CHECK_EQUAL(coinstake.nTime, 1713938400U);
    BOOST_CHECK_EQUAL(coinstake.out.nValue, 50 * COIN);
    BOOST_CHECK_EQUAL(coinstake.out.scriptPubKey.size(), 0U);
