...
static constexpr std::array<unsigned char, 4> BIP324_TEST_VECTOR_MAGIC{{0xf9, 0xbe, 0xb4, 0xd9}};
    ...
    const std::string& out_ciphertext_endswith_hex)
{
    ...
    cipher.InitializeForTest(ellswift_theirs, in_initiating, MakeUCharSpan(BIP324_TEST_VECTOR_MAGIC));
    BOOST_CHECK(cipher);
    ...
    for (unsigned error = 0; error <= 12; ++error) {
        ...
        dec_cipher.InitializeForTest(ellswift_theirs, (error == 1) ^ in_initiating, MakeUCharSpan(BIP324_TEST_VECTOR_MAGIC), /*self_decrypt=*/true);
        BOOST_CHECK(dec_cipher);
...
BOOST_AUTO_TEST_CASE(packet_test_vectors) {
    // BIP324 key derivation uses network magic in the HKDF process. The published vectors use
    // Bitcoin mainnet magic bytes; production transport still uses this fork's active params.
