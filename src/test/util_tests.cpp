...
BOOST_AUTO_TEST_CASE(util_ParseMoney)
{
    ...
    BOOST_CHECK_EQUAL(ParseMoney("20000000000.00").value(), MAX_MONEY);

    // Parsing amount that cannot be represented should fail
    BOOST_CHECK(!ParseMoney("20000000000.00000001"));
    BOOST_CHECK(!ParseMoney("20000000001.00"));
...
BOOST_AUTO_TEST_CASE(message_sign)
{
    ...
    const std::string expected_signature =
        "IMK3GXu4xXjb2n6j2efXgD9vVv8cpk3zMDLo5834vFH0ctRd5bc/rR2q6xlXbvyizQxFyij8pZaglwnZQDv8Kys=";
...
BOOST_AUTO_TEST_CASE(message_verify)
{
    ...
    const std::array<unsigned char, 32> other_privkey_bytes = {
        0x54, 0x1A, 0x6D, 0x2B, 0xE9, 0x44, 0x17, 0x90,
        0x2E, 0x5C, 0x3F, 0xA8, 0x76, 0x10, 0xB1, 0xCD,
        0x43, 0x82, 0x0E, 0x9B, 0xF4, 0x61, 0x35, 0x27,
        0x99, 0xBA, 0xE2, 0x70, 0x18, 0x4D, 0x63, 0x01
    };

    CKey privkey;
    privkey.Set(privkey_bytes.begin(), privkey_bytes.end(), true);
    BOOST_REQUIRE(privkey.IsValid());
    const std::string valid_address = EncodeDestination(PKHash(privkey.GetPubKey()));
    std::string valid_signature;
    BOOST_REQUIRE(MessageSign(privkey, "Trust no one", valid_signature));

    CKey other_privkey;
    other_privkey.Set(other_privkey_bytes.begin(), other_privkey_bytes.end(), true);
    BOOST_REQUIRE(other_privkey.IsValid());
    const std::string other_address = EncodeDestination(PKHash(other_privkey.GetPubKey()));
    BOOST_REQUIRE(other_address != valid_address);

    CKey uncompressed_privkey;
    uncompressed_privkey.Set(privkey_bytes.begin(), privkey_bytes.end(), false);
    BOOST_REQUIRE(uncompressed_privkey.IsValid());
    const std::string uncompressed_address = EncodeDestination(PKHash(uncompressed_privkey.GetPubKey()));
    std::string uncompressed_signature;
    BOOST_REQUIRE(MessageSign(uncompressed_privkey, "Trust me", uncompressed_signature));
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            EncodeDestination(ScriptHash(CScript() << OP_TRUE)),
            "signature should be irrelevant",
            "message too"),
