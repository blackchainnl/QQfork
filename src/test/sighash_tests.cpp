...
void static RandomTransaction(CMutableTransaction& tx, bool fSingle)
{
    ...
    CAmount remaining_value = MAX_MONEY;
    for (int out = 0; out < outs; out++) {
        tx.vout.emplace_back();
        CTxOut &txout = tx.vout.back();
        txout.nValue = static_cast<CAmount>(InsecureRandRange(static_cast<uint64_t>(remaining_value) + 1));
        remaining_value -= txout.nValue;
        RandomScript(txout.scriptPubKey);
        if (txout.IsEmpty()) txout.scriptPubKey << OP_TRUE;
    }
}
