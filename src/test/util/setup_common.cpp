    ...
    : BasicTestingSetup(chainType, extra_args)
{
    ...
    Assert(ApplyArgsManOptions(*m_node.args, chainman_opts));
    ...
    : TestingSetup{ChainType::REGTEST, extra_args, coins_db_in_memory, block_tree_db_in_memory}
{
    ...
    this->mineBlocks(100);
    {
        ...
        assert(m_coinbase_txns.size() == 100);
    }
}
                                                                     ...
                                                                     bool submit)
{
    ...
    if (mempool_txn.nVersion >= 2) {
        mempool_txn.nTime = 0;
    }
    const CTransaction tx_const{mempool_txn};
    PrecomputedTransactionData txdata;
    txdata.Init(tx_const, std::vector<CTxOut>{utxo_to_spend.out}, true);

    SignatureData sigdata = DataFromTransaction(mempool_txn, 0, utxo_to_spend.out);
    ProduceSignature(
        keystore,
        MutableTransactionSignatureCreator{mempool_txn, 0, utxo_to_spend.out.nValue, &txdata, nHashType},
        utxo_to_spend.out.scriptPubKey,
        sigdata);
    UpdateInput(mempool_txn.vin[0], sigdata);
    ...
    assert(VerifyScript(
        mempool_txn.vin[0].scriptSig,
        utxo_to_spend.out.scriptPubKey,
        &mempool_txn.vin[0].scriptWitness,
        verify_flags,
        TransactionSignatureChecker{&tx_const, 0, utxo_to_spend.out.nValue, txdata, MissingDataBehavior::FAIL},
        &serror));
