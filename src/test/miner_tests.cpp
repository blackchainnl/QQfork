...
static int64_t MinerTestMockTime()
{
    return Params().GetConsensus().nProtocolV3_1Time - 1;
}
...
void MinerTestingSetup::TestPackageSelection(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    tx.nTime = 0; // v2+ transactions do not serialize nTime; block time is the spend time.
    tx.vin.resize(1);
...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
...

    {
        ...
        for (unsigned int i = 0; i < tx_count_exceeding_sigops; ++i) {
            tx.vout[0].nValue -= sigops_test_fee;
            hash = tx.GetHash();
            bool spendsCoinbase = i == 0; // only first tx spends coinbase
            // If we don't set the # of sig ops in the CTxMemPoolEntry, template creation fails
            tx_mempool.addUnchecked(entry.Fee(sigops_test_fee).Time(Now<NodeSeconds>()).SpendsCoinbase(spendsCoinbase).FromTx(tx));
...

    {
        ...
        // orphan in tx_mempool, template creation skips the invalid entry
        hash = tx.GetHash();
...

    {
        ...
        // coinbase in tx_mempool, template creation skips the invalid entry
        tx.vin.resize(1);
...

    {
        ...
        // double spend txn pair in tx_mempool, template creation skips the invalid entry
        tx.vin[0].prevout.hash = txFirst[0]->GetHash();
        tx.vin[0].prevout.n = 0;
        tx.vin[0].scriptSig = CScript() << OP_1;
        tx.vout[0].nValue = BLOCKSUBSIDY - HIGHFEE;
    ...
    // None of the absolute or relative locked txs should have made it into the
    // template because CreateNewBlock checks finality and sequence locks.
    BOOST_CHECK_EQUAL(pblocktemplate->block.vtx.size(), 1U);
    // The assembler rechecks locktime and sequence locks against the current tip,
    // so the synthetic height/time mutation makes these entries eligible.
    for (int i = 0; i < CBlockIndex::nMedianTimeSpan; ++i) {
        CBlockIndex* ancestor{Assert(m_node.chainman->ActiveChain().Tip()->GetAncestor(m_node.chainman->ActiveChain().Tip()->nHeight - i))};
