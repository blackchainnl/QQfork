...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    const auto OpReturnScript = [](size_t payload_size) {
        return CScript() << OP_RETURN << std::vector<unsigned char>(payload_size, 0);
    };

    // MAX_OP_RETURN_RELAY-byte TxoutType::NULL_DATA (standard)
    t.vout[0].scriptPubKey = OpReturnScript(MAX_OP_RETURN_RELAY - 3);
    BOOST_CHECK_EQUAL(MAX_OP_RETURN_RELAY, t.vout[0].scriptPubKey.size());
    CheckIsStandard(t);

    // MAX_OP_RETURN_RELAY+1-byte TxoutType::NULL_DATA (non-standard)
    t.vout[0].scriptPubKey = OpReturnScript(MAX_OP_RETURN_RELAY - 2);
    ...
    const auto CheckDustThreshold = [&](const CScript& script_pub_key) {
    ...
    CheckDustThreshold(CScript() << std::vector<unsigned char>(33, 0x02) << OP_CHECKSIG);

    // Check uncompressed P2PK outputs dust threshold (must have leading 04/06/07)
    CheckDustThreshold(CScript() << std::vector<unsigned char>(65, 0x04) << OP_CHECKSIG);

    // Check P2PKH outputs dust threshold
    CheckDustThreshold(CScript() << OP_DUP << OP_HASH160 << std::vector<unsigned char>(20, 0) << OP_EQUALVERIFY << OP_CHECKSIG);

    // Check P2SH outputs dust threshold
    CheckDustThreshold(CScript() << OP_HASH160 << std::vector<unsigned char>(20, 0) << OP_EQUAL);

    // Check P2WPKH outputs dust threshold
    CheckDustThreshold(CScript() << OP_0 << std::vector<unsigned char>(20, 0));

    // Check P2WSH outputs dust threshold
    CheckDustThreshold(CScript() << OP_0 << std::vector<unsigned char>(32, 0));

    // Check P2TR outputs dust threshold (Invalid xonly key ok!)
    CheckDustThreshold(CScript() << OP_1 << std::vector<unsigned char>(32, 0));
    ...
    for (int op = OP_1; op <= OP_16; op += 1) {
        CheckDustThreshold(CScript() << (opcodetype)op << std::vector<unsigned char>(2, 0));
    }
}
