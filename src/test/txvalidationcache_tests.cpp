...
BOOST_FIXTURE_TEST_CASE(tx_mempool_block_doublespend, TestChain100Setup)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        uint256 hash = SignatureHash(scriptPubKey, spends[i], 0, hash_type, 0, SigVersion::BASE);
...
static void ValidateCheckInputsForAllFlags(const CTransaction &tx, uint32_t failing_flags, bool add_to_cache, CCoinsViewCache& active_coins_tip) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    for (int count = 0; count < 10000; ++count) {
        ...
        // This fixture exercises legacy script-cache invalidation. V4-only flags
        // are tested separately because they change signature hash requirements
        // and coinstake opcode semantics instead of simply tightening legacy
        // script acceptance.
        test_flags &= ~(SCRIPT_ENABLE_SIGHASH_FORKID |
                        SCRIPT_VERIFY_ISCOINSTAKE |
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, TestChain100Setup)
{
    ...
    // Replace the intentionally invalid signature before mining it. Blackcoin
    // enforces DERSIG at all heights, and default regtest remains in perpetual
    // Gold Rush, so block-validation fixtures use base-compatible signatures.
    {
        std::vector<unsigned char> vchSig;
        const int hash_type = SIGHASH_ALL;
        uint256 hash = SignatureHash(p2pk_scriptPubKey, spend_tx, 0, hash_type, 0, SigVersion::BASE);
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
        vchSig.push_back((unsigned char)hash_type);
