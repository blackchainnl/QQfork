...
// Copyright (c) 2013-2022 Quantum Quasar Developers
...
BOOST_AUTO_TEST_CASE(bip32_in_place_derivation)
{
    CExtKey base{DecodeExtKey(test1.vDerive[0].prv)};
    CExtKey expected;
    BOOST_REQUIRE(base.Derive(expected, 0x80000000UL));
    BOOST_REQUIRE(expected.Derive(expected, 1));

    CExtKey in_place{DecodeExtKey(test1.vDerive[0].prv)};
    BOOST_REQUIRE(in_place.Derive(in_place, 0x80000000UL));
    BOOST_REQUIRE(in_place.Derive(in_place, 1));

    BOOST_CHECK(in_place == expected);
}
