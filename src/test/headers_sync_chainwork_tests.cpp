...
BOOST_AUTO_TEST_CASE(headers_sync_state)
{
    ...
    const uint32_t n_bits{UintToArith256(Params().GetConsensus().powLimit).GetCompact()};
    ...
    const arith_uint256 chain_work{chain_start->nChainWork + GetBlockProof(proof_index) * (target_blocks - 1)};
