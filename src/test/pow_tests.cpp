...
BOOST_AUTO_TEST_CASE(get_next_work)
{
    ...
    int64_t nLastRetargetTime = pindexLast.nTime - chainParams->GetConsensus().nTargetSpacingV1;

    // Here (and below): expected_nbits is calculated in
    // CalculateNextTargetRequired(); redoing the calculation here would be just
    // reimplementing the same code that is written in pow.cpp. Rather than
    // copy that code, we just hardcode the expected result.
    unsigned int expected_nbits = 0x1d028699;
    // Blackcoin
...
BOOST_AUTO_TEST_CASE(get_next_work_pow_limit)
{
    ...
    int64_t nLastRetargetTime = 1231006505; // Far enough back to clamp to powLimit.
...
/* Test that negative actual spacing is neutralized after the V1 retargeting fix */
BOOST_AUTO_TEST_CASE(get_next_work_negative_spacing_fix)
{
    const auto chainParams = CreateChainParams(*m_node.args, ChainType::MAIN);
    CBlockIndex pindexLast;
    ...
    int64_t nLastRetargetTime = pindexLast.nTime + 1000;
...
BOOST_AUTO_TEST_CASE(get_next_work_v3_upper_limit_actual)
{
    const auto chainParams = CreateChainParams(*m_node.args, ChainType::MAIN);
    CBlockIndex pindexLast;
    pindexLast.nHeight = 46367;
    pindexLast.nTime = 1450000000;
    pindexLast.nBits = 0x1d010000;
    int64_t nLastRetargetTime = pindexLast.nTime - 100000;
    unsigned int expected_nbits = 0x1d022000;
    // Blackcoin
    BOOST_CHECK_EQUAL(CalculateNextTargetRequired(&pindexLast, nLastRetargetTime, chainParams->GetConsensus(), false), expected_nbits);
    /*
    BOOST_CHECK(PermittedDifficultyTransition(chainParams->GetConsensus(), pindexLast.nHeight+1, pindexLast.nBits, expected_nbits));
