...
struct MinerTestingSetup : public TestingSetup {
    MinerTestingSetup() : TestingSetup{ChainType::REGTEST, {"-checkpoints=0"}} {}
...
std::shared_ptr<CBlock> MinerTestingSetup::Block(const uint256& prev_hash)
{
    ...
    pblock->nBits = GetNextTargetRequired(
        WITH_LOCK(::cs_main, return m_node.chainman->m_blockman.LookupBlockIndex(prev_hash)),
        Params().GetConsensus(),
        /*fProofOfStake=*/false);
...
std::shared_ptr<CBlock> MinerTestingSetup::FinalizeBlock(std::shared_ptr<CBlock> pblock)
{
    ...
    */
    ...
    BOOST_CHECK_MESSAGE(
        Assert(m_node.chainman)->ProcessNewBlockHeaders({pblock->GetBlockHeader()}, true, ignored, false),
        ignored.ToString());
...
std::shared_ptr<const CBlock> MinerTestingSetup::BadBlock(const uint256& prev_hash)
{
    ...
    coinbase_spend.nTime = pblock->nTime;
...
BOOST_FIXTURE_TEST_SUITE(v2_block_time_tests, validation_block_tests::PostGTransactionTimeSetup)
