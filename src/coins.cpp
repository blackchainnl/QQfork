...
class CCoinsViewCacheCursor final : public CCoinsViewCursor
{
private:
    std::unique_ptr<CCoinsViewCursor> m_base_cursor;
    const CCoinsMap& m_cache;
    CCoinsMap::const_iterator m_cache_it;
    const CCoinsMap::const_iterator m_cache_end;
    bool m_cache_phase{false};
    bool m_valid{false};
    COutPoint m_key;
    Coin m_coin;
    ...
    {
        m_valid = false;
        while (!m_cache_phase && m_base_cursor && m_base_cursor->Valid()) {
            COutPoint key;
            Coin coin;
            if (m_base_cursor->GetKey(key) && m_base_cursor->GetValue(coin) && m_cache.find(key) == m_cache.end() && !coin.IsSpent()) {
                m_key = key;
                m_coin = coin;
                m_valid = true;
                return;
        ...
        m_cache_phase = true;
        while (m_cache_it != m_cache_end) {
            const auto& [key, entry] = *m_cache_it++;
...
public:
    CCoinsViewCacheCursor(const uint256& hash_block, std::unique_ptr<CCoinsViewCursor> base_cursor, const CCoinsMap& cache)
        : CCoinsViewCursor(hash_block),
          m_base_cursor(std::move(base_cursor)),
          m_cache(cache),
          m_cache_it(m_cache.begin()),
          m_cache_end(m_cache.end())
    {
        Advance();
    }
    ...
    {
        if (!m_valid) return;
        if (!m_cache_phase && m_base_cursor) m_base_cursor->Next();
        Advance();
    }
};
...
{
    return std::make_unique<CCoinsViewCacheCursor>(GetBestBlock(), base->Cursor(), cacheCoins);
}
...
void AddCoins(CCoinsViewCache& cache, const CTransaction &tx, int nHeight, bool check_for_overwrite, int nBlockTime) {
    ...
    for (size_t i = 0; i < tx.vout.size(); ++i) {
        ...
        cache.AddCoin(COutPoint(txid, i), Coin(tx.vout[i], nHeight, fCoinbase, fCoinstake, coin_time), overwrite);
    }
}
...
static const size_t MAX_OUTPUTS_PER_BLOCK = V4_MAX_BLOCK_WEIGHT / MIN_TRANSACTION_OUTPUT_WEIGHT;
