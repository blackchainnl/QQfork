...
struct CStakeCache{
    CStakeCache(uint32_t blockFromTime_, CAmount amount_, const CScript& scriptPubKey_) : blockFromTime(blockFromTime_), amount(amount_), scriptPubKey(scriptPubKey_){
    }
...
struct StakeWeightContext {
    uint16_t multiplier_ppm10k{10000};
    uint16_t pool_ppm10k{10000};
    int effective_unbonding_blocks{0};
