// Copyright (c) 2011-2021 The Blackcoin - Blackcoin developers
    ...
    m_passphrase_out(passphrase_out)
{
    ...
    ui->quantumStakingCheckBox->hide();
    // The two unlock-scope checkboxes are mutually exclusive: exactly one is
    // active — "For Legacy Staking Only" (mint PoS blocks only) or "Legacy and
    // Quantum Staking" (full unlock, required for quantum/Gold Rush/migration/
    // cold-staking actions). An exclusive button group gives radio-like
    // behaviour while keeping the checkbox appearance.
    auto* unlock_scope = new QButtonGroup(this);
    unlock_scope->setExclusive(true);
    unlock_scope->addButton(ui->stakingCheckBox);
    unlock_scope->addButton(ui->quantumStakingCheckBox);
