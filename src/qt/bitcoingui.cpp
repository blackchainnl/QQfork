...
void BitcoinGUI::createActions()
{
    ...
    stakingMiningAction = new QAction(platformStyle->SingleColorIcon(":/icons/tx_mined"), tr("S&taking && Mining"), this);
    stakingMiningAction->setStatusTip(tr("Enable staking and Gold Rush mining"));
    stakingMiningAction->setToolTip(stakingMiningAction->statusTip());
    stakingMiningAction->setCheckable(true);
    stakingMiningAction->setShortcut(QKeySequence(QStringLiteral("Alt+5")));
    tabGroup->addAction(stakingMiningAction);
    ...
    accountAction = new QAction(platformStyle->SingleColorIcon(":/icons/address-book"), tr("&Account"), this);
    accountAction->setStatusTip(tr("Inspect wallet balances by address and output"));
    accountAction->setToolTip(accountAction->statusTip());
    accountAction->setCheckable(true);
    accountAction->setShortcut(QKeySequence(QStringLiteral("Alt+6")));
    tabGroup->addAction(accountAction);
    ...
    connect(stakingMiningAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(stakingMiningAction, &QAction::triggered, this, &BitcoinGUI::gotoStakingMiningPage);
    ...
    connect(accountAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(accountAction, &QAction::triggered, this, &BitcoinGUI::gotoAccountPage);
#endif // ENABLE_WALLET
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        toolbar->addAction(stakingMiningAction);
...
void BitcoinGUI::enableHistoryAction(bool privacy)
{
    ...
    accountAction->setEnabled(!privacy);
    if (historyAction->isChecked() || accountAction->isChecked()) gotoOverviewPage();
}
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    stakingMiningAction->setEnabled(enabled);
    sendCoinsAction->setEnabled(enabled);
    receiveCoinsAction->setEnabled(enabled);
    historyAction->setEnabled(enabled);
    accountAction->setEnabled(enabled);
...
void BitcoinGUI::gotoStakingMiningPage()
{
    stakingMiningAction->setChecked(true);
    if (walletFrame) walletFrame->gotoStakingMiningPage();
}
...
void BitcoinGUI::gotoAccountPage()
{
    accountAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAccountPage();
    QTimer::singleShot(0, this, [this] { fitToScreen(); });
}
...
void BitcoinGUI::changeEvent(QEvent *e)
{
    if (e->type() == QEvent::PaletteChange) {
        ...
        stakingMiningAction->setIcon(platformStyle->SingleColorIcon(QStringLiteral(":/icons/tx_mined")));
        sendCoinsAction->setIcon(platformStyle->SingleColorIcon(QStringLiteral(":/icons/send")));
        receiveCoinsAction->setIcon(platformStyle->SingleColorIcon(QStringLiteral(":/icons/receiving_addresses")));
        historyAction->setIcon(platformStyle->SingleColorIcon(QStringLiteral(":/icons/history")));
        accountAction->setIcon(platformStyle->SingleColorIcon(QStringLiteral(":/icons/address-book")));
    }
...
void BitcoinGUI::fitToScreen()
{
    ...
    available.adjust(margin, margin, -margin, -margin);
    if (!available.isValid()) return;
