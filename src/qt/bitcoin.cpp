...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in blackcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in blackcoin.qrc)
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(blackcoin);
    Q_INIT_RESOURCE(blackcoin_locale);
    ...
    /// 6-7. Parse blackcoin.conf, determine network, switch to network specific
