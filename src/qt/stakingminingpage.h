...
private:
    ...
    // Default false so the FIRST show of the tab does only the cheap status
    // update and never an expensive detail sweep. The detail panels load only
    ...
    QPushButton* m_coldstake_redelegate{nullptr};
