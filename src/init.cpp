...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    */
    ...
    argsman.AddArg("-allowunsafequantumkeyrpc", "Process-wide expert opt-in for raw ML-DSA private-key RPCs. createquantumkey is not wallet-scoped and does not store or back up its output; dumpquantumkey exports from the selected normally unlocked wallet and rejects staking-only unlock. This option does not disable networking or restrict RPC access. Use only in an independently isolated offline process. Disabled by default.", ArgsManager::ALLOW_ANY, OptionsCategory::RPC);
