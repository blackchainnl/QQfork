...
## Protected final-release evidence path

Production publication does not accept a manually uploaded count or an
inventory generated by an unrelated resident node. Run the
`v30.1.1 exact mainnet quantum witness inventory` workflow from the exact final
candidate ref. The protected `production-resource-evidence` environment must
provide the same immutable connected-tip manifest, archival snapshot, and
...
connected-tip capture must be no more than 24 hours old when production
publication re-verifies it. The workflow artifact retains the evidence,
authorization, capture manifest, exact verifier binaries, production contract,
and any approved dispositions.

The signed `v30.1.1` publication path downloads only a successful, unexpired
artifact from `.github/workflows/quantum-witness-inventory.yml` at the exact
release SHA. It reruns the offline verifier against the bundled binaries and
contract, requires byte-identical verifier-derived authorization, and copies
the public evidence, authorization, capture manifest, and any dispositions into
the signed release bundle. Missing, stale, mismatched, manually substituted, or
incompletely dispositioned evidence blocks publication.
