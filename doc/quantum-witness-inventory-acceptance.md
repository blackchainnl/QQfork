...
## Exact-candidate mainnet procedure
...
field is inserted. Preserve the emitted JSON byte-for-byte as an optional
post-release qualification artifact.

## Optional protected post-release evidence path

When live-mainnet qualification is requested, do not accept a manually
uploaded count or an inventory generated by an unrelated resident node. Run
the `v30.1.1 exact mainnet quantum witness inventory` workflow from the exact
release ref. The protected `production-resource-evidence` environment must
then provide the same immutable connected-tip manifest, archival snapshot, and
external state root used by the paired resource gate through
`SHADOW_RESOURCE_FIXTURE_MANIFEST`, `SHADOW_RESOURCE_FIXTURE_ARCHIVE`, and
`SHADOW_RESOURCE_STATE_ROOT`. If a review set exists, the environment also
provides `QUANTUM_WITNESS_DISPOSITIONS`; its exact digest is a required workflow
input. A zero review set must not supply a dispositions file or digest.

The protected workflow rebuilds `blackcoind` and `blackcoin-cli` from the exact
source, safely extracts the manifest-authenticated archival snapshot, launches
the verifier-owned offline node, performs the complete paged inventory and
same-tip reconciliations, and derives a separate authorization document. The
connected-tip capture must be no more than 24 hours old when the optional
workflow verifies it. The workflow artifact retains the evidence,
authorization, capture manifest, exact verifier binaries, production contract,
and any approved dispositions.

The standalone workflow reruns the offline verifier against the bundled
binaries and contract and requires byte-identical verifier-derived
authorization. The immutable production release does not consume or append
this artifact. Missing runners, capture paths, or evidence do not block
publication and leave #14 Partial. When the optional workflow is run, stale,
mismatched, manually substituted, or incompletely dispositioned evidence fails
the qualification.
...
the final Migration block and first Final Lockout block. A live mainnet
artifact remains necessary to move #14 from Partial to Verified because
regtest fixtures cannot prove the current mainnet UTXO set or the live shadow
ledger. It is optional post-release qualification, not a publication
prerequisite. During the ongoing 243,000-block Gold Rush, that artifact is
partial-epoch evidence only. It cannot be represented as evidence that the
future epoch has completed.
